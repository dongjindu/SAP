*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVHR_EVAPD
*   generation date: 01/12/2013 at 14:32:23 by user T00289
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVHR_EVAPD          .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVHR_EVAPD'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVHR_EVAPD_EXTRACT
TOTAL = ZVHR_EVAPD_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVHR_EVAPD'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVHR_EVAPD-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVHR_EVAPD_EXTRACT
TOTAL = ZVHR_EVAPD_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVHR_EVAPD-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVHR_EVAPD'
...
TOTAL = ZVHR_EVAPD_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVHR_EVAPD-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVHR_EVAPD-UPD_FLAG NE SPACE.
