*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVHR_ADMIN
*   generation date: 03/15/2013 at 17:57:17 by user T00289
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVHR_ADMIN          .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVHR_ADMIN'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVHR_ADMIN_EXTRACT
TOTAL = ZVHR_ADMIN_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVHR_ADMIN'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVHR_ADMIN-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVHR_ADMIN_EXTRACT
TOTAL = ZVHR_ADMIN_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVHR_ADMIN-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVHR_ADMIN'
...
TOTAL = ZVHR_ADMIN_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVHR_ADMIN-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVHR_ADMIN-UPD_FLAG NE SPACE.
