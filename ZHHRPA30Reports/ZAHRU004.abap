*----------------------------------------------------------------------
* Program ID        : ZAHRU004
* Title             : [HR] LOA Tracking Report
* Created on        : 3/10/2009
* Created by        : I.G.MOON
* Specifications By : EUNA LEE
* Description       : [HR] LOA Tracking Report
*&--------------------------------------------------------------------&*
* Modification Logs
* Date       Developer  Issue No   Description
* 03/16/2010 VALERIAN              Move cost center MXTX12 from Engine
*            HIS20094              to Assembly.
* 03/29/2010 Valerian              Remove unnecessary action type
*                                  checking
* 10/23/2012 Valerian   UD1K955714 Add column for time admins ID and
*                                  Description
*&--------------------------------------------------------------------&*

REPORT zahru004 MESSAGE-ID zmco.

TABLES: pernr, pa0006, PER_DAYS.
INFOTYPES: 0000,
           0001,0002,
           0007 MODE n,
           0041 MODE n,
...
* email/dispaly
SELECTION-SCREEN BEGIN OF BLOCK b5 WITH FRAME TITLE text-020.
PARAMETERS: p_dis AS CHECKBOX DEFAULT 'X',
            p_email AS CHECKBOX.

SELECT-OPTIONS: s_days for PER_DAYS-DAY_PER.
...
START-OF-SELECTION.
  ...
  if p_email is NOT INITIAL and p_rver is INITIAL.
    MESSAGE e000 WITH 'Must input Distribution List'.
  endif.
GET pernr.
    ...
    LOOP AT p0000.
      IF p0000-massn EQ 'ZB' OR p0000-massn EQ 'ZC'.
        ...
        IF sy-subrc = 0 AND p0000-stat2 = l_stat2
          ...
          p0000-massn = l_massn.
          p0000-massg = l_massg.
          MODIFY p0000 INDEX l_index TRANSPORTING massn massg.
        ENDIF.
      ENDIF.
    ENDLOOP.
** End
...
** On 03/14/14
  IF $it_calc_pernr-endda IS INITIAL OR $it_calc_pernr-endda > pn-endda.
    $it_calc_pernr-cnt = pn-endda - $it_calc_pernr-begda + 1.
  ELSE.
...
FORM SEND_EMAIL .
  ...
  data: l_low(3), l_high(3), l_title type P15_TEXT150.

  CONCATENATE 'The Following TM' `'` 's have been on LOA between'
               into lt_body.
  l_low = s_days-low.
  l_high = s_days-high.

  CONCATENATE lt_body l_low 'and' l_high 'consecutive days:'
...
*  MOVE '===========================================' TO lt_body.
  ...
  MOVE: 'TM #' TO lt_body+0(10),
        'Name' TO lt_body+10(30),
        'Days' TO lt_body+40(10),
        'Start Date' TO lt_body+50(10).
  ...
  LOOP AT gt_out.
    if gt_out-cnt in s_days and gt_out-pernr is not INITIAL .
    MOVE: gt_out-pernr to lt_body+0(10),
          gt_out-sname to lt_body+10(30),
          gt_out-cnt TO lt_body+40(10).
    write: gt_out-begda MM/DD/YYYY to  lt_body+50(10).
    APPEND lt_body.
    endif.
  ENDLOOP.

  l_title = 'Consecutive Days LOA, Approaching'.
  CONCATENATE l_title l_high 'days' into l_title SEPARATED BY space.
