...
** Furong on 05/18/12 for SAP Tuning
...
*   w_col_pos 'NETWR' 10 'Actual PO Amt'  'CURR' ''   '' 'WAERS' '',
  ...
  DO 21 TIMES.
    READ TABLE it_day_text INDEX sy-index.
    MOVE : it_day_text-index(2) TO l_index.
    CONCATENATE : 'BDD' l_index INTO l_field01,
                  it_day_text-day(2) '/' it_day_text-day+2(2)
                                       INTO l_title.
    append_fieldcat :
...
FORM get_requirement_quantity.
*---
  PERFORM get_date_text USING sy-datum.

*  PERFORM get_week_text.
*
*  PERFORM get_month_text.
  ...
  LOOP AT it_matnr.
    ...
    DO.
      CALL FUNCTION 'Z_FPP_MD_STOCK_REQ_LIST_API'
        STARTING NEW TASK wa_taskname DESTINATION IN GROUP 'PG_STK'
...
FORM get_data_from_ztable.
  ...
  IF it_itab-cdate IS INITIAL.
    MESSAGE e999 WITH text-m03.
    EXIT.
  ENDIF.
  PERFORM get_date_text USING it_itab-cdate.
...
FORM save_data_to_ztable.
  ...
  ELSE.
    ROLLBACK WORK.
    MESSAGE s999 WITH 'Update ZTMM_TRIM_PLANR failed'.
  ENDIF.
...
FORM return_process USING p_taskname.
  ...
  LOOP AT lt_it_mdsux WHERE planr EQ space
                            AND delkz EQ space.
    MOVE : wa_matnr TO it_day-matnr.
    it_day-mng02 = lt_it_mdsux-mng02 * ( - 1 ).
    MOVE : lt_it_mdsux-dat00+4(4) TO it_day-day.
    COLLECT : it_day.
    CLEAR : it_day,lt_it_mdsux.
  ENDLOOP.
