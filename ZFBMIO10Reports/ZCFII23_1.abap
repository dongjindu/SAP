...
SELECTION-SCREEN BEGIN OF LINE.
SELECTION-SCREEN COMMENT (15) C_ORTYPE.
SELECTION-SCREEN POSITION 23.
PARAMETERS : P_AUART LIKE AUFK-AUART.
SELECTION-SCREEN END OF LINE.
*---------------------------------------------
SELECTION-SCREEN BEGIN OF LINE.
...
SELECTION-SCREEN POSITION 20.
SELECT-OPTIONS:
  S_POSID   FOR   IMPR-POSID. "IMPR-POSNR.
SELECTION-SCREEN END OF LINE.

SELECTION-SCREEN BEGIN OF LINE.
SELECTION-SCREEN COMMENT (15) C_PRNAM.
SELECTION-SCREEN POSITION 23.
...
*ARAMETERS: p_gnjhr LIKE impr-gjahr MEMORY ID gjr.
*------

SELECT-OPTIONS:
S_AKSTL FOR AUFK-AKSTL.
***********************
SELECTION-SCREEN BEGIN OF BLOCK S4 WITH FRAME TITLE C030.
...
*-----Start
    ...
    CLEAR L_OBJNR.
*Asset
    CONCATENATE 'OR' IT_OUT-AUFNR INTO L_OBJNR.
    SELECT SINGLE ANLN1 INTO GT_OUT-ANLN1
     FROM ANLI
      WHERE OBJNR EQ L_OBJNR.
*class and capitalization date
    SELECT SINGLE ANLKL AKTIV INTO (GT_OUT-ANLKL, GT_OUT-AKTIV)
       FROM ANIA
        WHERE OBJNR EQ L_OBJNR.
...
*  ENDIF.
  ...
  SELECT AUFNR AUART KTEXT KOSTV AKSTL
         USER0 USER4 PHAS0 PHAS1 PHAS2 PHAS3
         STORT KAPPL FUNC_AREA KDAUF AUFEX
    ...
    JOIN IMPR AS C
      ON C~POSNR = B~POSNR
     AND C~GJAHR = B~GJAHR
   WHERE C~POSID IN S_POSID
     AND C~PRNAM = P_PRNAM
     AND C~KOSTL IN S_AKSTL.
