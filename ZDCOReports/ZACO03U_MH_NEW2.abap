...
*---collect unpaid leave
    PERFORM collect_UNPAID_time.
...
*Employee
    i_emp-pernr = py_times-pernr. COLLECT i_emp.
...
*
** Adjust avg. hc.....................
*  DATA: l_fte  TYPE pranz,
*        l_avg  TYPE pranz.
*
*  LOOP AT it_time.
*    AT END OF pernr.
*      SUM.
*      IF it_time-emp_avg <> 1.
*        READ TABLE i_emp WITH KEY pernr = it_time-pernr.
*        IF  i_emp-hire <= s_workdt-low
*        AND i_emp-fire >  s_workdt-high.
*          it_time_cp         = it_time.
*          it_time_cp-emp_avg = 1 - it_time-emp_avg.
*          APPEND it_time_cp.
*        ENDIF.
*      ENDIF.
*    ENDAT.
*  ENDLOOP.
*
**fill back to it_time
*  LOOP AT it_time_cp.
**    READ TABLE it_time WITH KEY pernr = it_time_cp-pernr
**                                lgart = '1'.
**    IF sy-subrc = 0.
*    LOOP AT it_time WHERE pernr = it_time_cp-pernr
*                      AND lgart = '1'.
*      l_tabix = sy-tabix.
*      it_time-emp_avg = it_time-emp_avg + it_time_cp-emp_avg.
**      it_time-hire    = it_time_cp-hire.
**      it_time-fire    = it_time_cp-fire.
...
*&      Form  calc_work_days
*&---------------------------------------------------------------------*
FORM calc_work_days USING    p_d1
                             p_d2
                    CHANGING p_cnt.
...
* Headcount - valid on end of month(working day)
  ...
  LOOP AT i_emp WHERE fire >= s_workdt-high.
    LOOP AT it_time WHERE pernr = i_emp-pernr.
*                     AND lgart = '1'
*                     AND dtype = '1'.
      l_tabix = sy-tabix.
      it_time-emp_cnt = 1.
...
*---- error... FIXME
    ...
    ELSE.
      ...
      IF it_time-kostl(2) = 'MX'.
        PERFORM calc_net_mh USING    it_time-anzhl
                            CHANGING it_time-netmh.
       it_time-netmh = it_time-netmh * -1.
      ELSE.
        it_time-netmh = it_time-anzhl * -1.
      ENDIF.

      it_time-anzhl = it_time-anzhl * -1.
...
FORM collect_supporting_time.
  ...
  data : l_skostl like catsdb-skostl.
