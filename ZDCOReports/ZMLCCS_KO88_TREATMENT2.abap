************************************************************************
report  zmlccs_ko88_treatment2 .
************************************************************************
*...
*02.08.2002 AHY (Do not read all orders if no order number specified.)
************************************************************************
*Selection texts:
...
parameters:
   p_bukrs type bukrs memory id buk modif id wri.
...
* ansonsten: kstar-Einträge zu Aufträgen löschen!
else.
...
*-----------------------------------------------------------------------
write: / 'Control data: ',
       / 'Controlling area = ', s_control_data-kokrs,
       / 'Posting period   = ', s_control_data-poper,
       / 'Fiscal year      = ', s_control_data-gjahr.

skip 2.

write: / 'Legend for TREATMENT_TYPE:'.
write: / 'S = split balance of production order in accordance',
         '    with cost component split of S-Price'.
write: / 'K = assign disputed costs into cost component structure',
         '    on the basis of cost element and origin group'.
skip.
*-----------------------------------------------------------------------
...
loop at lt_order_old into ls_order_old
   ...
   message i101(c+) with 'Delete existing settings'
                         'for order number = * '
...
if not ip_kstar is initial.
   if ip_bukrs is initial.
      message i101(c+) with 'Please enter company code.' '' '' ''.
      c_error = k_x.
      exit.
   else.
