...
* 9/08/2011  Valerian     UD1K953055   Combine CATSDB Data source with
*                                      PA2002. This program is used
*                                      for temporary purpose. Program
*                                      will be removed after golive.
*
...
PARAMETERS :
               ...
               p_trun(1).
SELECT-OPTIONS: s_begda FOR p0001-begda NO-EXTENSION.       "UD1K953055
...
* BEGIN OF UD1K953055
  ...
  IF sy-subrc = 0.
    IF s_begda-high IS INITIAL.
      ...
      MODIFY s_begda INDEX 1.
    ENDIF.

    IF s_begda-low >= dat1 AND
       s_begda-high <= dat2.

      dat2 = s_begda-low - 1.
      IF dat2 < dat1.
        MESSAGE e000 WITH 'Start Date should be greater than' dat1.
      ENDIF.

    ELSE.
      MESSAGE e000 WITH 'Start Date should be in period:' p_perid.
    ENDIF.
  ENDIF.
* END OF UD1K953055
