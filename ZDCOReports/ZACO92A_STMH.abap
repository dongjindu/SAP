...
*& Addl documentation     :
*& Description            : Standard M/H each MIP Material (FERT OR
*HALB)
*& Modification Log
...
*----------------------------------------------------------------------*
FORM extract_material.
  REFRESH it_zcor07.CLEAR it_display.
  LOOP AT it_display WHERE zu_lbkum <> 0.
    MOVE : it_display-matnr    TO it_zcor07-matnr,
           it_display-zu_lbkum TO it_zcor07-zu_lbkum.
    APPEND it_zcor07.
    CLEAR it_zcor07.
  ENDLOOP.
  ...
  SELECT kokrs bdatj aufnr fsc_matnr shop llv_matnr kstar elemt
         ...
         INTO CORRESPONDING FIELDS OF TABLE it_shopcost_at
          FROM ztco_shopcost_at
            FOR ALL ENTRIES IN it_zcor07
           WHERE kokrs EQ p_kokrs
             AND bdatj EQ p_bdatj
             AND poper EQ p_poper
             AND typps IN ('M','V', 'E')
              AND elemt EQ '070'
             AND fsc_matnr EQ it_zcor07-matnr.
...
FORM gathering_data_shopcost USING p_klvar.
  CLEAR it_zcor07.REFRESH :it_st_source,it_shopcost.
  CASE p_klvar.
    WHEN 'PPC1'.
...
FORM ml_qty_filled.
  LOOP AT it_ztco_stmh.
    READ TABLE it_zcor07 WITH KEY matnr = it_ztco_stmh-fsc_matnr.
    IF sy-subrc = 0.
