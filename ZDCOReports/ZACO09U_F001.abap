...
FORM put_shop_mat_info.
  ...
  DATA : BEGIN OF it_ckmlmv011 OCCURS 0,
...
* Sort (Material - ALL)
  ...
  LOOP AT it_kis1.
    ...
    it_tmp_shopcost-lot_size = it_kis1-losgr.
* Controlling Area
...
* Parents Material (FSC)
    ...
    it_tmp_shopcost-kalst     = it_ckikekokey-kalst.
...
* Child Material Info
    ...
    IF it_tmp_shopcost-typps = 'M'.
      CLEAR it_tmp_shopcost-kalnr.
      SELECT SINGLE kalnr INTO it_tmp_shopcost-kalnr
          FROM ckmlhd
         WHERE matnr = it_tmp_shopcost-llv_matnr
           AND bwkey = it_tmp_shopcost-bwkey.
    ENDIF.
* Period (From / To)
*Issue # 20041201-001 requested by HS CHO
*Changed by wskim,on 01112005
...
FORM tre_fraction_to_v.
  ...
  DATA :  kokrs         LIKE it_tmp_shopcost-kokrs,
          ...
          lot_size      LIKE it_tmp_shopcost-lot_size.
...
* Value
        ...
        it_frac_addon-lot_size  = it_khs1-menge.
...
* Appending
  ...
  LOOP AT it_tmp_shopcost.
    MOVE-CORRESPONDING it_tmp_shopcost TO it_shop_pln.
    IF it_shop_pln-kostl <> ''.
      it_shop_pln-shop = it_shop_pln-kostl(4).
    ENDIF.
    it_shop_pln-par_werks = it_tmp_shopcost-werks.
    IF it_shop_pln-typps = 'M'.
      IF it_shop_pln-par_werks <> ''.
        CONCATENATE it_shop_pln-par_werks  it_shop_pln-llv_matnr
        INTO  it_shop_pln-resou SEPARATED BY space.
      ELSE.
        CONCATENATE it_shop_pln-bwkey  it_shop_pln-llv_matnr
        INTO  it_shop_pln-resou SEPARATED BY space.
      ENDIF.
    ELSE.
      CONCATENATE it_shop_pln-kostl  it_shop_pln-lstar
         INTO it_shop_pln-resou SEPARATED  BY space.
    ENDIF.
    CLEAR : it_shop_pln-menge, it_shop_pln-lot_size.
    COLLECT it_shop_pln. CLEAR it_shop_pln.
  ENDLOOP.

  LOOP AT it_shop_pln.
    CLEAR it_tmp_shopcost.

    READ TABLE it_tmp_shopcost WITH KEY klvar     =  it_shop_pln-klvar
                                werks     =  it_shop_pln-par_werks
                                fsc_matnr =  it_shop_pln-fsc_matnr
                                typps     =  it_shop_pln-typps
                                kstar     =  it_shop_pln-kstar
                                LLV_MATNR =  it_shop_pln-LLV_MATNR
                                kostl     =  it_shop_pln-kostl.
    it_shop_pln-lot_size = it_tmp_shopcost-lot_size.
    it_shop_pln-menge    = it_tmp_shopcost-menge.
    MODIFY it_shop_pln. CLEAR it_shop_pln.
  ENDLOOP.


  LOOP AT it_tmp_shopcost.
    MOVE-CORRESPONDING it_tmp_shopcost TO it_shop_pln_cc.
    IF it_shop_pln_cc-kostl <> ''.
      it_shop_pln_cc-shop = it_shop_pln_cc-kostl(4).
    ENDIF.
    it_shop_pln_cc-par_werks = it_tmp_shopcost-werks.
    IF it_shop_pln_cc-typps = 'M'.
      IF it_shop_pln_cc-par_werks <> ''.
        CONCATENATE it_shop_pln_cc-par_werks  it_tmp_shopcost-llv_matnr
        INTO  it_shop_pln_cc-resou SEPARATED BY space.
      ELSE.
        CONCATENATE it_shop_pln_cc-bwkey  it_tmp_shopcost-llv_matnr
        INTO  it_shop_pln_cc-resou SEPARATED BY space.
      ENDIF.
    ELSE.
      CONCATENATE it_shop_pln_cc-kostl  it_shop_pln_cc-lstar
         INTO it_shop_pln_cc-resou SEPARATED  BY space.
    ENDIF.
    COLLECT it_shop_pln_cc . CLEAR it_shop_pln_cc.
  ENDLOOP.

  DATA : it_shop_plan2 LIKE it_shop_pln_cc OCCURS 0 WITH HEADER LINE.
  DATA : it_temp_plan LIKE it_shop_pln_cc OCCURS 0 WITH HEADER LINE.

  it_temp_plan[] = it_shop_pln_cc[].

*  loop at it_shop_pln_cc .
*     read table it_temp_plan with key BDATJ = it_shop_pln_cc-BDATJ
*                                      POPER
*                                      KLVAR
*                                      FSC_MATNR
*                                      SHOP
...
*                                      HWAER
*                                      MEEHT
*   check it_shop_pln_cc-FSC_MATNR = '4B28XXEMFDT65 0009' .
*   check  it_shop_pln_cc-ELEMT = '070' .
*   check it_shop_pln_cc-LLV_MATNR = '771113K010'.
*       break-point.
*       move-corresponding it_shop_pln_cc  to it_shop_plan2.
*       append it_shop_plan2.
*  endloop.
*
*
*
...
*-----Start
  CLEAR : record_num.
  LOOP AT it_shop_pln WHERE bdatj EQ  p_bdatj
                            AND poper EQ p_perab.
*-----End
* LOG
...
*  BREAK-POINT.
  CLEAR : record_num.
  LOOP AT it_shop_pln_cc WHERE bdatj EQ  p_bdatj
                            AND poper EQ p_perab.
*-----End
* LOG
...
*    endif.
*    clear ztco_shopcost.
*    if  it_shop_pln_cc-FSC_MATNR = '4B28XXEMFDT65 0009'
*    and it_shop_pln_cc-ELEMT = '070'
*    and it_shop_pln_cc-LLV_MATNR = '771113K010'.
*       break-point.
*    endif.
*
*    modify it_shop_pln_cc. clear it_shop_pln_cc.
*    l_Cnt = l_Cnt + 1.
*  endloop.
*
*  insert ztco_shop_pln_cc from table it_shop_pln_cc.
*  commit work.
*
*
*    move-corresponding  it_shop_pln_cc to ztco_shop_pln_cc.
