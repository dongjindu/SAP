************************************************************************
* Program Name      : ZACOU142
* ( Copy Actual to NAFTA – Distributed Cost )
* Author            : IG.Moon
...
*
************************************************************************
report zacou142 message-id zmco.
include zacoui00.
...
form get_data.
  ...
  from cosp
          into corresponding fields of table gt_cosp
           where lednr = '00'
             ...
             and wrttp = '04'
             and versn = '000'
             group by objnr kstar twaer meinh.
  ...
  loop at it_row_tab.
    ...
    call function 'Z_CO_RATE_GENERIC_GET'
         exporting
              i_kokrs = p_kokrs
              i_kostl = it_row_tab-kostl
              i_kstar = it_row_tab-kstar
         tables
              t_rate  = g_t_rate.

    read table g_t_rate index 1.
    if sy-subrc eq 0 and g_t_rate-rate_d <> 0.
      <fs_dis> = <fs_wkg> * ( g_t_rate-rate_d / 100 ).
    else.
