*$*$--------------------------------------------------------------$*$*
*$ Correction Inst.         0120061532 0000331386                     $*
*$------------------------------------------------------------------$*
*$ Valid for       :                                                  $*
*$ Software Component   ISPSADIN   ISPSADIN - Merge IS-PS & DI        $*
*$  Release 10A          All Support Package Levels                   $*
*$ Software Component   DI         Discrete Industries (IS-A, IS-AD, C$*
*$  Release 46B          All Support Package Levels                   $*
*$  Release 46C2         All Support Package Levels                   $*
*$*$--------------------------------------------------------------$*$*
*&-------------------------------------------------------------------*
*& Object          REPS PPC_CONSISTENCY_CHECK
*& Object Header   PROG PPC_CONSISTENCY_CHECK
*&-------------------------------------------------------------------*
*& REPORT PPC_CONSISTENCY_REPORT
*&-------------------------------------------------------------------*
*>>>> START OF INSERTION <<<<
...
FORM DATA_PREPARE TABLES   lt_material_components
  ...
  SELECT
     ...
     t1~flg_asynch
     t1~orderid
  INTO (ls_ext_mat_comp-ACCASSOBJ,
...
* Selektiere die Ausschu?eldungen
  SELECT
      t1~accassobj
      t1~postdate
      t2~matnr
...
* a) MARA
    ...
    CALL FUNCTION 'MATERIAL_READ'
         EXPORTING
              SCHLUESSEL = LS_MTCOM  " Materialstammschl?selfelder
         IMPORTING
...
* Schl?selfelder f? WM-Daten
  DATA: LS_WMDATA_KEY TYPE PPCPR_TYPE_WMDATA_KEY.
* Arbeitsstruktur f? WM-Daten
...
* Fehler bei WM-Aufruf?
  ...
  CLEAR EF_WMREL.
*--> Pr?en, ob Lagerort WM-relevant ist
...
* falls Kundeneinzelbestand: f?le  kdauf, kdpos
    ...
    ENDIF.
* falls Projektbestand: f?le pspel
...
*--> Initialisierung
  ...
  READ TABLE ST_WM_RELEV INTO LS_WM_RELEV
                         WITH KEY WERKS = IF_WERKS
                                  LGORT = IF_LGORT.
  IF SY-SUBRC <> 0.
* Pr?ung f? Werk, Lagerort noch nicht erfolgt
