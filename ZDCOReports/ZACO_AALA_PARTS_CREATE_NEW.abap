...
DATA: BEGIN OF it_altfsc OCCURS 0,
      ...
      END OF it_altfsc.
DATA: it_shopcost LIKE ztco_shop_pln OCCURS 10 WITH HEADER LINE.
...
parameters : p_kokrs like keko-kokrs memory id CAC  OBLIGATORY,
             ...
             p_KLVAR LIKE keko-KLVAR  MEMORY ID KRT OBLIGATORY.

select-options: s_perab for keko-POPER MEMORY ID vpe OBLIGATORY.
SELECTION-SCREEN END OF BLOCK bl2.
SELECTION-SCREEN BEGIN OF BLOCK bl1 WITH FRAME TITLE text-001.
*version
PARAMETERS:  p_aalayr  LIKE ztco_aala_source-gjahr default sy-datum(4),
...
* CHECK IF FSC COSTING EXIST IN SELECTED PLAN/TIME
  ...
  sort it_shopcost by artnr.
  LOOP AT it_mat.
    READ TABLE it_shopcost WITH KEY artnr = it_mat-matnr binary search.
    IF sy-subrc NE 0.
...
FORM read_shopcost.
  SELECT * INTO TABLE it_shopcost
   ...
   WHERE kokrs = p_kokrs
     AND bdatj = p_bdatj
     AND poper in s_perab
     and KLVAR = p_KLVAR
     and artnr IN p_matnr
*     AND record_type = p_record_type
     AND typps = 'M'
     and kstar <> c_sub_cos_ele.   "Exclude oil/...
...
* reading the control area currency key
  ...
  LOOP AT it_shopcost.
    ...
    it_aala-gjahr = p_aalayr. "sy-datum(4).
    ...
    it_aala-peinh = it_shopcost-peinh.
    it_aala-meeht = it_shopcost-meeht.
...
*        PERFORM DELETE_VERSION.
        ...
        APPEND it_output.
*    SAVE THE PLAN USED
        CONCATENATE p_bdatj s_perab-low p_KLVAR
...
* COMBINE THE CONDITION AND PRICE
  sort it_info by knumh.
  LOOP AT it_price.
    READ TABLE it_info WITH KEY knumh = it_price-knumh binary search.
    IF sy-subrc EQ 0.
...
* COMBINE THE ZP13 TYPE PRICE (AALA PRICE)
  ...
  SORT it_price BY kschl knumh datbi datab lifnr matnr.
  LOOP AT it_info.
...
FORM serach_mip_raw.
  ...
  SELECT * INTO TABLE mt_shopmip
   ...
   WHERE kokrs = p_kokrs              AND
         bdatj = p_bdatj              AND
         poper in s_perab             AND
         KLVAR = p_KLVAR              AND
         artnr = lt_shopmip-llv_matnr AND
