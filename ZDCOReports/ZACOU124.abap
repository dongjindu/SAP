*----------------------------------------------------------------------
* Program ID        : ZACOU124
* Title             : [CO] Duty ME Reconciliation
...
* 01/28/2011 VALERIAN   UD1K950699 Correct GR total amount.
* 01/31/2011 VALERIAN   UD1K950705 Correct GR total amount - 2
*----------------------------------------------------------------------
*
...
*
*----------------------------------------------------------------------
REPORT zacou124 MESSAGE-ID zmco.
INCLUDE : zacoui00,zacou124_top.
*----------------------------------------------------------------------
* Selection-Screen
*----------------------------------------------------------------------
...
FORM get_data.
  ...
  CASE p_entry.
    WHEN '06'.
      PERFORM get_row_data_06.
      CHECK g_error EQ space.
      PERFORM : get_row_data_ekbz_06_08,
                refine_itab_06_08.
    WHEN '08'.
...
*  SELECT b~entno b~matnr
*         a~bukrs a~hkont a~sgtxt AS fentp a~gjahr a~belnr
*         a~dmbtr
...
*       ON  b~entno EQ a~zuonr
*     WHERE a~bukrs EQ p_bukrs
*      AND  a~hkont EQ p_hkont
*      AND  a~sgtxt EQ p_entry
*      AND  b~entno IN s_entno
*      AND  b~entno IN ( select ENTNO from ZTMM_DUTY_HD
*                              WHERE entdate IN s_entrd )
*     group BY a~bukrs A~HKONT a~sgtxt a~gjahr a~belnr a~dmbtr
*              b~entno b~matnr b~entcurr b~uom b~cl_doc_no.
  ...
  LOOP AT sum_i_bsis.
    SELECT entno matnr
       ...
       AND  entno IN ( select ENTNO from ZTMM_DUTY_HD
                                WHERE entdate IN s_entrd )
       group BY entno MATNR entcurr uom cl_doc_no.
  ENDLOOP.
...
***
    ...
    MOVE : i_bsis-menge TO *ztcou124-xmenge,
           ...
           i_bsis-pstqty TO *ztcou124-rmenge.
...
FORM create_calc_tab TABLES p_gt_calc STRUCTURE gt_calc
  ...
  DATA : BEGIN OF $itab OCCURS 0,
           index     LIKE sy-tabix,
           dtyamt    LIKE ztcou124-duty_amt, "Amount
         END   OF  $itab.

  quantity =  tempq.
  m_idx = 0.

  CLEAR *ztcou124-dtyup. " Clear garbage
  ...
  IF sy-subrc EQ 0.
    ...
    IF dtyamt GT 0.
      IF balance GE quantity.
         *ztcou124-dtyup = dtyamt / quantity .
      ELSE.
        IF balance <> '0'.
           *ztcou124-dtyup = dtyamt / balance .
        ENDIF.
      ENDIF.
    ENDIF.
  ENDIF.
...
***   Decrease quantity from old PO#
    ...
    MOVE : *ztcou124-entno         TO p_gt_calc-entno,
           *ztcou124-xmenge        TO p_gt_calc-xmenge,
           *ztcou124-xmenge2       TO p_gt_calc-xmenge2,
           *ztcou124-entamount     TO p_gt_calc-entamount,
           *ztcou124-duty_amt      TO p_gt_calc-duty_amt,
           *ztcou124-mpf_amt       TO p_gt_calc-mpf_amt,
           *ztcou124-dtyup         TO p_gt_calc-dtyup,
           it_row_tab-rmenge       TO p_gt_calc-rmenge,
           it_row_tab-bmenge       TO p_gt_calc-bmenge,
           *ztcou124-rmenge        TO p_gt_calc-pstqty,
           *ztcou124-gjahr         TO p_gt_calc-gjahr.
...
** Adjust total amount with Duty amount
  ...
  DATA : $dtyamt LIKE ztcou124-duty_amt,
         $adjust LIKE ztcou124-duty_amt.
...
FORM reduce_matnr_sum TABLES   p_itab_for_matnr_sum STRUCTURE
  ...
  READ TABLE p_itab_for_matnr_sum INDEX p_m_idx .

  IF sy-subrc EQ 0.
    itab_for_matnr_sum-bmenge = itab_for_matnr_sum-bmenge -
              ( itab_for_matnr_sum-bmenge - p_tempq ).

    MODIFY itab_for_matnr_sum INDEX p_m_idx.
  ENDIF.
...
FORM create_none_po   TABLES p_gt_calc STRUCTURE gt_calc
   ...
   *ztcou124-dtyup = 0. " Clear garbage
  CLEAR p_gt_calc.

  IF tempq EQ 0.
     *ztcou124-dtyup = 0..
  ELSE.
     *ztcou124-dtyup = dtyamt / tempq .
  ENDIF.

  tempq = -1 * tempq .

  MOVE-CORRESPONDING *ztcou124 TO p_gt_calc.
...
FORM dist_amount_by_bsis USING    p_$key
  ...
  DATA : BEGIN OF $itab OCCURS 0,
           index     LIKE sy-tabix,
           dtyamt    LIKE ztcou124-duty_amt, "Amount
           dstamt    LIKE ztcou124-dstamt, "Amount
         END   OF  $itab.
...
*      P_GT_CALC-ACCAMT TO P_GT_CALC-ACCAMTS UNIT P_GT_CALC-WAERS,
*      P_GT_CALC-VARAMT TO P_GT_CALC-VARAMTS UNIT P_GT_CALC-WAERS.
...
FORM get_row_data_08.
  ...
  SELECT a~entno b~matnr
     ...
     GROUP BY A~FENTP a~entno A~DUTY_AMT
              b~matnr b~entcurr b~uom b~cl_doc_no.
