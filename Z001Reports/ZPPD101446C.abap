...
* PD/Org Infotype 1014
* Authors : Mrudula - Annance Consulting
*---------------------------------------------------------------------*

REPORT ZPPD101446C MESSAGE-ID ZP.

* SELECTION SCREEN
** PARAMETER

PARAMETER : FILE1014 LIKE  RLGRAP-FILENAME DEFAULT
          'C:\WINDOWS\SAP\1014.txt'.

* data decleration
** Tables
** internal tables

DATA : BEGIN OF _P1014 OCCURS 10.
...
PERFORM READ_DATA.
PERFORM INIT_BDC USING 'HRPD1014' SY-UNAME.
LOOP AT _P1014.
...
FORM POPULATE_BDC.
   ...
   PERFORM DYNPRO TABLES BDC_DATA USING:
            ...
            ' ' 'PPHDR-OTYPE' _P1014-OTYPE ' ', "OBJ TYPE
            ' ' 'PM0D1-SEARK' _P1014-OBJID ' ', "OBJ ID
            ' ' 'PPHDR-INFTY' '1014' ' ',       "INFOTYPE
            ' ' 'PPHDR-SUBTY' _P1014-SUBTY ' ', "SUBTYPE
            ' ' 'PPHDR-ISTAT' '1' ' ',          "PLANNING STATUS
            ' ' 'PPHDR-BEGDA' _P1014-BEGDA1 ' ', "BEGIN DT
            ' ' 'PPHDR-ENDDA' _P1014-ENDDA1 ' ', "END DT
            ' ' 'PM0D1-DPATT' _P1014-DPATT ' ' , "DATA SAMPLE
            ' ' 'BDC_OKCODE' '/05' ' ',          "CREATE-F5
            'X' 'MP101400' '2000' ' ',
            ' ' 'P1014-REDUN' _P1014-REDUN ' ', "INDICATOR
            ' ' 'BDC_OKCODE' '/11' ' ',            "SAVE
            'X' 'SAPMH5A0' '1000' ' '         ,
