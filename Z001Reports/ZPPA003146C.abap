...
* PA Infotype 0031 - using PA30
* Authors : Hemang/Mrudula - Annance Consulting
*---------------------------------------------------------------------*
REPORT ZPPA003146C MESSAGE-ID ZP.

* Internal table declaration for reading the source data

DATA: BEGIN OF _P0031 OCCURS 0,
       PERID(13),
       PERNR LIKE P0031-PERNR,
       RFPNR LIKE Q0031-RFPNR.
DATA: END OF _P0031.
...
PARAMETER : FILE0031 LIKE RLGRAP-FILENAME
                      DEFAULT 'C:\WINDOWS\SAP\0031.TXT' .

* Source Code

PERFORM INIT_BDC USING 'HRPA0031' SY-UNAME.
PERFORM UPLOAD_0031 USING FILE0031 'Error'.
...
LOOP AT ITAB.
  SPLIT ITAB-FILE1 AT DELIMITER INTO
       _P0031-PERID _P0031-RFPNR T.
MOVE _P0031-PERID TO _P0031-PERNR.
IF _P0031-PERID NE SPACE.
    APPEND _P0031.
ENDIF.
  ENDLOOP.
ENDFORM.
...
FORM POPULATE_BDC.
DATA CNT5 TYPE n.
DATA: TEMPER LIKE _P0031-PERID VALUE '00000000'.
CLEAR _P0031.
CNT5 = 1.
LOOP AT _P0031.
 IF TEMPER <> _P0031-PERNR.
   IF CNT5 <> 1.
     ...
     CNT5 = 1.
  ENDIF.
   CLEAR SSN. CONCATENATE '=c..' _P0031-PERID INTO SSN.
      ...
      PERFORM DYNPRO TABLES BDC_DATA USING:
                  'X' 'MP003100' '2000' ' ',
                  ' ' 'Q0031-RFPNR' _P0031-RFPNR CNT5." changed by
*                                         "Hem 1/18 - added CNT as parm
ELSE.

*
* PERFORM DYNPRO TABLES BDC_DATA USING: 'X' 'MP003100' '2000' ' ',
*Commented by SSD.

  PERFORM DYNPRO TABLES BDC_DATA USING:
                         ' ' 'Q0031-RFPNR' _P0031-RFPNR CNT5 .
ENDIF.
 CNT5 = CNT5 + 1.
TEMPER = _P0031-PERNR.
ENDLOOP.
