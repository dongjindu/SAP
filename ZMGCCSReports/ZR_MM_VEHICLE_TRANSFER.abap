*----------------------------------------------------------------------
* Program ID        : ZR_MM_VEHICLE_TRANSFER
* Title             : Transfer Test Vehicle Tracking record
* Created on        : 11/16/2007
* Created by        : Rakesh Gandhi
* Specifications By : Paul Shrewsbury
* Description       : This program transfers test vehicle tracking
*                     records
*----------------------------------------------------------------------
REPORT ZR_MM_VEHICLE_TRANSFER MESSAGE-ID ZMCO.
TYPE-POOLS : SLIS,
             ICON.
...
** Changed by Furong on 02/21/08 " UD1K942932
...
*-TTB_BUTTON. The structure is STB_BUTTON
    CLEAR LS_TOOLBAR.
    MOVE 'TRF_BATCH'      TO LS_TOOLBAR-FUNCTION.
    MOVE  ICON_TRANSFER   TO LS_TOOLBAR-ICON.
    MOVE 'TRANSFER BATCH' TO LS_TOOLBAR-QUICKINFO.
    MOVE 'Transfer Batch' TO LS_TOOLBAR-TEXT.
...
*----------------------------------------------------------------------*
INITIALIZATION.
  SY-TITLE = 'Test Vehicle Tracking - Transfer'.
...
*-Body Number
    IF P_BODY = 'X'.
      CLEAR: S_VIN[], S_VIN, S_FSC[], S_FSC, S_SHOPDT[], S_SHOPDT,
             ...
             S_UDEPT, S_GLNO[], S_GLNO,
             S_ENTTYP[], S_FTZ[], S_DPAID[],
...
FORM ALV_FIELDCAT.
  ...
  APPEND_FIELDCAT :
     ...
     ' '  'USAGE_DEPT'     'Asking Dept.(To)'  16 'CHAR' 'X' ' ',
     ' '  'USAGE_TEXT1'    'Usage Text(To)'    30 'CHAR' 'X' ' ',
** Changed by Furong on 02/20/08 " UD1K942932
...
** End of change
  ...
  LOOP AT GT_FIELDCAT.
    IF GT_FIELDCAT-FIELDNAME = 'USAGE_DEPT'.
...
FORM UPDATE_ORIGINAL_BATCH.
  ...
  READ TABLE IT_CABN WITH KEY ATNAM = 'P_ASK_DEPT'.  " Asking Dept(From)
  PERFORM UPDATE_TABLE USING IT_CABN-ATFOR IT_CABN-ATNAM
                                                  IT_CHARG1-USAGE_DEPT.

  READ TABLE IT_CABN WITH KEY ATNAM = 'P_USAGE_DEPT'.  " Asking Dept(To)
  PERFORM UPDATE_TABLE USING IT_CABN-ATFOR IT_CABN-ATNAM
                                                  IT_CHARG1-USAGE_DEPT.

  READ TABLE IT_CABN WITH KEY ATNAM = 'P_USAGE_TEXT'.  " Usage text
  PERFORM UPDATE_TABLE USING IT_CABN-ATFOR IT_CABN-ATNAM
                                                  IT_CHARG1-USAGE_TEXT1.
...
FORM FILL_INTERN_TABLE.
  DATA: L_DATE    LIKE SY-DATUM,
        ...
        L_USAGE_DEPT TYPE ZE_USAGE_DEPT.
...
*-Validations for Shop Date
  ...
  IF NOT L_RP18_SHOP_DATE IS INITIAL.
    PERFORM USER_SPECIFIC_DATE USING  L_RP18_SHOP_DATE
                                      CHANGING L_DATE.
  ENDIF.
...
*-Validations for Ship out Date
  ...
  IF NOT L_SHIPOUT_DATE IS INITIAL.
    PERFORM USER_SPECIFIC_DATE USING  L_SHIPOUT_DATE
                               CHANGING L_DATE.
  ENDIF.
...
*-Validations for Destination/Using Department
  ...
  ELSEIF NOT ( S_UDEPT-LOW IS INITIAL ) AND S_UDEPT-HIGH IS INITIAL.
    READ TABLE IT_USAGEDEP WITH KEY USAGE_DEPT = L_USAGE_DEPT
                                                 BINARY SEARCH.
    IF SY-SUBRC <> 0.
      EXIT.
    ENDIF.
  ELSEIF S_UDEPT-LOW IS INITIAL AND NOT ( S_UDEPT-HIGH IS INITIAL ).
    IF L_USAGE_DEPT > S_UDEPT-HIGH.
      EXIT.
    ENDIF.
  ELSEIF NOT ( S_UDEPT-LOW IS INITIAL AND S_UDEPT-HIGH IS INITIAL ).
    IF ( L_USAGE_DEPT < S_UDEPT-LOW OR L_USAGE_DEPT > S_UDEPT-HIGH ).
      EXIT.
    ENDIF.
  ENDIF.
...
FORM UPDATE_ORIGINAL_BATCH1.
  ...
  IF SY-SUBRC = 0.
    ...
    IF NOT L_DKTXT IS INITIAL.
      L_DKTXT1 = L_DKTXT.
      PERFORM UPDATE_TABLE USING IT_CABN-ATFOR IT_CABN-ATNAM L_DKTXT1.
    ENDIF.      " IF NOT l_dktxt IS INITIAL
  ENDIF.
