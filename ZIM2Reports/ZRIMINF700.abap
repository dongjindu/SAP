...
*&
*&---------------------------------------------------------------------*
REPORT  ZRIMINF700   MESSAGE-ID ZIM
                     LINE-SIZE 116
...
TABLES : ZTREQHD,ZTREQIT,LFA1,ZTREQST,ZTMLCHD,ZTMLCSG2, ZTMLCSG910,
         ZTMLCSG7G,ZTMLCSG9O,DD07T,ZTREQIL.
...
FORM P3000_DATA_WRITE USING      W_ERR_CHK.
  ...
  SET PF-STATUS 'INF700'.           " GUI STATUS SETTING
  SET  TITLEBAR 'INF700'.           " GUI TITLE SETTING..
...
DATA: L_TEXT(5), L_TEXT1(5), L_TEXT2(12).
  ...
  SKIP 2.
  WRITE:/31  '(Irrevocable Documentary Credit Application)'.
  ...
  WRITE:/ SY-ULINE.
  SKIP 2.
  WRITE:/8 'Document/EDI No :',40 ZTREQST-ZFDOCNOR.
  SKIP 2.
...
*--------------------------------------------------------------------
  ...
  WRITE:/8 'Opening application date :',ZTREQST-ZFRVDT,
        ...
        /8 'Credit providing subject :',W_DOM_TEX2.
*>> 수입추천번호.
  W_COUNT = 0.
  LOOP AT IT_ZTREQIL.
...
*--------------------------------------------------------------------
  ...
  WRITE:/8 '40A','Form of Documentary Credit:',W_DOM_TEX1,
        /8 '20 ','Documentry Credit Number  :',ZTREQST-ZFOPNNO,
        /8 '31C','Date of Issue             :',ZTREQST-ZFOPNDT,
        /8 '31D','Date and Place of Expiry  :','(date)',
                                               ZTMLCHD-ZFEXDT,
        /8 '                               ','(place)',
        /40                                    ZTMLCHD-ZFEXPL,
        /8 '51A','Applicant Bank            :',ZTMLCHD-ZFOBNM,
        /8 '51B','Advising Bank             :',ZTMLCHD-ZFABNM,
...
*       L_TEXT1 = ZTMLCHD-ZFALCP. CONDENSE L_TEXT1 NO-GAPS.
       ...
       WRITE: /8 '39A','Percentage Credit Amount Tolerance:',
                   L_TEXT2.
    WHEN 'X'.
       WRITE: /8 '39B','Maximum Credit Amount     :'.
    WHEN '2AA'.
       WRITE: /8 '39C','Up to Credit Amount      :'.
    WHEN '2AB'.
       WRITE:/8  '39C','Not Exceeding Credit Amount :'.
    WHEN OTHERS.
  ENDCASE.
...
*  ENDCASE.
  CASE ZTMLCHD-ZFTRMB.
       WHEN '2AO' OR SPACE.
          WRITE:/8 '42C','Drafts at ...             :',ZTMLCHD-ZFTRTX1.
       WHEN '2AM'.
          WRITE:/7 '*42M','Mixed Payment Details     :',ZTMLCHD-ZFTRTX1.

       WHEN '2AN'.
          WRITE:/8 '*42P','Deferred Payment Details  :',ZTMLCHD-ZFTRTX1.
       WHEN OTHERS.
  ENDCASE.
...
*--------------------------------------------------------------------
  ...
  WRITE:/8 '43P','Partial Shipments         :',W_DOM_TEX1.
...
*     WHERE ZFREQNO = P_REQNO.
   ...
   IF NOT ( ZTREQST-ZFDOCST EQ 'O' OR ZTREQST-ZFDOCST EQ 'A' ).
      MESSAGE S104 WITH P_REQNO W_MAX_ZFAMDNO ZTREQST-ZFDOCST.
      LEAVE PROGRAM.
   ENDIF.
   ...
   REFRESH: IT_ZTMLCSG7G,IT_ZTMLCSG8E,IT_ZTREQIL.
