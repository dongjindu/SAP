...
*&
*&---------------------------------------------------------------------*
REPORT  ZRIMINF707   MESSAGE-ID ZIM
                     LINE-SIZE 116
...
FORM P3000_DATA_WRITE USING      W_ERR_CHK.
  ...
  SET PF-STATUS 'INF707'.           " GUI STATUS SETTING
  SET  TITLEBAR 'INF707'.           " GUI TITLE SETTING..
...
DATA: L_TEXT(5), L_TEXT1(5), L_TEXT2(12).
  ...
  WRITE:/ SY-ULINE.
  SKIP 2.
  WRITE:/8 'Document/EDI No :',ZTREQST-ZFDOCNOR.
  SKIP 2.
...
*        /8 '개설(의뢰)은행    :',ZTMLCHD-ZFOBNM,
*        /8         '           ',ZTMLCHD-ZFOBBR,
*        /8 '                   ','[Tel No.]',ZTMLCHD-ZFOBPH,
*        /8 '(희망)통지은행    :',ZTMLCHD-ZFABNM,ZTMLCHD-ZFABBR,
        /8 'Opening applicant    :',ZTMLCSG2-ZFAPPNM, ZTMLCSG2-ZFAPPAD1,
...
*--------------------------------------------------------------------
  ...
  WRITE:/8 '20 ','Sender''s Reference(L/C No):',ZTREQST-ZFOPNNO,
        ...
        /8 '52A','Issunig Bank              :',ZTMLCHD-ZFOBNM,
        /40                                    ZTMLCHD-ZFOBBR,
        /8 '30 ','Date of Amendment         :',ZTREQST-ZFOPNDT,
  ...
  ENDIF.
  IF ZTMLCAMHD-ZFIDCD EQ '-'.
     WRITE:/7 '*33B','Decrease of Documentary Credit Amount       :',
            ZTMLCAMHD-WAERS RIGHT-JUSTIFIED,
            ZTMLCAMHD-ZFIDAM CURRENCY ZTMLCAMHD-WAERS LEFT-JUSTIFIED.
  ENDIF.
...
*>>수입의뢰 Status
   ...
   IF NOT ( ZTREQST-ZFDOCST EQ 'O' OR  ZTREQST-ZFDOCST  EQ 'A' ).
      MESSAGE S104 WITH P_REQNO P_AMDNO ZTREQST-ZFDOCST.
      LEAVE PROGRAM.
   ENDIF.
   SELECT SINGLE *
      FROM ZTMLCHD
