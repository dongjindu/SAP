...
DATA : BEGIN OF IT_TAB OCCURS 0,
       ...
       ZFBLNO          LIKE   ZTCUCLCST-ZFBLNO,   " B/L 관리번호.
       ZFCLSEQ         LIKE   ZTCUCLCST-ZFCLSEQ,  " 통관순번.
       ZFCSQ           LIKE   ZTCUCLCST-ZFCSQ,    " 비용순번.
       ZFCAMT          LIKE   ZTCUCLCST-ZFCAMT,   " 비용금액.
       ZFCAMT1         LIKE   ZTCUCLCST-ZFCAMT,   " 관세.
       ZFCAMT2         LIKE   ZTCUCLCST-ZFCAMT,   " 부가세.
       ZFKRW           LIKE   ZTCUCLCST-ZFKRW,    " 원화통화.
       ZFCSCD          LIKE   ZTCUCLCST-ZFCSCD,   " 비용코드.
       ZFCSCD_NM(6)    TYPE   C,                  " 비용코드.
       ZFOCDT          LIKE   ZTCUCLCST-ZFOCDT,   " 발생일.
       ZFVEN           LIKE   ZTCUCLCST-ZFVEN,    " Vendor
       ZFVEN_NM(20)    TYPE   C,                  " Vendor.
       ZFPAY           LIKE   ZTCUCLCST-ZFPAY,    " 지불처.
...
* 수출환급용여?
  SELECTION-SCREEN : BEGIN OF LINE, COMMENT 1(15) TEXT-003, POSITION 1.
     SELECTION-SCREEN : COMMENT 32(3) TEXT-031, POSITION 36.
...
*&---------------------------------------------------------------------*
FORM P2000_INIT.
  SET  TITLEBAR 'ZIMV3'.           " GUI TITLE SETTING..
...
FORM P3000_TITLE_WRITE.
  ...
  WRITE : /35  '[ 관세/부가세 POSTING - 유환 ]'
               COLOR COL_HEADING INTENSIFIED OFF.
  WRITE : / 'Date : ', SY-DATUM, 78 'Page : ', W_PAGE.
  WRITE : / SY-ULINE.
...
FORM P3000_DATA_WRITE USING     W_ERR_CHK.
  ...
  CLEAR : SV_ZFVEN, SV_ZFPAY, SV_ZTERM, SV_MWSKZ, SV_ZFWERKS,
          SV_ZFBLNO, SV_ZFCLSEQ, SV_BUKRS,
          SUM_ZFCAMT, SUM_ZFCAMT1, SUM_ZFCAMT2, W_LOOP_CNT.

   MOVE 'N' TO W_ERR_CHK.

   SET PF-STATUS 'ZIMV3'.           " GUI STATUS SETTING
   SET  TITLEBAR 'ZIMV3'.           " GUI TITLE SETTING..
...
*>> 초기 비교값 MOVE.
      IF W_LINE  =  1.
         ...
         MOVE   IT_TAB-ZFOCDT   TO  SV_ZFOCDT.
      ENDIF.
*>> 비교값이 틀리면 SUM WRITE.
      ...
      IF  IT_TAB-ZFCSCD  =  '001'.
          ADD  IT_TAB-ZFCAMT   TO  SUM_ZFCAMT1.
      ELSE.
          ADD  IT_TAB-ZFCAMT   TO  SUM_ZFCAMT2.
      ENDIF.
...
FORM P3000_LINE_WRITE.
  ...
  WRITE:/ SY-VLINE, '  '       NO-GAP,
       ...
       SY-VLINE                NO-GAP,
       IT_TAB-ZFCAMT CURRENCY 'KRW' NO-GAP, " 비용금?
...
FORM P1000_READ_TEXT USING    W_ERR_CHK.
  ...
  SELECT  H~ZFHBLNO  I~ZFBLNO  I~ZFCLSEQ  I~ZFCSQ  I~ZFOCDT
          ...
          I~ZFVEN    I~ZFPAY   I~BUKRS    I~WAERS
...
* 유환건만 Check!
    ...
    ELSE.
        IF P_Y = 'X'.
           IF  ZTIDS-ZFITKD NE 'A'.
               DELETE IT_TAB INDEX W_TABIX.
               CONTINUE.
           ENDIF.
        ELSE.
           IF  ZTIDS-ZFITKD EQ 'A'.
               DELETE IT_TAB INDEX W_TABIX.
               CONTINUE.
           ENDIF.
        ENDIF.
...
*지불처, 거래처명 DISPLAY.
  ...
  SORT IT_TAB BY BUKRS ZFVEN ZFPAY ZTERM MWSKZ ZFWERKS ZFBLNO ZFCLSEQ.
  ...
  DO.
    ...
    IF ( MARKFIELD EQ 'x' ) OR ( MARKFIELD EQ 'X' ).
         MOVE : SV_BUKRS       TO IT_SELECTED-BUKRS,
                ...
                SV_ZFOCDT      TO IT_SELECTED-ZFOCDT,
                SUM_ZFCAMT1    TO IT_SELECTED-ZFCAMT1,
                SUM_ZFCAMT2    TO IT_SELECTED-ZFCAMT2,
...
* Hide
       HIDE: SV_ZFVEN,   SV_ZFPAY,    SV_ZTERM,    SV_MWSKZ, SV_ZFWERKS,
             SUM_ZFCAMT, SUM_ZFCAMT1, SUM_ZFCAMT2, SV_ZFOCDT, SV_BUKRS.
...
* 관?
       ...
       '관세  '         NO-GAP,
       SY-VLINE         NO-GAP,
       SUM_ZFCAMT1      CURRENCY 'KRW'   NO-GAP, " 금?
       '     '          NO-GAP,
       SY-VLINE         NO-GAP,
       '                             ', SY-VLINE NO-GAP.
* 부가?
       ...
       '부가세'         NO-GAP,
       SY-VLINE         NO-GAP,
       SUM_ZFCAMT2      CURRENCY 'KRW' NO-GAP, " 금?
       '     '          NO-GAP,
       SY-VLINE         NO-GAP,
       '                             ', SY-VLINE NO-GAP.
* Term, Tax, Plant
...
*      ' ' 'BSEG-EMPFB'  L_ZFPAY,               " Payee
      ' ' 'BSEG-ZLSPR'  'B',                   " 지급보류.
      ' ' 'BSEG-SGTXT'  '관세/부가세(유환)'.
*>> 관세가 존재하면...
  IF IT_SELECTED-ZFCAMT1 GT 0.
     IF P_Y = 'X'.
        L_ZFIOCAC1  =  ZTIMIMG11-ZFIOCAC7.
     ELSE.
        L_ZFIOCAC1  =  ZTIMIMG11-ZFIOCAC8.
     ENDIF.
     W_WRBTR = IT_SELECTED-ZFCAMT1.
     WRITE W_WRBTR CURRENCY 'KRW' TO TEMP_WRBTR.
     PERFORM P2000_DYNPRO USING :
         ...
         ' ' 'BSEG-BUPLA'  ZVT001W-J_1BBRANCH,    " Business Place
         ' ' 'BSEG-SGTXT'  '관세/부가세(유환)'.   " 텍스트.
  ENDIF.
*>> 부가세가 존재하면...
  IF IT_SELECTED-ZFCAMT2 GT 0.
     L_ZFIOCAC1  =  ZTIMIMG11-ZFIOCAC6.
     W_WRBTR = IT_SELECTED-ZFCAMT2.
     WRITE W_WRBTR CURRENCY 'KRW' TO TEMP_WRBTR.
     PERFORM P2000_DYNPRO USING :
         ' ' 'RF05A-NEWBS' '40',                  " Posting Key
         ' ' 'RF05A-NEWKO' L_ZFIOCAC1,            " ACCOUNT
