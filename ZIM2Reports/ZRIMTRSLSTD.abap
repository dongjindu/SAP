...
*& [Change Request
*&---------------------------------------------------------------------*
REPORT  ZRIMTRLSTD   MESSAGE-ID ZIM
                     LINE-SIZE 92
...
FORM P1000_GET_IT_TAB USING    W_ERR_CHK.
  ...
  FROM   ZTBL     AS  A   INNER  JOIN  ZTIV    AS  B
  ...
  LOOP AT IT_TAB_DETAIL.
    ...
    WHERE  ZFBLNO   EQ   IT_TAB_DETAIL-ZFBLNO.
    IF SY-SUBRC EQ 0.
      MOVE  ZTIDSUS-ZFEDT   TO  IT_TAB_DETAIL-ZFEDT.
    ELSE.
      DELETE  IT_TAB_DETAIL  INDEX  W_TABIX.
      CONTINUE.
    ENDIF.
...
FORM P1000_APPEND_IT_FIELDCAT.
  ...
  PERFORM FIELDCAT USING : 'ZFEDT'       'C/Clear'        '10'.
