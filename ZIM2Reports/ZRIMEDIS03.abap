...
*&     1. SELECT시 로직 변경...
*&---------------------------------------------------------------------*
REPORT  ZRIMEDIS03    MESSAGE-ID ZIM
                      LINE-SIZE 132
...
FORM P3000_LINE_WRITE.
  ...
  WRITE:/ SY-VLINE, MARKFIELD  AS CHECKBOX,SY-VLINE NO-GAP,
          ...
          (02)IT_TAB-ZFEDIST NO-GAP,(02)'/' NO-GAP,    " Download 상태.
...
*>> EDI FLAT FILE CREATE FUNCTION CALL(HEADER)
   CALL  FUNCTION 'ZIM_KHNP_CUSDEC_EDI_DOC'
         EXPORTING
...
*>> EDI FLAT FILE CREATE FUNCTION CALL(란사항)
   CALL  FUNCTION 'ZIM_KHNP_CUSDEC_HS_EDI_DOC'
         EXPORTING
            W_ZFBLNO      = ZTIDR-ZFBLNO
            W_ZFCLSEQ     = ZTIDR-ZFCLSEQ
            W_ZFDHENO     = W_ZFDHENO
...
*>> EDI FLAT FILE CREATE FUNCTION CALL(행사항)
   CALL  FUNCTION 'ZIM_KHNP_CUSDEC_HSD_EDI_DOC'
         EXPORTING
            W_ZFBLNO      = ZTIDR-ZFBLNO
            W_ZFCLSEQ     = ZTIDR-ZFCLSEQ
            W_ZFDHENO     = W_ZFDHENO
...
FORM P3000_FILE_DOWNLOAD .
  ...
  W_FILENAME  =  'C:\한전Data\IN\importh.TXT'.
  ...
  IF SY-SUBRC NE 0.
     MESSAGE E461(ZIM1) WITH W_FILENAME.
  ENDIF.

  W_FILENAME  =  'C:\한전Data\IN\importd.TXT'.
  CALL FUNCTION 'WS_DOWNLOAD'
       EXPORTING
            FILENAME = W_FILENAME
            FILETYPE = 'ASC'
  ...
  W_FILENAME  =  'C:\한전Data\IN\importm.TXT'.
  CALL FUNCTION 'WS_DOWNLOAD'
       EXPORTING
            FILENAME = W_FILENAME
            FILETYPE = 'ASC'
            MODE     = 'A'
       TABLES
            DATA_TAB = IT_EDIFILE_ITEM2.

ENDFORM.                    " P3000_FILE_DOWNLOAD
