...
*&
*&-----------------------------------------------------------------*
REPORT ZRIMDOANTC    MESSAGE-ID ZIM
                     LINE-SIZE 116
...
*                     CALL TRANSACTION 'ZIM03' AND SKIP  FIRST
*SCREEN.
*                  ELSE.
*                     CALL TRANSACTION 'ZIM13' AND SKIP  FIRST
*SCREEN.
*                  ENDIF.*
*
*               WHEN 'IMPREQ'.
*                  SET PARAMETER ID 'ZPHBLNO'    FIELD ''.  "HOUSE
*B/L.
...
*>>어음금액정렬
  ...
  CONCATENATE  ZTPMTHD-ZFPNAMC W_PNAM INTO W_PNAM
               SEPARATED BY SPACE.
  WRITE: W_PNAM TO W_PNAM LEFT-JUSTIFIED.
*>>기타수수료정렬
...
*                                               W_ADBK-NAME2,
    ...
    IF NOT ZTPMTHD-ZFHBLNO IS INITIAL.
        WRITE:/ '선하증권(항공화물운송장)번호',
                                            50 ZTPMTHD-ZFHBLNO.
    ENDIF.

    WRITE:/ '어음금액',                 50 W_PNAM.

    IF NOT ZTPMTHD-ZFBKCH IS INITIAL.
