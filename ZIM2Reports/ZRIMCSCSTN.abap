...
*& Report  ZRIMCSCSTN                                                  *
*&---------------------------------------------------------------------*
*&  프로그램명 : 통관비용 Posting -  무?
*&      작성자 : 김연중 INFOLINK Ltd.                                  *
*&      작성일 : 2000.09.08                                            *
*&  적용회사PJT:                                                       *
*&---------------------------------------------------------------------*
*&   DESC.     : 무환 통관비용을 조회하여 BDC로 Posting한다.
*&
*&---------------------------------------------------------------------*
*& [변경내용]
*&
*&---------------------------------------------------------------------*
REPORT  ZRIMCSCSTN   MESSAGE-ID ZIM
                     LINE-SIZE 120
...
DATA : BEGIN OF IT_TAB OCCURS 0,
       ...
       ZFCSCD_NM(15)   TYPE   C,                  " 비용명?
       ZFCAMT          LIKE   ZTCUCLCST-ZFCAMT,   " 비용금?
       WAERS           LIKE   ZTCUCLCST-WAERS,    " 통화.
...
FORM P3000_TITLE_WRITE.
  ...
  WRITE : /44 '[ Clearance expense POSTING - Non-monetary transaction ]'
               COLOR COL_HEADING INTENSIFIED OFF.
  WRITE : / 'Date : ', SY-DATUM, 102 'Page : ', W_PAGE.
  WRITE : / SY-ULINE.
  FORMAT COLOR COL_HEADING INTENSIFIED ON.
  WRITE : / SY-VLINE, ' ',  SY-VLINE NO-GAP,
            'House B/L No    '               NO-GAP, SY-VLINE NO-GAP,
            'Occuring date    '              NO-GAP, SY-VLINE NO-GAP,
            'Expense            '            NO-GAP, SY-VLINE NO-GAP,
  ...
  WRITE : / SY-VLINE, ' ',                           SY-VLINE NO-GAP,
            ...
            'Clearance sequence  '           NO-GAP, SY-VLINE NO-GAP,
            'Company/Term/Tax/Plant'            NO-GAP, SY-VLINE NO-GAP,
...
FORM P3000_DATA_WRITE USING     W_ERR_CHK.
   ...
   SET PF-STATUS 'ZIMV6'.           " GUI STATUS SETTING
   SET  TITLEBAR 'ZIMV6'.           " GUI TITLE SETTING..
...
FORM P1000_READ_TEXT USING    W_ERR_CHK.
  ...
  CLEAR : SV_BUKRS,   SV_ZFVEN,  SV_ZFPAY,  SV_ZTERM, SV_MWSKZ,
          SV_ZFWERKS, SV_WAERS,  W_GRP_MARK.
  ...
  SELECT  I~BUKRS    H~ZFHBLNO I~ZFBLNO   I~ZFCLSEQ  I~ZFCSQ  I~ZFOCDT
          I~ZFCSCD   I~ZFCAMT  I~ZTERM    I~MWSKZ    I~ZFWERKS
          I~ZFVEN    I~ZFPAY   H~KOSTL    I~ZFKRW    I~ZFVAT
  ...
  SORT IT_TAB BY BUKRS  ZFVEN    ZFPAY ZTERM  MWSKZ   ZFWERKS.
...
FORM P3000_SUM_LINE_WRITE.
  ...
  WRITE:/ SY-VLINE, MARKFIELD  AS CHECKBOX,
       ...
       IT_TAB-ZFOCDT    NO-GAP,   SY-VLINE NO-GAP,      " 발생?
       IT_TAB-ZTERM     NO-GAP, " Terms Of Payment
       '/'              NO-GAP,
       IT_TAB-MWSKZ     NO-GAP, " Tax Code
       ' /'             NO-GAP,
       IT_TAB-ZFWERKS   NO-GAP, " Plant
       ' '              NO-GAP, SY-VLINE NO-GAP,
       IT_TAB-ZFCAMT CURRENCY IT_TAB-ZFKRW NO-GAP, " 금?
       '     '          NO-GAP,  SY-VLINE   NO-GAP,
       IT_TAB-ZFVEN     NO-GAP,                      " Vendor
       IT_TAB-ZFVEN_NM  NO-GAP,                      " Vendor ?
       SY-VLINE         NO-GAP.
* Hide
       ...
       '          '               NO-GAP, SY-VLINE  NO-GAP,
       '              '              NO-GAP, SY-VLINE NO-GAP,
       IT_TAB-ZFVAT CURRENCY IT_TAB-ZFKRW NO-GAP, " VAT
       '     '                    NO-GAP,  SY-VLINE   NO-GAP,
       IT_TAB-ZFPAY               NO-GAP, " 지불?
       IT_TAB-ZFPAY_NM            NO-GAP, " 지불처 ?
       SY-VLINE                   NO-GAP.
...
*>> MESSAGE BOX.
  ...
  LOOP AT IT_SELECTED.
    ...
    IF IT_SELECTED-GRP_MARK  NE  GRP_MARK_T.
       ...
       MOVE  SUM_ZFVAT             TO  IT_SELECTED_SUM-ZFVAT.
...
DATA :   L_ZFPAY     LIKE   IT_SELECTED-ZFPAY,
  ...
  W_WRBTR = IT_SELECTED_SUM-ZFCAMT + IT_SELECTED_SUM-ZFVAT.
  W_WMWST = IT_SELECTED_SUM-ZFVAT.
  WRITE W_WRBTR CURRENCY 'KRW' TO TEMP_WRBTR.
  WRITE W_WMWST CURRENCY 'KRW' TO TEMP_WMWST.
*--------------------------------------------------------------------
...
*&---------------------------------------------------------------------*
*&      Form  P4000_COST_CENTER
*&---------------------------------------------------------------------*
FORM P4000_COST_CENTER.

     CLEAR : TEMP_KOSTL, TEMP_AUFNR, TEMP_ZZWORK.
     MOVE 'B2A164' TO TEMP_AUFNR.
     IF ( IT_TAB-KOSTL = 'LV994') OR
        ( IT_TAB-KOSTL = 'LW991') OR ( IT_TAB-KOSTL = 'LW992') OR
        ( IT_TAB-KOSTL = 'LW993') OR
        ( IT_TAB-KOSTL = 'LP991') OR ( IT_TAB-KOSTL = 'LP992') OR
        ( IT_TAB-KOSTL = 'LP994') OR
        ( IT_TAB-KOSTL = 'LI991') OR ( IT_TAB-KOSTL = 'LI992') OR
        ( IT_TAB-KOSTL = 'LI993') OR ( IT_TAB-KOSTL = 'LI993').
        MOVE IT_TAB-KOSTL TO TEMP_KOSTL.
        EXIT.
     ENDIF.
     IF ( IT_TAB-KOSTL = 'LA991').
        MOVE IT_TAB-KOSTL TO TEMP_KOSTL.
        MOVE 'IP99999' TO TEMP_ZZWORK.
        EXIT.
     ENDIF.
     MOVE 'LA991' TO TEMP_KOSTL.
     MOVE 'IP99999' TO TEMP_ZZWORK.
     IF IT_SELECTED-ZFWERKS = '1010'.
        MOVE 'LL991' TO TEMP_KOSTL.
        MOVE 'IP08300' TO TEMP_ZZWORK.
     ENDIF.
     IF IT_SELECTED-ZFWERKS = '1020'.
        MOVE 'LM992' TO TEMP_KOSTL.
        MOVE 'CP48153' TO TEMP_ZZWORK.
        ENDIF.
     IF IT_SELECTED-ZFWERKS = '1030'.
        MOVE 'LN993' TO TEMP_KOSTL.
        MOVE 'GP40200' TO TEMP_ZZWORK.
     ENDIF.

ENDFORM.                    " P4000_COST_CENTER
*&---------------------------------------------------------------------*
