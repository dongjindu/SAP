...
*& Report  ZRIMSTCCST                                                  *
*&---------------------------------------------------------------------*
*&  Program Name : Inventory Amount by Import Progress steps           *
...
*&
*&---------------------------------------------------------------------*
REPORT  ZRIMSTCCST   MESSAGE-ID ZIM
                     LINE-SIZE 255
...
SELECTION-SCREEN: COMMENT  34(20)  TEXT-012, POSITION 56.
PARAMETERS: R_MATNR  RADIOBUTTON GROUP B2.
SELECTION-SCREEN: COMMENT  60(20)  TEXT-013.
...
*<<Arrival~ Bonded Warehouse.>>=================>
  PERFORM P1000_READ_TR_DATA.

*<<Bonde Warehouse ~ Customs Clearance.>>=======>
...
*> Main List Internal Table.
    ...
    ENDIF.
    APPEND IT_LCTAB1.
    PERFORM P2000_MOVE_TAB USING IT_PO-WAERS
                                 IT_PO-WERKS
                                 IT_PO-EKGRP
                                 IT_PO-MATKL
                                 IT_PO-MATNR
...
FORM P3000_TITLE_WRITE.
  ...
  WRITE : /70 ' [ Inventory Amount by Import Progress steps ]'
  ...
  WRITE : / SY-VLINE NO-GAP,
          (05) 'Currency'          CENTERED NO-GAP, SY-VLINE NO-GAP.


  WRITE AT 8(TITLE_LINE) W_GR_TITLE  CENTERED NO-GAP.
  ...
  WRITE : (20) 'Ordering'              CENTERED NO-GAP, SY-VLINE NO-GAP,
          ...
          (20) 'Arrival inventory(Bonded transportation)'
                                       CENTERED NO-GAP, SY-VLINE NO-GAP,
          (20) 'Bonded warehouse inventory(Not yet clearance)'
                                       CENTERED NO-GAP, SY-VLINE NO-GAP,
...
FORM P2000_GROUP_SETTING.
  CASE 'X'.
    ...
    WHEN R_MATNR.
      W_GROUP = 'MATNR'. CODE_LINE = 18. TEXT_LINE = 30.
      W_GR_TITLE = 'Materials'.
    WHEN R_MATKL.
...
*       text
*----------------------------------------------------------------------*
FORM P2000_MOVE_TAB USING    P_WAERS      " Currency
                             P_WERKS      " Plant
                             P_EKGRP      " Purchasing Group
                             P_MATKL      " Material Text
                             P_MATNR      " Material
                             P_LIFNR      " Vendor
                             P_ZFAMT      " Amount
                             P_STEP.      " Step
  CLEAR : IT_TAB, IT_TOT.
  IT_TAB-WAERS = P_WAERS.
  CASE 'X'.
    WHEN R_WERKS.
  ...
  CASE P_STEP.
    WHEN 'LC'.
      IT_TAB-LC_ZFAMT = P_ZFAMT.
    WHEN 'OP'.
      IT_TAB-BL_ZFAMT = P_ZFAMT.
    WHEN 'BL'.
      IT_TAB-IB_ZFAMT = P_ZFAMT.
    WHEN 'TR'.
      IT_TAB-TR_ZFAMT = P_ZFAMT.
    WHEN 'ST'.
      IT_TAB-ST_ZFAMT = P_ZFAMT.
    WHEN 'GR'.
      IT_TAB-GR_ZFAMT = P_ZFAMT.
  ENDCASE.

  MOVE-CORRESPONDING IT_TAB TO IT_TOT.
  COLLECT : IT_TAB, IT_TOT.
  CLEAR   : IT_TAB, IT_TOT.
...
FORM P3000_SUM_WRITE USING    P_WAERS.
  ...
  WRITE AT 8(TITLE_LINE) 'Total sum' CENTERED NO-GAP.
...
FORM P3000_LINE_WRITE.
  ...
  WRITE AT 8(CODE_LINE) <FS> NO-GAP.
  WRITE : (01) '' NO-GAP.
  WRITE AT WL_NLINE(TEXT_LINE) WL_FNAME NO-GAP.

  WRITE : SY-VLINE NO-GAP,
         (20) IT_TAB-LC_ZFAMT CURRENCY IT_TAB-WAERS
                    RIGHT-JUSTIFIED  NO-GAP, SY-VLINE NO-GAP,
         (20) IT_TAB-BL_ZFAMT CURRENCY IT_TAB-WAERS
                    RIGHT-JUSTIFIED  NO-GAP, SY-VLINE NO-GAP,
         (20) IT_TAB-IB_ZFAMT CURRENCY IT_TAB-WAERS
                    RIGHT-JUSTIFIED  NO-GAP, SY-VLINE NO-GAP,
         (20) IT_TAB-TR_ZFAMT CURRENCY IT_TAB-WAERS
                    RIGHT-JUSTIFIED  NO-GAP, SY-VLINE NO-GAP,
         (20) IT_TAB-ST_ZFAMT CURRENCY IT_TAB-WAERS
                    RIGHT-JUSTIFIED  NO-GAP, SY-VLINE NO-GAP,
         (20) IT_TAB-GR_ZFAMT CURRENCY IT_TAB-WAERS
                    RIGHT-JUSTIFIED  NO-GAP, SY-VLINE NO-GAP.
  HIDE IT_TAB.
  NEW-LINE.
  WRITE AT 1(MAX_LINE) SY-ULINE.
...
FORM P2000_GET_GR_NAME CHANGING P_FNAME.
  ...
  CASE 'X'.
    ...
    WHEN R_MATNR.
      SELECT SINGLE MAKTX INTO P_FNAME
             FROM MAKT
...
MODULE D0100_LIST_CHECK_SCR0100 INPUT.
  ...
  CASE W_SELECT.
    WHEN 'LC'.
      ...
      DETAIL_LINE = 175.
      SORT IT_LCTAB2 BY WAERS (W_GROUP) EBELN EBELP.
      CLEAR : W_FIELD.
      CONCATENATE 'IT_LCTAB2-' W_GROUP INTO W_FIELD.
      ASSIGN (W_FIELD) TO <FS2>.
      LOOP AT IT_LCTAB2 WHERE  WAERS EQ IT_TAB-WAERS.
        IF <FS> EQ <FS2>.
    ...
    WHEN 'BL'.                         " L/C Open.
      ...
      DETAIL_LINE = 222.
      SORT IT_OPTAB2 BY WAERS (W_GROUP) EBELN EBELP.
      CLEAR : W_FIELD.
      CONCATENATE 'IT_OPTAB2-' W_GROUP INTO W_FIELD.
      ASSIGN (W_FIELD) TO <FS2>.
      LOOP AT IT_OPTAB2 WHERE  WAERS EQ IT_TAB-WAERS.
        IF <FS> EQ <FS2>.
    ...
    WHEN 'IB'.
      ...
      DETAIL_LINE = 214.
      SORT IT_BLTAB2 BY WAERS (W_GROUP) EBELN EBELP.
      CLEAR : W_FIELD.
      CONCATENATE 'IT_BLTAB2-' W_GROUP INTO W_FIELD.
      ASSIGN (W_FIELD) TO <FS2>.
      LOOP AT IT_BLTAB2 WHERE  WAERS EQ IT_TAB-WAERS.
        IF <FS> EQ <FS2>.
    ...
    WHEN 'TR'.
      ...
      DETAIL_LINE = 183 .
      SORT IT_TRTAB2 BY WAERS (W_GROUP) EBELN EBELP.
      CLEAR : W_FIELD.
      CONCATENATE 'IT_TRTAB2-' W_GROUP INTO W_FIELD.
      ASSIGN (W_FIELD) TO <FS2>.
      LOOP AT IT_TRTAB2 WHERE  WAERS EQ IT_TAB-WAERS.
        IF <FS> EQ <FS2>.
    ...
    WHEN 'ST'.
      ...
      DETAIL_LINE = 193 .
      SORT IT_STTAB2 BY WAERS (W_GROUP) EBELN EBELP.
      CLEAR : W_FIELD.
      CONCATENATE 'IT_STTAB2-' W_GROUP INTO W_FIELD.
      ASSIGN (W_FIELD) TO <FS2>.
      LOOP AT IT_STTAB2 WHERE  WAERS EQ IT_TAB-WAERS.
        IF <FS> EQ <FS2>.
    ...
    WHEN 'GR'.
      W_DT_TITLE = ' Not yet G/R inventory after clearance '.
      DETAIL_LINE = 165.
      SORT IT_GRTAB2 BY WAERS (W_GROUP) EBELN EBELP.
      CLEAR : W_FIELD.
      CONCATENATE 'IT_GRTAB2-' W_GROUP INTO W_FIELD.
      ASSIGN (W_FIELD) TO <FS2>.
      LOOP AT IT_GRTAB2 WHERE  WAERS EQ IT_TAB-WAERS.
        IF <FS> EQ <FS2>.
  ...
  CASE W_SELECT.
    ...
    WHEN 'IB'.
      WRITE : (24) 'House B/L No.'     CENTERED NO-GAP, SY-VLINE NO-GAP,
              (10) 'B/L create'        CENTERED NO-GAP, SY-VLINE NO-GAP,
    ...
    WHEN 'GR'.
      WRITE : (24) 'House B/L No.'     CENTERED NO-GAP, SY-VLINE NO-GAP,
              ...
              (14) 'Entry No'          CENTERED NO-GAP, SY-VLINE NO-GAP.
