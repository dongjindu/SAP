  ...
  CASE ok_code.
    ...
    WHEN 'POST'.
      IF it_summ_data[] IS INITIAL.
        MESSAGE i000 WITH 'Already parked '.
      ELSE.
...
* BEGIN OF UD1K954236 - Adjust BDC to park document instead post it
* Copy from program ZACOU151 and modify it.
*&---------------------------------------------------------------------*
...
*1
  ...
  PERFORM bdc_field   USING 'RF05V-NEWBS'   '31'.            "31
  PERFORM bdc_field   USING 'BKPF-XBLNR'    p_refer.         "1912033020  :  Reference
  lw_claim_amount_c = abs( p_ven1 ).
  PERFORM bdc_field   USING 'RF05V-NEWKO'   lw_claim_amount_c.         "500187
*2
  ...
  PERFORM bdc_field   USING 'RF05V-NEWBS'  '40'.            "40
  lw_claim_amount_c = abs( p_expact ).
  PERFORM bdc_field   USING 'RF05V-NEWKO'   lw_claim_amount_c.       "532160
*
...
*3 end
    IF w_bdc_tot = w_tabix.
      ...
      OR p_acct12 = it_summ_data_temp-acct11
      OR p_acct32 = it_summ_data_temp-acct11.
        PERFORM bdc_field   USING 'BSEG-ZUONR'   it_summ_data-entno.
        PERFORM bdc_field   USING 'BSEG-SGTXT'   it_summ_data-text11.
      ELSE.
        PERFORM bdc_field   USING 'BSEG-SGTXT'   ''.
      ENDIF.
      PERFORM bdc_field   USING 'RF05V-NEWBS'  '40'.
      lw_claim_amount_c = abs( it_summ_data_temp-acct11 ).
      PERFORM bdc_field   USING 'RF05V-NEWKO'  lw_claim_amount_c. "532165
*4
      w_tabix   = w_bdc_tot.
...
*12
  ...
  PERFORM bdc_field       USING 'BDC_CURSOR'   'RF05V-NEWBS'.
  PERFORM bdc_field       USING 'BDC_OKCODE'   '=BP'.

  CALL TRANSACTION 'F-63' USING it_bdc
                          MODE p_bdcmod
                          UPDATE 'S'
