...
FORM get_table_data.
  ...
  PERFORM show_progress USING 'Read FTZ I/F...' '30'.
...
** f-43
* "KR" Select
  SORT it_summ_data BY no.
  READ TABLE it_summ_data INDEX 1.
* not "CN" Select
  READ TABLE it_summ_data_temp WITH KEY no = '2_XX'.
  w_amt      = it_summ_data-duty_amt
             + it_summ_data_temp-duty_amt.
*1
  ...
  PERFORM bdc_field   USING 'RF05A-NEWKO'   p_ven1.         "500187

* Only "KR"
  IF it_summ_data-no = '1_KR'.

    IF it_summ_data_temp-duty_amt = 0.
*2
      PERFORM bdc_dynpro  USING 'SAPMF05A'     '0302'.
      PERFORM bdc_field   USING 'BDC_CURSOR'   'RF05A-NEWKO'.
      PERFORM bdc_field   USING 'BDC_OKCODE'   '/00'.
      lw_claim_amount_c = abs( w_amt ).
      ...
      PERFORM bdc_field   USING 'RF05A-NEWKO'   p_expact.   "532160
*3
      ...
      PERFORM bdc_field   USING 'BSEG-WRBTR'    lw_claim_amount_c. "10,000
      PERFORM bdc_field   USING 'RF05A-NEWBS'  '31'.            "31
      PERFORM bdc_field   USING 'RF05A-NEWKO'   it_summ_data-acct11. "532165
*4
...
*5
      ...
      PERFORM bdc_field   USING 'RKEAK-FIELD(15)'  'US'.          "US
*6
      PERFORM bdc_dynpro  USING 'SAPLKACB'   '0002'.
      PERFORM bdc_field   USING 'BDC_CURSOR' 'DKACB-ERGOKONT'.
...
*11-1
      ...
      PERFORM bdc_field   USING 'BDC_OKCODE'   '/00'.
      lw_claim_amount_c = abs( it_summ_data-duty_amt ).
      PERFORM bdc_field   USING 'BSEG-WRBTR'    lw_claim_amount_c. "8,000
      PERFORM bdc_field   USING 'RF05A-NEWBS'  '40'.            "31
      PERFORM bdc_field   USING 'RF05A-NEWKO'   p_expact.   "532160
*11-2
      PERFORM bdc_dynpro  USING 'SAPLKACB'     '0002'.
      PERFORM bdc_field   USING 'BDC_CURSOR'   'DKACB-ERGOKONT'.
...
*13-1 a
      ...
      PERFORM bdc_field   USING 'BSEG-WRBTR'    lw_claim_amount_c. "2000
      PERFORM bdc_field   USING 'RF05A-NEWBS'  '31'.            "31
      PERFORM bdc_field   USING 'RF05A-NEWKO'   p_ven2.
*13-2 a
      PERFORM bdc_dynpro  USING 'SAPLKACB'     '0002'.
      PERFORM bdc_field   USING 'BDC_CURSOR'   'DKACB-ERGOKONT'.
      PERFORM bdc_field   USING 'BDC_OKCODE'   '=COBL_XERGO'.
...
*7
      ...
      PERFORM bdc_field   USING 'BDC_CURSOR'   'RF05A-NEWKO'.
      PERFORM bdc_field   USING 'BDC_OKCODE'   '/00'.
      lw_claim_amount_c = abs( w_total_amt_cn ).
      PERFORM bdc_field   USING 'BSEG-WRBTR'    lw_claim_amount_c.          "382,531.31-
      PERFORM bdc_field   USING 'BSEG-ZFBDT'    w_docdt.    "01/17/2012
      PERFORM bdc_field   USING 'BSEG-SGTXT'    w_ventxt.             "US-custom-01912012033 06
      PERFORM bdc_field   USING 'RF05A-NEWBS'  '40'.                  "40
      PERFORM bdc_field   USING 'RF05A-NEWKO'   it_summ_data-acct11. "215011
    ENDIF.
*8 x
    PERFORM bdc_dynpro  USING 'SAPMF05A'    '0300'.
    PERFORM bdc_field   USING 'BDC_CURSOR'  'RF05A-NEWKO'.
    PERFORM bdc_field   USING 'BDC_OKCODE'  '/00'.
    lw_claim_amount_c = abs( it_summ_data-duty_amt ).
    PERFORM bdc_field   USING 'BSEG-WRBTR'   lw_claim_amount_c. "280,738.84
    PERFORM bdc_field   USING 'BSEG-MWSKZ'  'U0'.                   "U0
    ...
    PERFORM bdc_field   USING 'RF05A-NEWKO'  it_summ_data-acct12.
*    PERFORM bdc_field   USING 'BDC_SUBSRC'   'SAPLKACB'.
...
*10 x
    ...
    PERFORM bdc_field   USING 'BDC_CURSOR'  'RF05A-NEWKO'.
    PERFORM bdc_field   USING 'BDC_OKCODE'  '/00'.
    lw_claim_amount_c = abs( it_summ_data-amt012 ).         "amt012 ).
    PERFORM bdc_field   USING 'BSEG-WRBTR'   lw_claim_amount_c.     "485
    PERFORM bdc_field   USING 'BSEG-MWSKZ'  'U0'.                   "U0
    PERFORM bdc_field   USING 'BSEG-SGTXT'   it_summ_data-text11.   "06
*   Next data read
    lw_claim_amount_c = abs( it_summ_data-acct11 ).         "amt012 ).
    READ TABLE it_summ_data INDEX w_tabix.
    IF sy-subrc = 0.
*11
      PERFORM bdc_field   USING 'RF05A-NEWBS'  '40'.
      lw_claim_amount_c = abs( it_summ_data-acct11 ).       "amt012 ).
      PERFORM bdc_field   USING 'RF05A-NEWKO'  lw_claim_amount_c.
    ELSE.
*11
      PERFORM bdc_dynpro  USING 'SAPLKACB'     '0002'.
      PERFORM bdc_field   USING 'BDC_CURSOR'   'DKACB-ERGOKONT'.
...
FORM park_data_move.
  ...
  CLEAR: w_total_amt_xx,  w_total_amt_cn.
...
*    it_summ_data-text12    =  it_data-text11.    "text
* "KR"
    IF it_data-land1     = 'KR'.
...
* "space"
    ELSEIF it_data-land1  = ''.
      ...
      it_summ_data-land1 = ''.
      it_summ_data-entno = ''.
      w_total_amt_xx = w_total_amt_xx + it_data-duty_amt.
* "CN"
    ELSE.
      it_summ_data-no    = '3_CN'.
      it_summ_data-land1 = ''.
      w_total_amt_cn = w_total_amt_cn + it_data-duty_amt.
    ENDIF.
...
* total(screen)
  w_total_amt_cn = w_total_amt_cn
                 + p_amt012 + p_amt022 + p_amt032.

ENDFORM.                    " park_data_move
*&---------------------------------------------------------------------*
...
*  AND p_text22 = ''.
  ELSE.
    ...
    OR p_amt022 = 0.
*    OR p_text22 = ''.
      MESSAGE i000 WITH 'no.2 all required entry fields'.   "i e a
      w_check = '2'.
      EXIT.
    ENDIF.
  ENDIF.
...
*   clear: it_post_data, it_post_data[].
*   loop at it_summ_data where land1 = 'KR'.
*     it_post_data-duty_amt = it_post_data-duty_amt
*                           + it_summ_data-duty_amt.
*   endloop.
*   it_post_data-acct11 = p_ven1.     "Vendor
*   it_post_data-text11 = p_ventxt.   "text
*   it_post_data-acct12 = ''.         "Duty MPF
*   it_post_data-text12 = ''.         "text
...
* balance data --> high amount
  SORT it_data DESCENDING BY entno duty_amt.
