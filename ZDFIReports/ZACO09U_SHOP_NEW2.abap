...
*                                                                      *
*&=====================================================================*
REPORT  ZACO09U_SHOP_NEW2   MESSAGE-ID zmco_shop.
...
DATA: BEGIN OF gt_bom OCCURS 0,
         ...
         eitm   LIKE stpox-eitm,     "eitm,      " EndItem
         stgb   LIKE stpox-stgb,     "stgb,      " Str.Type
         ojtxb  LIKE stpox-ojtxb,     " description
         upgn   LIKE stpox-upgn,     "upgn,      " user field - upg
         chk,
      END OF gt_bom.
...
SELECTION-SCREEN COMMENT  1(15)  text-032 FOR FIELD p_std.        "Standard Plan
*SELECTION-SCREEN POSITION 15.
PARAMETERS : p_std RADIOBUTTON GROUP ra01 USER-COMMAND  cty.

SELECTION-SCREEN COMMENT  35(15) text-031 FOR FIELD p_bpl.        "Business Plan
*SELECTION-SCREEN POSITION 39.
PARAMETERS : p_bpl RADIOBUTTON GROUP ra01.
SELECTION-SCREEN END OF LINE.

PARAMETERS: p_freig LIKE keko-freig DEFAULT 'X'.                  "Rel. std cost est.
...
PARAMETERS : p_elehk LIKE tckh4-elehk  DEFAULT 'K1'   MODIF ID div.

SELECTION-SCREEN SKIP 1.

* Costing Type
SELECTION-SCREEN BEGIN OF BLOCK bl2 WITH FRAME TITLE text-002.
PARAMETERS : p_step1  RADIOBUTTON GROUP ra02,                     "Step1: Collect LTP quantity
             p_step2  RADIOBUTTON GROUP ra02.                     "Step2: Calculate ABP Shop Cost
PARAMETERS : p_plscn  LIKE plaf-plscn DEFAULT '001' OBLIGATORY.   "Planning Scenario
...
PARAMETERS : p_bdatj LIKE keko-bdatj MEMORY ID bdtj OBLIGATORY
                          DEFAULT sy-datum(4).                    "Year
* periods
*selection-screen begin of line.
*selection-screen comment  1(30) text-021. "From
*selection-screen position 33.
PARAMETERS: p_perab LIKE covja-perab MEMORY ID vpe MODIF ID per
                          DEFAULT sy-datum+4(2) OBLIGATORY.       "From Period
...
* Controlling Area Information
  ...
  IF p_step1 = 'X'.               "Step1: Collect LTP quantity
...
*----End
  IF p_perab > 12 OR p_perab < 1.
    MESSAGE e000 WITH 'Period & sould be in the range from 1 to 12.'.
*    MESSAGE e007.
  ENDIF.
*Issue # 20041201-001 requested by HS CHO
*Changed by wskim,on 01112005
...
*           FROM zvco_rp1             "Vehicle Scheduling - Reference Rate Routing (Copy :ZVPP_RP1)
*          WHERE plnnr = c_rp_plnnr.
*  SORT it_zvco_rp1 BY usr00.
*  IF  it_zvco_rp1[] IS INITIAL.
*    MESSAGE e000 WITH 'Check your SHOP assignment using CA33'.
**    MESSAGE e077.
*  ENDIF.
...
* Appending
    LOOP AT it_t_kis1.
      ...
      it_kis1-losgr = it_ckikekokey-losgr.   "Lot Size for Product Costing

      APPEND it_kis1. CLEAR it_kis1.


      CASE it_t_kis1-typps.
        WHEN 'E'.
          gr_ccact-sign   = 'I'.
          gr_ccact-option = 'EQ'.
          gr_ccact-low    = it_t_kis1-parob.  "Partner Object
          APPEND gr_ccact.
          CLEAR  gr_ccact.
...
*      endif.
    ...
    MODIFY gt_shop INDEX l_idx
            TRANSPORTING fevor mtart kalnr beskz
                         sobsl vspvb shop peinh
                         matkl.
...
* CO-OM: View of Cost Component Layout..
  ...
  IF it_coomco[] IS INITIAL .
    MESSAGE e000 WITH text-081.
*    MESSAGE e081.  "There is no Costing Rate - Check T-Code 'KSBT'
  ENDIF.
...
* Success
  MESSAGE s000 WITH 'Successfully Completed Data Creation'.
...
FORM get_products.
  ...
  DATA lt_keko2 TYPE TABLE OF ckiuser WITH HEADER LINE.

  CLEAR  : it_ckikekokey.
  REFRESH: it_ckikekokey, r_matnr, lt_keko, lt_keko2.
...
*//Modify..03/22/2011..T00020..
  DATA : l_bdatj     TYPE bdatj,
         l_period    TYPE spmon.

  l_bdatj = p_bdatj - 1.

  CLEAR : l_where, l_where2.

  CASE 'X'.
    WHEN  p_bpl.
      l_where  = 'POPER IN GR_POPER'.
      CONCATENATE 'BDATJ = ' `'` p_bdatj `'` INTO l_where2
           RESPECTING BLANKS.
    WHEN OTHERS.
      CONCATENATE 'BDATJ IN (' `'` p_bdatj `'` ',' `'` l_bdatj `'` ')'
           INTO l_where2 RESPECTING BLANKS.
  ENDCASE.

*//

  SELECT * INTO CORRESPONDING FIELDS OF TABLE lt_keko2
    FROM keko
...
*//Modify..03/22/2011..T00020..
    SORT lt_keko2 BY matnr kadat DESCENDING.
    CONCATENATE p_bdatj p_perab+1(2) INTO l_period.
    LOOP AT r_matnr.
      LOOP AT lt_keko2 WHERE matnr     = r_matnr-low
                       AND   kadat(6) <= l_period
                       AND   bidat(6) >= l_period.
        MOVE-CORRESPONDING lt_keko2 TO lt_keko.
        APPEND lt_keko.
        CLEAR  lt_keko.
      ENDLOOP.
    ENDLOOP.
...
*
  SELECT * INTO CORRESPONDING FIELDS OF TABLE it_plaf_temp
   ...
   FROM plaf AS a                       "Planned order Table

   INNER JOIN mara AS b
      ON a~matnr = b~matnr

   WHERE a~plscn = p_plscn              "Scenario
     AND a~sobes = 'E'                  "Special procurement (Standard in-house production)
     AND a~stlan = '6'                  "BOM usage (Costing)
  ...
  DELETE FROM ztco_shop_plh          "Cost By Shop (STD/ABP) - Header
     WHERE kokrs = p_kokrs
       AND bdatj = p_bdatj
       AND poper = p_perab
       AND klvar = p_klvar
...
* LOG
  it_plaf-erdat = sy-datum.
  it_plaf-erzet = sy-uzeit.
  it_plaf-ernam = sy-uname.
  MODIFY it_plaf TRANSPORTING erdat erzet ernam
...
* CCS INFO
  ...
  REFRESH gt_shop_cc. CLEAR   gt_shop_cc.
...
*ABP
  IF p_bpl = 'X'.
    gr_poper-option = 'EQ'.
    gr_poper-sign   = 'I'.
    gr_poper-low    = p_perab.  "From Period
    APPEND gr_poper.
...
*
  SORT it_kis1 BY matnr typps upgvc.

  LOOP AT it_ckikekokey.

    CHECK it_ckikekokey-stlan <> space.            "BOM Usage
...
* get UPG
    ...
    LOOP AT gt_bom.
      ...
      IF gt_bom-stufe = 1.   "Level
        w_upg = gt_bom.
      ELSE.

        READ TABLE it_kis1 WITH KEY matnr = gt_bom-idnrk
                                    typps = 'M'
                                    upgvc = space BINARY SEARCH.
        CHECK sy-subrc = 0.
...
* Parents Material (FSC)
    READ TABLE it_ckikekokey WITH KEY kalnr = it_kis1-kalnr
                                      BINARY SEARCH.
    CHECK sy-subrc = 0.
...
FORM get_bom TABLES f_ckiuser STRUCTURE ckiuser .
  ...
  CALL FUNCTION 'CS_BOM_EXPL_MAT_V2'
    EXPORTING
      ...
      mdmps                 = 'X' "Limited multi-lvl- explode phantom at leas
...
FORM get_upg TABLES f_ckiuser STRUCTURE ckiuser .
  ...
  CHECK f_ckiuser-stlan <> space.  "BOM Usage
...
* get UPG
  ...
  LOOP AT gt_bom.
    ...
    READ TABLE it_kis1 WITH KEY indx  = gt_bom-index
                                BINARY SEARCH.
