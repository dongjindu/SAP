...
SELECT-OPTIONS: p_fictr FOR fmfctr-fictr MEMORY ID fis,
                p_fipos FOR fmfpo-fipos,   "default '600000' option GE,
...
AT LINE-SELECTION.
  ...
  CASE v_field.
*{ 09/21/11 PaUL iNSERT IFMFPO-BEZEICH
    WHEN 'IFMFPO-BEZEICH'.
*}
    WHEN 'ITAB-GEBER'.
...
* filter for commitment item
    ...
    READ TABLE ifmfpo  WITH KEY fipos = ifmit-rfipex.
    CHECK sy-subrc = 0.
...
* fill key field
  LOOP AT itab.
    CASE p_act.
      WHEN '1'.
        itab-key(10) = itab-fictr.  itab-key+10(1) = itab-knzaepo.
      WHEN '2'.
        itab-key(1) = itab-knzaepo. itab-key+1(10) = itab-fipos.
      WHEN '3'.
        itab-key(10) = itab-geber.  itab-key+10(1) = itab-knzaepo.
    ENDCASE.
...
FORM get_budget_period USING    f_fictr f_fipos f_geber
                       CHANGING f_profil.
  READ TABLE ifmfpo  WITH KEY fipos = f_fipos.
  IF sy-subrc = 0.
...
* commitment item text
  ...
  IF sy-subrc <> 0. CLEAR ifmfpo-bezeich. ENDIF.
...
*    itab-mamt   = itab-mamt * -1.
  ...
  WRITE:  gv NO-GAP, itab-fictr(6)       NO-GAP,
          ...
          gv NO-GAP, ifmfpo-bezeich(14)  NO-GAP,
