*----------------------------------------------------------------------*
* Program ID        : ZAFIU133
* Title             : Print Account Document - Excel
* Created on        : 04/06/2009
* Created by        : IG. Moon
* Specifications By : Andy Choi
* Description       : Print Accounting Document via Excel
*----------------------------------------------------------------------*
* Modification Logs
* Date       Developer  Request ID  Description
* 05/14/2012 VALERIAN   UD1K954591  Put additional information in print
*                                   document memo for G/L Acct.'151020'
* 02/28/2013 VALERIAN   UD1K956525  Add Reference Key in data selection
*-----------------------------------------------------------------------

REPORT zafiu133     MESSAGE-ID zmfi
...
DATA: tmp_dir LIKE rlgrap-filename,
      gv_dir  LIKE rlgrap-filename,
      gv_dir2 LIKE rlgrap-filename.
...
SELECT-OPTIONS:
  ...
  s_awkey  FOR   bkpf-awkey VISIBLE LENGTH 10,              "UD1K956525
...
'C:\SapXls\IV_template.xls' NO-DISPLAY.

DATA scr VALUE space.
*PARAMETERS scr AS CHECKBOX DEFAULT ' '.
...
*-------------------------------------------------------*
  ...
  ELSEIF sy-index  > p_limit.
    MESSAGE s000(zmfi) WITH 'Too many documents has been selected'.
    EXIT.
  ENDIF.
...
* BEGIN OF UD1K956525
  DATA: l_awkeylen TYPE I.
  IF NOT s_awkey[] IS INITIAL.
    LOOP AT s_awkey WHERE low <> ' '.
      l_awkeylen = STRLEN( s_awkey-low ).
      IF l_awkeylen <= 10.
        CONCATENATE s_awkey-low s_gjahr-low INTO s_awkey-low.
        modify s_awkey INDEX sy-tabix.
      ENDIF.
    ENDLOOP.
  ENDIF.
* END OF UD1K956525
  ...
  SELECT bukrs belnr gjahr bstat
   ...
   WHERE bukrs IN s_bukrs
     ...
     AND awkey IN s_awkey.                                  "UD1K956525
...
*// 2011.08.24 insert by YN.Kim.
      MOVE it_cb-bltxt         TO it_bkpf-bktxt.
      APPEND it_bkpf.
    ENDLOOP.
  ENDIF.
...
*  CREATE OBJECT excelsheet 'EXCEL.SHEET'.
*  CALL METHOD OF excelsheet 'APPLICATION' = excel.
  ...
  CALL METHOD  OF  workbooks 'OPEN' = open_workbooks
...
FORM excel_preview.
  ...
  CALL METHOD OF sheet 'PrintPreview' = exl_preview.
...
FORM write_memo TABLES   p_it_zsfiivprnmemo STRUCTURE zsfiivprnmemo
  ...
  LOOP AT p_it_zsfiivprnmemo.
    ...
    __fill_item $ix :
                  ...
                  p_it_zsfiivprnmemo-text.                  "UD1K954591
    l_pos = 0.
  ENDLOOP.
...
FORM check_tmp_dir CHANGING subrc.
  ...
  CLEAR : tmp_file, tmp_dir, gv_dir, gv_dir2.
...
FORM get_dirname USING    p_p_directory
  ...
  gv_dir = p_p_directory.

  PERFORM filter_dir CHANGING gv_dir.
*  p_p_directory = gv_dir.

*  $strlen = strlen( p_p_directory ).
  $strlen = strlen( gv_dir ).
...
FORM excel_form_check.
  ...
  tmp_file = 'C:\SapXls\IV_TEMPLATE.XLS'.
  ...
  SUBMIT zafiu132 WITH p_file EQ 'C:\SapXls\IV_template.xls'
  AND RETURN.
...
FORM excel_save.
  ...
  CALL METHOD OF sheet 'SAVEAS' = exl_saveas
