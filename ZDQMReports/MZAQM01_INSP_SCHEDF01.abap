...
FORM SAVE_DATA_0300.
  ...
  PERFORM DELETE_DELETED_ITEM_DB.
...
FORM CREATE_INSPECTION_LOT.
  ...
  IF SY-SUBRC <> 0.
    IF SY-SUBRC = 2.
      MESSAGE W000(ZMQM)
         WITH 'Material Inspection type is not consistent'(W02).
      EXIT.
    ENDIF.
  ...
  CLEAR : IT_ZSQM_INSP_SCH_ITEM_F-MARK.
...
FORM DELETE_SELECTED_ROW.
  ...
  DATA : LW_ITEM_F_INDEX LIKE SY-TABIX.

  LOOP AT IT_ZSQM_INSP_SCH_ITEM_F WHERE MARK = C_MARK
                                    AND STATUS = C_CREATION.
    LW_ITEM_F_INDEX = SY-TABIX.
    CLEAR IT_ITEM_F_B.
    READ TABLE IT_ITEM_F_B
             WITH KEY VEHICLE = IT_ZSQM_INSP_SCH_ITEM_F-VEHICLE
                      IYEAR   = IT_ZSQM_INSP_SCH_ITEM_F-IYEAR
                      ART     = IT_ZSQM_INSP_SCH_ITEM_F-ART
                      MATNR   = IT_ZSQM_INSP_SCH_ITEM_F-MATNR.

    IF SY-SUBRC = 0.
      CLEAR IT_ITEM_F_D.
      READ TABLE IT_ITEM_F_D
               WITH KEY VEHICLE = IT_ITEM_F_B-VEHICLE
                        IYEAR   = IT_ITEM_F_B-IYEAR
                        ART     = IT_ITEM_F_B-ART
                        MATNR   = IT_ITEM_F_B-MATNR.
      IF SY-SUBRC NE 0.
        MOVE-CORRESPONDING IT_ITEM_F_B TO IT_ITEM_F_D.
        APPEND IT_ITEM_F_D.
      ENDIF.
    ENDIF.

    DELETE IT_ZSQM_INSP_SCH_ITEM_F INDEX LW_ITEM_F_INDEX.
  ENDLOOP.

ENDFORM.                    " DELETE_SELECTED_ROW
*&------------------------------------------------------------------*
*&      Form  DELETE_DELETED_ITEM_DB
*&------------------------------------------------------------------*
FORM DELETE_DELETED_ITEM_DB.

 LOOP AT IT_ITEM_F_D.

  DELETE FROM ZTQM_INSP_ITEM WHERE VEHICLE = IT_ITEM_F_D-VEHICLE
                               AND IYEAR   = IT_ITEM_F_D-IYEAR
                               AND ART     = IT_ITEM_F_D-ART
                               AND MATNR   = IT_ITEM_F_D-MATNR.
  IF SY-SUBRC NE 0.
    ROLLBACK WORK.
    MESSAGE E000(ZMQM)
          WITH TEXT-E11 'ZTQM_INSP_ITEM'.
  ...
  DELETE FROM ZTQM_INSP_ITEM_F WHERE VEHICLE = IT_ITEM_F_D-VEHICLE
                                 AND IYEAR   = IT_ITEM_F_D-IYEAR
                                 AND ART     = IT_ITEM_F_D-ART
                                 AND MATNR   = IT_ITEM_F_D-MATNR.
  IF SY-SUBRC NE 0.
  ...
  DELETE FROM ZTQM_INSP_S_ITEM  WHERE VEHICLE = IT_ITEM_F_D-VEHICLE
                                  AND IYEAR   = IT_ITEM_F_D-IYEAR
                                  AND ART     = IT_ITEM_F_D-ART
                                  AND MATNR   = IT_ITEM_F_D-MATNR.
  IF SY-SUBRC NE 0.
...
ENDFORM.                    " DELETE_DELETED_ITEM_DB
