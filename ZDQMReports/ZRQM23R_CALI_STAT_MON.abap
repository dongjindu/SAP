************************************************************************
* Program Name      : ZRQM23R_CALI_STAT_MON
* Author            : SeungLyong, Lee
* Creation Date     : 2003.12.29.
* Specifications By : SeungLyong, Lee
* Pattern           : Report 1.2 - Call Screen
* Development Request No : UD1K905325
* Addl Documentation:
* Description       : Calibration Status Report by Monthly
*
* Modification Logs
...
REPORT  ZRQM23R_CALI_STAT_MON    NO STANDARD PAGE HEADING .
...
*// Structures
TABLES : ZSQM_CALI_MON. "/Calibration Status List by Monthly
TABLES : ZSQM_CALI_MON_B. "/Calibration Status : Basic List
...
DATA : IT_ZSQM_CALI_MON LIKE ZSQM_CALI_MON OCCURS 0
                                            WITH HEADER LINE.
*- Calibration Status List for only Confirmed order list
DATA : IT_ZSQM_CALI_CNF   LIKE ZSQM_CALI_MON OCCURS 0
                                            WITH HEADER LINE.
*- Calibration Status : Basic List
DATA : IT_ZSQM_CALI_MON_B LIKE ZSQM_CALI_MON_B OCCURS 0
                                            WITH HEADER LINE.
...
*-- get Calibration Status data from DB.
  ...
  IF IT_ZSQM_CALI_MON[] IS INITIAL.
...
START-OF-SELECTION.
  CHECK NOT IT_ZSQM_CALI_MON[] IS INITIAL.
...
*-- Fill Department Text
  CHECK NOT IT_ZSQM_CALI_MON_B[] IS INITIAL.

  SORT IT_ZSQM_CALI_MON_B BY MONTH ASCENDING.
...
*&------------------------------------------------------------------*
FORM GET_DATA_FROM_DB.
  DATA : LW_FIRST_DAY TYPE DATUM,
         LW_LAST_DAY  TYPE DATUM.

  CONCATENATE P_YEAR '0101' INTO LW_FIRST_DAY.
  CONCATENATE P_YEAR '1231' INTO LW_LAST_DAY.

  REFRESH : IT_ZSQM_CALI_MON, IT_ZSQM_CALI_CNF.

*-- Get order List and equipment Status
  SELECT E~AUFNR A~EQUNR C~PARNR AS PARNR_DEP
         G~SWERK B~STAT G~ABCKZ F~GSTRP
     INTO CORRESPONDING FIELDS OF TABLE IT_ZSQM_CALI_MON
       FROM ( ( ( ( ( EQUI AS A INNER JOIN JEST AS B
         ...
         WHERE  A~EQTYP = C_EQTYP_CALIBRATION
           ...
           AND  F~GSTRP BETWEEN LW_FIRST_DAY AND LW_LAST_DAY.
...
*- Fill confirmed order status field of IT_ZSQM_CALI_MON from
*- IT_ZSQM_CALI_CNF-STAT_AUF

  LOOP AT IT_ZSQM_CALI_CNF.

    READ TABLE IT_ZSQM_CALI_MON WITH KEY
                                   AUFNR = IT_ZSQM_CALI_CNF-AUFNR.
    IF SY-SUBRC = 0.
      LW_ORDER_INDEX = SY-TABIX.
      MOVE: IT_ZSQM_CALI_CNF-STAT_AUF TO IT_ZSQM_CALI_MON-STAT_AUF.
      MODIFY IT_ZSQM_CALI_MON INDEX LW_ORDER_INDEX.
    ENDIF.
  ENDLOOP.
...
*&------------------------------------------------------------------*
FORM RET_BASIC_LIST.
  REFRESH IT_ZSQM_CALI_MON_B.

  LOOP AT IT_ZSQM_CALI_MON.

    CLEAR IT_ZSQM_CALI_MON_B.

    MOVE : IT_ZSQM_CALI_MON-GSTRP+4(2) TO IT_ZSQM_CALI_MON_B-MONTH.

    CASE IT_ZSQM_CALI_MON-ABCKZ.
      WHEN C_EQUIP_ABC_INTERNAL.
        MOVE : 1 TO IT_ZSQM_CALI_MON_B-IN_SUM,
               1 TO IT_ZSQM_CALI_MON_B-IN_NCNF.
        IF NOT IT_ZSQM_CALI_MON-STAT_AUF IS INITIAL.
          MOVE : 1 TO IT_ZSQM_CALI_MON_B-IN_CNF.
          CLEAR IT_ZSQM_CALI_MON_B-IN_NCNF.
        ENDIF.
      WHEN C_EQUIP_ABC_EXTERNAL.
        MOVE : 1 TO IT_ZSQM_CALI_MON_B-EX_SUM,
               1 TO IT_ZSQM_CALI_MON_B-EX_NCNF.
        IF NOT IT_ZSQM_CALI_MON-STAT_AUF IS INITIAL.
          MOVE : 1 TO IT_ZSQM_CALI_MON_B-EX_CNF.
          CLEAR IT_ZSQM_CALI_MON_B-EX_NCNF.
        ENDIF.
      WHEN C_EQUIP_ABC_PATROL.
        MOVE : 1 TO IT_ZSQM_CALI_MON_B-PA_SUM,
               1 TO IT_ZSQM_CALI_MON_B-PA_NCNF.
        IF NOT IT_ZSQM_CALI_MON-STAT_AUF IS INITIAL.
          MOVE : 1 TO IT_ZSQM_CALI_MON_B-PA_CNF.
          CLEAR IT_ZSQM_CALI_MON_B-PA_NCNF.
        ENDIF.
    ENDCASE.

    COLLECT IT_ZSQM_CALI_MON_B.
...
*
**  LOOP AT IT_ZSQM_CALI_MON_B.
**    LW_INDEX = SY-TABIX.
**
**    SELECT SINGLE A~ORGTX
**                   INTO IT_ZSQM_CALI_MON_B-ORGTX
**         FROM T527X AS A INNER JOIN IHPA AS B
**              ON A~ORGEH = B~PARNR
**           WHERE B~PARNR = IT_ZSQM_CALI_MON_B-PARNR_DEP
**             AND B~PARVW = C_EQUIP_PARVW_DEPT.
**
**    MODIFY IT_ZSQM_CALI_MON_B INDEX LW_INDEX.
**  ENDLOOP.
*
...
FORM MASK_COLUMNS_OF_ALV_GRID TABLES   PT_FIELDCAT TYPE LVC_T_FCAT.
  ...
  CASE WA_LEVEL.
    WHEN C_BASIC.
      WA_STRUCTURE_NAME = 'ZSQM_CALI_MON_B'.

    WHEN C_DETAIL.
      WA_STRUCTURE_NAME = 'ZSQM_CALI_MON'.
  ENDCASE.
