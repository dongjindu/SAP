************************************************************************
* Program Name      : SAPMZRQM06_Q_SCORE_VENDOR
...
REPORT  SAPMZRQM06_Q_SCORE_VENDOR  NO STANDARD PAGE HEADING   .
...
*-- Get GR/GI data from DB
  SELECT   B~LIFNR B~MATNR B~BWART B~MEINS
     ...
     INTO CORRESPONDING FIELDS OF TABLE IT_ZTQM_Q_SCORE
       FROM  ( ZTQM_Q_SCORE AS A INNER JOIN MARA AS B
         ...
         WHERE A~ISSUEDAT IN S_PERIOD
          ...
          GROUP BY A~LIFNR A~MATNR  A~MEINH  A~MEINS.
...
FORM COLLECT_DATA_BY.
  ...
  DATA : LW_ZSQM_Q_GRGI LIKE IT_ZSQM_Q_GRGI.

  SORT IT_ZSQM_Q_GRGI  BY LIFNR MATNR ASCENDING.
  SORT IT_ZTQM_Q_SCORE BY LIFNR MATNR ASCENDING.
  ...
  LOOP AT IT_ZSQM_Q_GRGI.
    ...
    MOVE-CORRESPONDING IT_ZSQM_Q_GRGI TO LW_ZSQM_Q_GRGI.

    CLEAR : IT_ZSQM_Q_S_VEND.

    MOVE-CORRESPONDING LW_ZSQM_Q_GRGI TO IT_ZSQM_Q_S_VEND.
    CLEAR : IT_ZSQM_Q_S_VEND-MATNR,
            IT_ZSQM_Q_S_VEND-BWART.

    IT_ZSQM_Q_S_VEND-MEINH = 'MIN'.

*   '101', '102',            "/GR
*   '261', '901', '903',    "/GI
*   '262', '902', '904'
    CASE LW_ZSQM_Q_GRGI-BWART.
      WHEN '101'.
*       N/A
      WHEN '102'.
        IT_ZSQM_Q_S_VEND-MENGE_GR = IT_ZSQM_Q_S_VEND-MENGE_GR * -1.
      WHEN '261' OR '901' OR '903'.
        IT_ZSQM_Q_S_VEND-MENGE_GI = IT_ZSQM_Q_S_VEND-MENGE_GR.
        CLEAR IT_ZSQM_Q_S_VEND-MENGE_GR.
      WHEN '262' OR '902' OR '904'.
        IT_ZSQM_Q_S_VEND-MENGE_GI = IT_ZSQM_Q_S_VEND-MENGE_GR.
        IT_ZSQM_Q_S_VEND-MENGE_GI = IT_ZSQM_Q_S_VEND-MENGE_GI * -1.
        CLEAR IT_ZSQM_Q_S_VEND-MENGE_GR.
    ENDCASE.

    AT END OF MATNR.
      READ TABLE IT_ZTQM_Q_SCORE WITH KEY LIFNR = LW_ZSQM_Q_GRGI-LIFNR
                                          MATNR = LW_ZSQM_Q_GRGI-MATNR.
...
FORM CALCULATE_QUAL_SCORE.
  ...
  LOOP AT IT_ZSQM_Q_S_VEND.
    ...
    IF NOT IT_ZSQM_Q_S_VEND-MENGE_GI IS INITIAL.

      IT_ZSQM_Q_S_VEND-Q_SCORE =
            ( IT_ZSQM_Q_S_VEND-RKMNG / IT_ZSQM_Q_S_VEND-MENGE_GI )
           * 1000000
           + (  IT_ZSQM_Q_S_VEND-LINESTOP + IT_ZSQM_Q_S_VEND-QNT_CAMP
              + IT_ZSQM_Q_S_VEND-QNT_SALV + IT_ZSQM_Q_S_VEND-QNT_REPR ).

    ENDIF.

    MODIFY IT_ZSQM_Q_S_VEND.
  ENDLOOP.

  SORT IT_ZSQM_Q_S_VEND BY Q_SCORE ASCENDING.
...
FORM FILL_TEXT.
  ...
  LOOP AT IT_ZSQM_Q_S_VEND.
    ...
    MOVE : SY-TABIX TO IT_ZSQM_Q_S_VEND-SRNO.
...
* Set field attribute
  ...
  LOOP AT PT_FIELDCAT.
    ...
    ELSEIF PT_FIELDCAT-FIELDNAME = 'NAME1'.
      PT_FIELDCAT-KEY_SEL = C_MARK.
      PT_FIELDCAT-KEY     = C_MARK.
      PT_FIELDCAT-COLTEXT = 'Vendor'(T50).
