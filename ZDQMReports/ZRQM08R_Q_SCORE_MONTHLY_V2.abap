************************************************************************
* Program Name      : ZRQM08R_Q_SCORE_MONTHLY_V2
...
REPORT  ZRQM08R_Q_SCORE_MONTHLY_V2  NO STANDARD PAGE HEADING   .
...
**- Vendor : Delete by request during 1st int.test : 01/30/2004-sllee
...
**-- Get text of Vendor from table LFA1 -> 01/30/2004 : sllee
...
*         append an icon to show detail List of selected item.
    ...
    MOVE 'BY_VENDOR'        TO LS_TOOLBAR-FUNCTION.
    MOVE ICON_SUPPLIER      TO LS_TOOLBAR-ICON.
    MOVE 'Quality Score by Vendor'(T12) TO LS_TOOLBAR-QUICKINFO.
    MOVE 'by Vendor'(T13)   TO LS_TOOLBAR-TEXT.
    MOVE ' '                TO LS_TOOLBAR-DISABLED.

    APPEND LS_TOOLBAR TO E_OBJECT->MT_TOOLBAR.

***--- by Material Report Button.
    CLEAR LS_TOOLBAR.
    MOVE 3 TO LS_TOOLBAR-BUTN_TYPE.
    APPEND LS_TOOLBAR TO E_OBJECT->MT_TOOLBAR.
    CLEAR LS_TOOLBAR.
    MOVE 0 TO LS_TOOLBAR-BUTN_TYPE. "/ Button Type
    MOVE 'BY_MATERIAL'        TO LS_TOOLBAR-FUNCTION.
    MOVE ICON_MATERIAL        TO LS_TOOLBAR-ICON.
    MOVE 'Quality Score by Material'(T14) TO LS_TOOLBAR-QUICKINFO.
    MOVE 'by Material'(T15)   TO LS_TOOLBAR-TEXT.
    MOVE ' '                TO LS_TOOLBAR-DISABLED.
...
*   function codes defined in Class Definition and react accordingly.
    ...
    DATA : LW_QNS_MON LIKE ZSQM_QS_MON.
    ...
    ELSE.
      ...
      READ TABLE IT_ZSQM_QS_MON INTO LW_QNS_MON
                       INDEX LW_LINE_ROW-INDEX.

*"/get first day of selecte month for interface
      CONCATENATE P_YEAR
                  LW_QNS_MON-MONTH
                  '01'
                          INTO WA_INT_DATE.
      ...
      CASE E_UCOMM.
        WHEN 'BY_VENDOR'.
          EXPORT WA_INT_DATE FROM WA_INT_DATE TO MEMORY ID 'BY_VENDOR'.
          EXPORT S_EXTWG-LOW FROM S_EXTWG-LOW
                                          TO MEMORY ID 'BY_VENDOR_EXT'.

          CALL TRANSACTION 'ZQMR22' AND SKIP FIRST SCREEN.

          FREE MEMORY ID 'BY_VENDOR'.
          FREE MEMORY ID 'BY_VENDOR_EXT'.

        WHEN 'BY_MATERIAL'.
         EXPORT WA_INT_DATE FROM WA_INT_DATE TO MEMORY ID 'BY_MATERIAL'.

          CALL TRANSACTION 'ZQMR21' AND SKIP FIRST SCREEN.
...
*-   Using Purchasing source List(table : 'EORD')
  ...
  SELECT   B~ZBUDAT AS BUDAT B~BWART B~MEINS
       ...
       INTO CORRESPONDING FIELDS OF TABLE IT_ZSQM_QS_LIST
         FROM ( ( ( MKPF AS A        INNER JOIN MSEG AS B
            ...
            ON   D~MATNR = E~MATNR
*             AND B~WERKS = E~WERKS
                                    ) INNER JOIN EORD AS F
            ON   E~MATNR = F~MATNR
             AND E~WERKS = F~WERKS
           WHERE          "A~MJAHR = P_YEAR
                 B~ZBUDAT BETWEEN WA_BUDAT_LOW AND WA_BUDAT_HIGH
...
*             AND G~ZEORD = '00001'
             AND B~BWART IN ('101', '102',            "/GR
                              '261', '901', '903',    "/GI
                              '262', '902', '904')
            GROUP BY B~ZBUDAT B~BWART B~MEINS .
...
* Movement Type ”101” is positive (“102” is reduction)
* MSEG-BWART=261, 901,903 positive (“262”, “902”, “904” are reduction)
...
*  SORT IT_ZSQM_QS_DEF   BY BUDAT    ASCENDING.
...
*      PT_FIELDCAT-F4AVAILABL = ' '.
*      PT_FIELDCAT-VALEXI     = ' '.
      PT_FIELDCAT-COLTEXT = 'Month'(T60).
