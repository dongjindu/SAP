...
*                    MODIF ID EQT.
PARAMETERS : P_EQTYP  TYPE EQTYP  DEFAULT 'Z' OBLIGATORY
...
* Set listbox for Equipment type : 'Y' & 'Z'

  WA_FLD_NAME = 'P_EQTYP'.

  IF IT_LIST_BOX[] IS INITIAL.
    SELECT EQTYP AS KEY
           TYPTX AS TEXT
      INTO CORRESPONDING FIELDS OF TABLE IT_LIST_BOX
        FROM T370U
          WHERE EQTYP IN ('Y', 'Z')
            AND SPRAS = SY-LANGU.
...
*--  Get Equipment Summary list
  IF P_EQTYP = 'Z'.
    ...
    PERFORM RET_EQUIP_SUM_LIST_Z.

  ELSEIF P_EQTYP = 'Y'.

    PERFORM RET_EQUIP_SUM_LIST_Y.
...
*- Retrieve Data.
* If Equipment No is inputted for selection
...
*               AND A~EQART EQ 'QM_1000'
  ...
  ELSE.
*-   If EQUNR was not inputed, Get Equipment Number ranges data
...
* select data for usage decision
  SELECT A~EQTYP A~EQUNR B~AUFNR C~AUTYP C~AUART
*         D~GSTRP D~GLTRP
...
* select data for order confirmation
...
*- Retrieve Data.
  IF NOT S_EQUNR[] IS INITIAL.
    ...
    SELECT A~EQTYP A~EQUNR D~EQKTX  A~EQART
           C~KOSTL C~ABCKZ  E~ARBPL C~SWERK
           F~AUTYP F~AUART F~AUFNR  F~OBJNR AS OBJ_AUF
           G~GSTRP G~GLTRP

       INTO CORRESPONDING FIELDS OF TABLE IT_ZSQM_EQ_SUMM
         FROM  ( ( ( ( ( EQUI AS A INNER JOIN AFIH AS B
            ON   A~EQUNR = B~EQUNR ) INNER JOIN ILOA AS C
            ON   B~ILOAN = C~ILOAN ) INNER JOIN EQKT AS D
            ON   A~EQUNR = D~EQUNR ) INNER JOIN CRHD AS E
            ON   C~CR_OBJTY = E~OBJTY
             AND  C~PPSID   = E~OBJID ) INNER JOIN AUFK AS F
            ON   B~AUFNR  = F~AUFNR   ) INNER JOIN AFKO AS G
            ON   F~AUFNR  = G~AUFNR
          WHERE  A~EQTYP = P_EQTYP
            AND  A~EQUNR = S_EQUNR
            AND  C~SWERK IN S_SWERK
            AND  E~ARBPL IN S_ARBPL
            AND  F~AUART IN S_AUART
  ...
  SELECT A~EQTYP A~EQUNR B~AUFNR C~AUTYP C~AUART
   ...
   INTO CORRESPONDING FIELDS OF TABLE LT_ZVQM_EQ_SUM_UD
      FROM ( ( (  EQUI  AS A INNER JOIN AFIH AS B
         ON A~EQUNR  = B~EQUNR ) INNER JOIN AUFK AS C
         ON B~AUFNR  = C~AUFNR ) INNER JOIN QALS AS F
         ON C~AUFNR  = F~AUFNR )  LEFT OUTER JOIN QAVE AS K
         ON F~PRUEFLOS = K~PRUEFLOS
        FOR ALL ENTRIES IN IT_ZSQM_EQ_SUMM
          WHERE  A~EQTYP =  P_EQTYP
             AND A~EQUNR =  IT_ZSQM_EQ_SUMM-EQUNR
