...
* 02/09/2004 SL.LEE       UD1K907066  Copied from
*                                     ZSQM02U_INSP_SCORE_FORM
*                                      Requested by Mr. YK.Moon
*
*
************************************************************************
...
TABLES : ZSQM_INSP_RESULT_01, "/QM-Inspection Scorecard(Inspection lot)
                              " for Forms
         ZSQM_MIC,      "/MIC data/result data of Inspection Lot
         ZSQM_QNS_ITEM. "/Insp. Scheduling ITEM
...
*-- Inspection Result from QAMR/QAMV  :02/09/2204 - SLLEE
DATA : IT_ZSQM_MIC  LIKE ZSQM_MIC OCCURS 15 WITH HEADER LINE.
...
*-- Get Inspection Result by scheduling date : 02/09/2004 - sllee
*-- Regeusted by Mr. Moon.(02/07/2004)
  PERFORM GET_INSP_RESULT_ETC   USING P_PRUEFL
                                      WA_GENERAL_DATA-INSP_TYPE.
...
*&------------------------------------------------------------------*
*&      Form  GET_INSP_RESULT_ETC
*&------------------------------------------------------------------*
FORM GET_INSP_RESULT_ETC  USING P_QPLOS     TYPE QPLOS
                                P_INSP_TYPE .
  DATA : LW_INDEX  LIKE SY-TABIX.
  DATA : LW_FLD_NAME(30) TYPE C.
  DATA : LW_VERW(8) TYPE C.

  FIELD-SYMBOLS : <LW_FS>.

  REFRESH IT_ZSQM_MIC. CLEAR IT_ZSQM_MIC.

  SELECT A~PRUEFLOS A~MERKNR A~SATZSTATUS  A~CODE1 AS CODE1_RESULT
       A~MBEWERTG A~DBEWERTG  B~VERWMERKM
       B~KURZTEXT A~PRUEFER C~NAME_TEXTC
     INTO CORRESPONDING FIELDS OF TABLE IT_ZSQM_MIC
       FROM ( QAMR AS A INNER JOIN QAMV AS B
         ON   A~PRUEFLOS  = B~PRUEFLOS
          AND A~VORGLFNR  = B~VORGLFNR
          AND A~MERKNR    = B~MERKNR ) LEFT OUTER JOIN USER_ADDR AS C
*          AND A~MERKNR    = B~MERKNR ) INNER JOIN USER_ADDR AS C
         ON   A~PRUEFER   = C~BNAME
            WHERE  A~PRUEFLOS = P_QPLOS.
*                AND  A~SATZSTATUS = '5'  "/Process completed
*                AND  A~MBEWERTG = 'R'.    "/Rejected ('Not OK')

*-- Get Inspection Scheduling data by Inspection lot No.
  CLEAR ZSQM_QNS_ITEM.

  CASE P_INSP_TYPE .
    WHEN C_INSP_TYPE_ISIR  OR C_INSP_TYPE_REGULAR.

      SELECT SINGLE * INTO CORRESPONDING FIELDS OF ZSQM_QNS_ITEM
        FROM ZTQM_QNS_ITEM
    ...
    WHEN C_INSP_TYPE_MS.
      ...
      SELECT SINGLE * INTO CORRESPONDING FIELDS OF ZSQM_QNS_ITEM
        FROM ZTQM_QNS_IT_MS
          WHERE  PRUEFLOS_MS = P_QPLOS.

  ENDCASE.

*-- Check  Planned date of MIC
...
*-- prepare for Get MIC Code field of Inspection scheduling data
    LW_VERW = IT_ZSQM_MIC-VERWMERKM.

    REPLACE '_' WITH ' ' INTO LW_VERW.   "/Delete '_'
    IF SY-SUBRC = 0.
      CONDENSE LW_VERW NO-GAPS.            "/remove blank and condense
    ENDIF.

    CASE P_INSP_TYPE .
      WHEN C_INSP_TYPE_ISIR  OR C_INSP_TYPE_REGULAR.
        CONCATENATE  'ZSQM_QNS_ITEM-DATUV_0'
                      LW_VERW+3(2)   '0'
                                             INTO LW_FLD_NAME.
      WHEN C_INSP_TYPE_MS.
        CONCATENATE  'ZSQM_QNS_ITEM-DATUV_1'
                      LW_VERW+3(2)   '0'
                                             INTO LW_FLD_NAME.
    ENDCASE.

    IF LW_FLD_NAME IS INITIAL.
      CONTINUE.
    ENDIF.

    ASSIGN (LW_FLD_NAME) TO <LW_FS>.

    IF  <LW_FS> IS INITIAL. "/Not exist planned date of MIC
      DELETE IT_ZSQM_MIC INDEX LW_INDEX.   "Delete MIC Item
    ENDIF.

*    - Valuation Text.
    IF NOT IT_ZSQM_MIC-MBEWERTG IS INITIAL.
      PERFORM GET_VALUATION_TEXT   USING    IT_ZSQM_MIC-MBEWERTG
                                   CHANGING IT_ZSQM_MIC-MBEWERTG_T.

    ENDIF.
*    - Result Text
    IF NOT IT_ZSQM_MIC-CODE1_RESULT IS INITIAL.
      PERFORM GET_CODE_TEXTS USING   '1'
                                     'Q1'
                                     IT_ZSQM_MIC-CODE1_RESULT
                                     '000001'
                        CHANGING     IT_ZSQM_MIC-KURZTEXT_RESULT.

    ENDIF.

    MODIFY IT_ZSQM_MIC INDEX LW_INDEX.

  ENDLOOP.


ENDFORM.                    " GET_INSP_RESULT_ETC
*&------------------------------------------------------------------*
*&      Form  GET_INSP_SCH_DATA
*&------------------------------------------------------------------*
FORM GET_INSP_SCH_DATA USING  P_QPLOS     TYPE QPLOS
                              P_INSP_TYPE.
