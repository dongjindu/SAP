************************************************************************
* Program Name      : ZRQM19R_Q_SCORE_MATERIAL_V2_R
* Author            : Rashmit
...
REPORT  ZRQM19R_Q_SCORE_MATERIAL_V2_R  NO STANDARD PAGE HEADING   .
...
*                                         WITH HEADER LINE.
data: begin of IT_ZSQM_QS_LIST_MD occurs 0.
include structure ZSQM_QS_LIST_V2.
data:mblnr like mseg-mblnr,
     qmatv like marc-qmatv,
 end of IT_ZSQM_QS_LIST_MD.
data: IT_ZSQM_QS_LIST_MD_T like IT_ZSQM_QS_LIST_MD occurs 0 with header
line.
*-- Notification Data List
DATA : IT_ZSQM_QS_LIST_NT LIKE ZSQM_QS_LIST_V2 OCCURS 0
                                         WITH HEADER LINE.
* data: begin of IT_ZSQM_QS_LIST_NT occurs 0.
*include structure ZSQM_QS_LIST_V2,
*data:LNDWNTIME like ZSQM_CI_QMEL-LNDWNTIME,
* end of IT_ZSQM_QS_LIST_NT.
...
*                B~LIFNR F~NAME1.
  ...
  SELECT   b~mblnr B~BWART C~EXTWG B~MATNR
*  E~MAKTX
  B~MEINS
        G~LIFNR "rl
*         B~lifnr  "rl
...
*         ON   C~MATNR = E~MATNR )
*         INNER JOIN EORD AS G    "rl
*         ON   D~MATNR = G~MATNR   "rl
*          AND D~WERKS = G~WERKS )  "rl
*end  comment 1
* uncomment for vendor no and GI info
INNER JOIN EORD AS G    "rl
         ON   C~MATNR = G~MATNR   "rl
          AND B~WERKS = G~WERKS )  "rl

          INNER JOIN LFA1 AS H
         ON   G~LIFNR = H~LIFNR
       WHERE  B~ZBUDAT IN S_PERIOD "A~BUDAT IN S_PERIOD
         AND  B~WERKS IN S_WERKS
*         AND  D~QMATV = C_MARK "rl
*         AND  E~SPRAS = SY-LANGU
         AND  G~ZEORD = '00001'
         AND  B~BWART IN ('101', '102',            "/GR
                          '261', '901', '903',     "/GI
...
*       GROUP BY B~BWART C~EXTWG B~MATNR E~MAKTX B~MEINS
*                G~LIFNR H~NAME1.
***Add change to delete duplicate enteries.


***get material description
loop at IT_ZSQM_QS_LIST_MD.
select single  maktx from makt into IT_ZSQM_QS_LIST_MD-maktx where
matnr = IT_ZSQM_QS_LIST_MD-matnr and
spras = sy-langu.
select single  qmatv from marc into IT_ZSQM_QS_LIST_MD-qmatv where
matnr = IT_ZSQM_QS_LIST_MD-matnr and
Qmatv = 'X'.
modify IT_ZSQM_QS_LIST_MD.
endloop.


delete IT_ZSQM_QS_LIST_MD where Qmatv ne 'X'.
...
*-- Get Defect data from  Notification (Complaint Quantity)
  ...
  SELECT B~EXTWG A~MATNR D~MAKTX

  A~MGEIN as meins
*  SUM( A~LINESTOP ) AS LINESTOP
       SUM( A~RKMNG ) AS RKMNG
      sum( A~lndwntime ) as linestop
...
*           AND A~MAWERK = C~WERKS
        ...
        GROUP BY B~EXTWG A~MATNR D~MAKTX
        A~MGEIN .
