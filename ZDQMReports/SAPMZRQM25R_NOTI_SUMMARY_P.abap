...
*
* 05/19/2006 Haseeb       UD1K920783   Auto refresh requested by danial.
*            Mohammad     Hlp Desk #65MA6A3471
* 11/30/2006 Manju        UD1K923261   Program fix for Notification text
* 05/30/2007 Manju        UD1K940691   Notification Printing fix - *
*                                      Convert text stream to ITF format
* 10/07/2011 KDM          UP1K920274   FM Option Changed - Attach Error *
*                                      Index : KDM01
...
**Modified by Haseeb Mohammad 05-19-2006
DATA  SAVE_FLAG TYPE I VALUE 0.
DATA INI_DATE LIKE SY-DATUM.
**End of Modification
...
DATA : BEGIN OF WA_ST,
         ...
         REL  TYPE C   VALUE 'X',     "/Release
         CMP  TYPE C   VALUE 'X',     "/Completion
       END OF WA_ST.
...
* BEGIN OF CHANGE 100565 09/22/04
*DATA : IT_ZSQM_NOTI_SUM LIKE ZSQM_NOTI_SUM OCCURS 0
*                      WITH HEADER LINE,

DATA: BEGIN OF  IT_ZSQM_NOTI_SUM OCCURS 0.
        INCLUDE STRUCTURE   ZSQM_NOTI_SUM.
DATA: RKMNG LIKE QMEL-RKMNG. " 100565 add qty 02_26_07
DATA: SENDDAT LIKE QMEL-SENDDAT.
DATA: END OF IT_ZSQM_NOTI_SUM.

*DATA:   IT_ZSQM_NOTI_SUM_R LIKE ZSQM_NOTI_SUM_R OCCURS 0
*                              WITH HEADER LINE.
DATA: BEGIN OF  IT_ZSQM_NOTI_SUM_R OCCURS 0.
        INCLUDE STRUCTURE ZSQM_NOTI_SUM_R.
DATA: PLANDAT LIKE QMEL-PLANDAT.
DATA: SENDDAT LIKE QMEL-SENDDAT.
DATA: END OF IT_ZSQM_NOTI_SUM_R.

*DATA:   IT_ZSQM_NOTI_SUM_P LIKE ZSQM_NOTI_SUM_P OCCURS 0
*                              WITH HEADER LINE.
*&-----------------------------------------------------------Shiva.
DATA: BEGIN OF WA_NOTIF_SP,
        QMNUM        LIKE ZSQM_NOTI_SUM_P-QMNUM,
        KURZTEXT_COD LIKE ZSQM_NOTI_SUM_P-KURZTEXT_COD,
        CODEGRP_VH   LIKE ZSQM_NOTI_SUM_P-CODEGRP_VH,
        CODE_VH      LIKE ZSQM_NOTI_SUM_P-CODE_VH,
        MATNR        LIKE ZSQM_NOTI_SUM_P-MATNR,
        MAKTX        LIKE ZSQM_NOTI_SUM_P-MAKTX,
        RKMNG        LIKE ZSQM_NOTI_SUM_P-RKMNG,
        KURZTEXT_FE  LIKE ZSQM_NOTI_SUM_P-KURZTEXT_FE,
        KURZTEXT_UR  LIKE ZSQM_NOTI_SUM_P-KURZTEXT_UR,
        KURZTEXT_OT  LIKE ZSQM_NOTI_SUM_P-KURZTEXT_OT,
        KURZTEXT_AT  LIKE ZSQM_NOTI_SUM_P-KURZTEXT_AT,
        TXT04        LIKE ZSQM_NOTI_SUM_P-TXT04,
        TASK_TXT04   LIKE ZSQM_NOTI_SUM_P-TASK_TXT04,
        PLANDAT      LIKE ZSQM_NOTI_SUM_P-PLANDAT,
        SENDDAT      LIKE ZSQM_NOTI_SUM_P-SENDDAT,
       SUCCESS LIKE QMEL-SUCCESS,
        PANEL6       LIKE ZSQM_NOTI_SUM_P-PANEL6,
       COMPLETED LIKE QMEL-COMPLETED,
     END OF WA_NOTIF_SP.
DATA:   IT_ZSQM_NOTI_SUM_P LIKE WA_NOTIF_SP OCCURS 0
                              WITH HEADER LINE.
...
* haseeb
DATA   V_DISPLAY TYPE STRING.
* haseeb


DATA: BEGIN OF WA_TXTLINE1,
         TDLINE(45),
      END OF WA_TXTLINE1.
DATA IT_TXTLINE1 LIKE TABLE OF WA_TXTLINE1.

DATA: W_CUST_CONT1 TYPE REF TO CL_GUI_CUSTOM_CONTAINER,
      W_CUST_CONT2 TYPE REF TO CL_GUI_CUSTOM_CONTAINER,
      W_CUST_CONT3 TYPE REF TO CL_GUI_CUSTOM_CONTAINER,
      W_CUST_CONT4 TYPE REF TO CL_GUI_CUSTOM_CONTAINER,
      W_CUST_CONT5 TYPE REF TO CL_GUI_CUSTOM_CONTAINER,
      W_CUST_CONT6 TYPE REF TO CL_GUI_CUSTOM_CONTAINER,
      W_CUST_CONT7 TYPE REF TO CL_GUI_CUSTOM_CONTAINER,
      W_EDITOR1    TYPE REF TO CL_GUI_TEXTEDIT,
      W_EDITOR2    TYPE REF TO CL_GUI_TEXTEDIT,
      W_EDITOR3    TYPE REF TO CL_GUI_TEXTEDIT,
      W_EDITOR4    TYPE REF TO CL_GUI_TEXTEDIT,
      W_EDITOR5    TYPE REF TO CL_GUI_TEXTEDIT,
      W_EDITOR6    TYPE REF TO CL_GUI_TEXTEDIT,
      W_EDITOR7    TYPE REF TO CL_GUI_TEXTEDIT.
DATA: W_TDID   LIKE THEAD-TDID,
...
DATA: IT_TLINES LIKE TABLE OF WA_TLINE WITH HEADER LINE.
DATA: W_G_OKCODE LIKE SY-UCOMM.
*&-------------------------------------------------------end shiva.
DEFINE RANGE_SET.
  MOVE : 'I' TO &1-SIGN.
...
*-- Set listbox for notification type(ZSQM_NOTI_SEL-QMART) in scr. 9000
  ...
  PERFORM DEFAULT_VENDOR.
*  if sy-uname = '100565' or sy-uname = '100471'.
*    ZSQM_NOTI_SEL-lifnum = 'N006'.
*  endif.
...
*         append an icon to show detail List of selected item.
    ...
    MOVE 'IMPROVEMENT RESULT'  TO LS_TOOLBAR-FUNCTION.
*        MOVE ICON_DETAIL    TO LS_TOOLBAR-ICON.
    MOVE ICON_CREATE_TEXT TO LS_TOOLBAR-ICON.

    MOVE 'Improv result'(T12) TO LS_TOOLBAR-QUICKINFO.
    MOVE 'Imp result'(T13)      TO LS_TOOLBAR-TEXT.
...
***Modified by Haseeb Mohammad on 05-22-2006.
*  IF NOT GRID_CONTAINER IS INITIAL AND "/Not Created  for ALV GRID
*     WA_RENEWAL_FLG = C_MARK.

  IF NOT GRID_CONTAINER IS INITIAL. "/Not Created  for ALV GRID
    CLEAR WA_RENEWAL_FLG.
***End of change by Haseeb
*-- Display data on ALV GRID Control using method
*-- 'SET_TABLE_FOR_FIRST_DISPLAY'
    PERFORM SET_TABLE_TO_ALV.
...
*&------------------------------------------------------------------*
MODULE USER_COMMAND_9100 INPUT.
  DATA: BEGIN OF WA_LVC_TROW.
          INCLUDE STRUCTURE LVC_S_ROW.
  DATA: END OF WA_LVC_TROW.
  DATA:  IT_LVC_TROW TYPE LVC_T_ROW,
         W_INDEX TYPE I.
...
*-- Get data from DB AND Process
    ...
    WHEN OTHERS.
      ...
      IF W_G_OKCODE = 'IMPROVEMENT RESULT'.
        CALL METHOD ALV_GRID->GET_SELECTED_ROWS
          IMPORTING
            ET_INDEX_ROWS = IT_LVC_TROW.
        READ TABLE IT_LVC_TROW INTO WA_LVC_TROW INDEX 1.
        W_INDEX = WA_LVC_TROW-INDEX.
        READ TABLE IT_ZSQM_NOTI_SUM_P INDEX W_INDEX.
        IF IT_ZSQM_NOTI_SUM_P-TXT04 NE 'OSNO'.
          QMEL-QMNUM = IT_ZSQM_NOTI_SUM_P-QMNUM.
          QMEL-PLANDAT = IT_ZSQM_NOTI_SUM_P-PLANDAT.
          QMEL-COMPLETED = IT_ZSQM_NOTI_SUM_P-COMPLETED.
          ZSQM_NOTI_SUM_P-PANEL6 = IT_ZSQM_NOTI_SUM_P-PANEL6.
          CALL SCREEN '9200'.
        ELSE.
          MESSAGE ID 'ZMQM' TYPE 'I' NUMBER '001'
                                     WITH IT_ZSQM_NOTI_SUM_P-QMNUM.
          EXIT.
        ENDIF.
      ENDIF.
  ENDCASE.
...
*     I_STRUCTURE_NAME = 'ZSQM_NOTI_SUM_R'
      I_STRUCTURE_NAME = 'ZSQM_NOTI_SUM_P'
    CHANGING
...
* Add field for Quantity
    ELSEIF PT_FIELDCAT-FIELDNAME = 'RKMNG'.
      PT_FIELDCAT-COLTEXT = 'Complaint Quantity'(T62).
...
*     - Notification list for Q2, Q3 AND Q2 + Q3
      IF ZSQM_NOTI_SUM_P-QMNUM IS INITIAL.
        SELECT A~QMNUM A~QMART A~KATART_VH A~CODEGRP_VH A~CODE_VH
               A~MAWERK A~EXTWG A~KATART_AT A~CODEGRP_AT A~CODE_AT
* "100565 add qty 02_26_07
               A~RKMNG A~PANEL6
...
*          AND  B~FEGRP      IN (C_DEFECT_FEGRP1, C_DEFECT_FEGRP2)
   ...
   AND  I~SPRAS      = SY-LANGU
*HASEEB
      AND  A~QMNUM      =  ZSQM_NOTI_SUM_P-QMNUM
*HASEEB
...
*& Changes made by Haseeb Mohammad on 05-19-2006.
*& Transport Request UD1K920783 , Hlp Desk #65MA6A3471
*&---------------------------------------------------------------*
MODULE AUTO_REFRESH OUTPUT.
  IF SAVE_FLAG EQ 1.
    PERFORM GET_DATA_AND_PROCESS.
...
FORM MOVE_DATA_FOR_DISPLAY.
  ...
  LOOP AT IT_ZSQM_NOTI_SUM.
    CLEAR IT_ZSQM_NOTI_SUM_P.
    MOVE-CORRESPONDING IT_ZSQM_NOTI_SUM TO IT_ZSQM_NOTI_SUM_P.
    APPEND IT_ZSQM_NOTI_SUM_P.
  ENDLOOP.
...
*       EXPORTING I_STRUCTURE_NAME = 'ZSQM_NOTI_SUM_R'
       EXPORTING I_STRUCTURE_NAME = 'ZSQM_NOTI_SUM_P'
...
*                 IT_OUTTAB        = IT_ZSQM_NOTI_SUM_R[].
                 IT_OUTTAB        = IT_ZSQM_NOTI_SUM_P[].

*             IT_OUTTAB        =   IT_ZSQM_NOTI_SUM[].
...
*   CONCATENATE SY-DATUM(6) '01' INTO ZSQM_NOTI_SEL-ERDAT_L.
  ...
  MOVE: INI_DATE TO ZSQM_NOTI_SEL-ERDAT_L.
...
FORM DEFAULT_VENDOR.
  ...
  ENDIF.
ENDFORM.                    " DEFAULT_VENDOR
*&---------------------------------------------------------------------*
*&      Form  copy_other_fields_to_table
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
...
*----------------------------------------------------------------------*
FORM COPY_OTHER_FIELDS_TO_TABLE.
  DATA: WA_QMNUM LIKE QMEL-QMNUM,
        WA_QMART LIKE QMEL-QMART.

  MOVE 'Q2' TO ZTQM_NOTI_PORTAL-QMART.
  MOVE QMEL-QMNUM TO ZTQM_NOTI_PORTAL-QMNUM.
  MODIFY ZTQM_NOTI_PORTAL.

ENDFORM.                    " copy_other_fields_to_table
*&---------------------------------------------------------------------*
*&      Form  save_other_text_box
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
*  <--  p2        text
*----------------------------------------------------------------------*
FORM SAVE_OTHER_TEXT_BOX USING WA_CHECK1 WA_CHECK3 WA_CHECK4 WA_CHECK5
WA_CHECK6 WA_CHECK7.
  IF QMEL-COMPLETED = 'X'.
* If the completed checkbox is checked check to see if all the
* fields and text boxes have been filled

    SELECT SINGLE * FROM ZTQM_NOTI_PORTAL WHERE QMART = 'Q2'
                                          AND QMNUM = QMEL-QMNUM.
    IF ZTQM_NOTI_PORTAL-ZAT_HMMA = SPACE OR
    ZTQM_NOTI_PORTAL-ZAT_TIER1 = SPACE OR
    ZTQM_NOTI_PORTAL-ZIN_TRANSIT = SPACE OR
    ZTQM_NOTI_PORTAL-ZOTHER = SPACE OR
    ZTQM_NOTI_PORTAL-ZGOOD_PART_IDEN = SPACE OR
    ZTQM_NOTI_PORTAL-ZCLEAN_DATE = SPACE OR
    ZTQM_NOTI_PORTAL-ZPER_INSPECTED = SPACE OR
    ZTQM_NOTI_PORTAL-ZPER_DEFECTED = SPACE OR
    ZTQM_NOTI_PORTAL-ZINSP_AGREEMENT = SPACE OR
    ZTQM_NOTI_PORTAL-ZWORK_INST = SPACE OR
    ZTQM_NOTI_PORTAL-ZPFMEA = SPACE OR
    ZTQM_NOTI_PORTAL-ZDFMEA = SPACE OR
    ZTQM_NOTI_PORTAL-ZCONTROL_PLAN = SPACE OR
    ZTQM_NOTI_PORTAL-ZPROCESS_FLOW = SPACE OR

    ZTQM_NOTI_PORTAL-ZOTHERS = SPACE OR
    WA_CHECK1 NE 'X' OR WA_CHECK3 NE 'X'
    OR WA_CHECK4 NE 'X'OR WA_CHECK5 NE 'X' OR WA_CHECK6 NE 'X'
    OR WA_CHECK7 NE 'X'.
      MESSAGE ID 'ZMQM' TYPE 'E' NUMBER '002'
                                           WITH ZTQM_NOTI_PORTAL-QMNUM.
      CLEAR: WA_CHECK1, WA_CHECK3 ,WA_CHECK4 ,WA_CHECK5,
      WA_CHECK6 ,WA_CHECK7.
...
*----------------------------------------------------------------------*
FORM MODIFY_STXH.
  STXH-TDFRELES = SY-SAPRL.
  STXH-TDFUSER = SY-UNAME.
  STXH-TDFDATE = SY-DATUM.
  STXH-TDFTIME = SY-UZEIT.

  STXH-TDLRELES = SY-SAPRL.
  STXH-TDLUSER = SY-UNAME.
  STXH-TDLDATE = SY-DATUM.
  STXH-TDLTIME = SY-UZEIT.
  MODIFY STXH.
