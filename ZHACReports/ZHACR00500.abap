...
*& #### :                                                               &*
*&--------------------------------------------------------------------------&*
REPORT ZHACR00500 MESSAGE-ID EU.
...
TYPES: BEGIN OF TYP_FIELDNAME,
       ALIAS     TYPE TABNAME,
       FIELDNAME TYPE FIELDNAME,
       AREA      TYPE CHAR30,
END OF TYP_FIELDNAME.

DATA: GT_TABLE     TYPE TYP_TABLE OCCURS 0 WITH HEADER LINE.
DATA: GT_FIELDNAME TYPE TYP_FIELDNAME OCCURS 0 WITH HEADER LINE.
...
DATA: GT_ABAP TYPE ABAPTXT255_TAB,
      GS_ABAP TYPE ABAPTXT255.

DATA: GT_ABAP1 TYPE ABAPTXT255_TAB.
DATA: GT_ABAP2 TYPE ABAPTXT255_TAB.
DATA: GT_ABAP3 TYPE ABAPTXT255_TAB.
DATA: GT_ABAP4 TYPE ABAPTXT255_TAB.
DATA: GT_ABAP5 TYPE ABAPTXT255_TAB.
DATA: GT_ABAP6 TYPE ABAPTXT255_TAB.


DATA: G_VALUE TYPE CHAR75.
...
DATA: GS_STR2 LIKE GS_STR.

DATA: G_NUMC2   TYPE NUMC2.
DATA: G_FIELD   TYPE CHAR30.
DATA: G_WORD    TYPE CHAR30.
DATA: G_BWORD   TYPE CHAR30.
...
DATA: GT_PGML LIKE ZACTPGML2 OCCURS 0 WITH HEADER LINE.
DATA: GT_STR2 LIKE AIND_STR2 OCCURS 0 WITH HEADER LINE.
DATA: GT_ASTB1 LIKE ZACTASTB1 OCCURS 0 WITH HEADER LINE.
DATA: GS_ASTB1 LIKE ZACTASTB1.
DATA: GT_ASTB2 LIKE ZACTASTB2 OCCURS 0 WITH HEADER LINE.
DATA: GS_ASTB2 LIKE ZACTASTB2.
DATA: GT_ASTB3 LIKE ZACTASTB3 OCCURS 0 WITH HEADER LINE.
DATA: GS_ASTB3 LIKE ZACTASTB3.

DATA: GT_D010INC LIKE D010INC OCCURS 0 WITH HEADER LINE.

SELECTION-SCREEN  BEGIN OF BLOCK: A05 WITH FRAME TITLE A05.

PARAMETERS:       P_ALLDEL   TYPE CHAR1 AS CHECKBOX.
...
*       Field catalog/columns - item table
  ...
  METHOD DISPLAY.
    ...
    REFRESH: GT_ASTB1, GT_ASTB2, GT_ASTB3, GT_ABAP1.
    CLEAR: GS_ASTB1, GS_ASTB2, GS_ASTB3, GS_ABAP.
    CLEAR: G_SW1.
...
*     PGM Source# SPLIT ##.
INCLUDE ZHACR00500_F02.
*INCLUDE ZHARC00500_F02.
...
*       Sub query# ### select## ## ###..### G_SW1# Initial## ## ## #.
        IF <FS_FIELD1> EQ 'SELECT' AND G_SW1 IS INITIAL.
          REFRESH:  GT_ABAP1.
          MOVE 'S'  TO  G_SW1.
        ENDIF.

*       SQL## ## ##
        IF <FS_FIELD1> CS '.' AND G_SW1 EQ 'S'.
          MOVE 'E'  TO  G_SW1.
          EXIT.
        ENDIF.

*       G_SW1, G_SW2# ###### SQL## ## ## switch
        CHECK G_SW1 IS NOT INITIAL.

        IF G_SW2 IS INITIAL.
          APPEND GS_ABAP  TO GT_ABAP1.
          MOVE 'X'  TO  G_SW2.
        ENDIF.
      ...
      CLEAR: G_SW2.
      CHECK GT_ABAP1[] IS NOT INITIAL AND G_SW1 EQ 'E'.

*     ## SQL## ## ####.
*     ## #### ## #####..
      PERFORM FIND_TABLE_COLUMN_RTN.

*     ### ### ### ### ##### #### ####.
**      CHECK GT_TABLE[] IS NOT INITIAL AND GT_FIELDNAME[] IS NOT INITIAL.
      PERFORM UPDATE_TO_TABLE_RTN.
...
*   this is not required for source code
  ...
  METHOD SEARCH_ABAP_SOURCE.
    ...
    LOOP AT GT_OBJECT ASSIGNING <LV_OBJ>.
      ...
      APPEND LINES OF LT_ABAP TO GT_ABAP.
...
START-OF-SELECTION.
  ...
  IF P_ALLDEL IS NOT INITIAL.
    DELETE FROM ZACTASTB1
          WHERE PROGNAME IN REPNAME.

    DELETE FROM ZACTASTB2.

    DELETE FROM ZACTASTB3.
...
INCLUDE ZHACR00500_F01.
*INCLUDE ZHARC00500_F01.
