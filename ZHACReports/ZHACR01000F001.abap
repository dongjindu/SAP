...
*   #####
    CASE LS_FDCAT-FIELDNAME.
      ...
      WHEN 'ARCSTATUS'.
        LS_FDCAT-SCRTEXT_L    = 'Archiving Status'.
        LS_FDCAT-SCRTEXT_M    = 'Archiving Status'.
        LS_FDCAT-SCRTEXT_S    = 'Archiving Status'.
        LS_FDCAT-COLTEXT      = 'Archiving Status'.
...
FORM SELECT_ADMI_RUN
  ...
  SELECT DISTINCT
   ...
   WHERE A~STATUS EQ 1
     AND A~DELETE_FLG EQ SPACE
     AND A~NOTGUILTY EQ SPACE
     AND A~WRITING EQ SPACE
...
* admi_varia
  ...
  IF SY-SUBRC EQ 0.
    SORT LT_LIST BY APPLIC OBJECT DOCUMENT.

    DELETE ADJACENT DUPLICATES FROM
      LT_LIST COMPARING APPLIC OBJECT DOCUMENT.
...
*     Used Table Space in Database [Byte]
      L_DB_SPACE_W = L_DB_SPACE_W + LT_LIST-DB_SPACE_W.
*     Used Index Database Space [Byte]
      L_DB_INDEX_W = L_DB_INDEX_W + LT_LIST-DB_INDEX_W.
*     Used Cluster Memory Space [Byte]
      L_DB_CLUST_W = L_DB_CLUST_W + LT_LIST-DB_CLUST_W.
*     Occupied Structure Memory Space [Byte]
      L_DB_STRUC_W = L_DB_STRUC_W + LT_LIST-DB_STRUC_W.
*     Size of All Header Data in an Archiving Session [Byte]
      L_FILE_HEAD = L_FILE_HEAD + LT_LIST-FILE_HEAD.
*     Total Application Data in an Archive Session
      L_FILE_DATA = L_FILE_DATA + LT_LIST-FILE_DATA.

      IF L_END_STATUS EQ C_FLAG_X.
        MOVE-CORRESPONDING LT_LIST TO PT_ADMI_RUN.

        PT_ADMI_RUN-DB_SPACE_W = L_DB_SPACE_W.
        PT_ADMI_RUN-DB_INDEX_W = L_DB_INDEX_W.
        PT_ADMI_RUN-DB_CLUST_W = L_DB_CLUST_W.
        PT_ADMI_RUN-DB_STRUC_W = L_DB_STRUC_W.
        PT_ADMI_RUN-FILE_HEAD = L_FILE_HEAD.
        PT_ADMI_RUN-FILE_DATA = L_FILE_DATA.
...
FORM MOVE_DATA_TO_LIST
  ...
  DATA L_STATUS_FIL LIKE ADMI_FILES-STATUS_FIL.
  DATA L_DATE TYPE CHAR30.
...
*   Delete Status, Stroage Status
    PERFORM SELECT_ADMI_FILES
                          ...
                          CHANGING GT_LIST_LEFT-STATUS_FIL
                                   ...
                                   GT_LIST_LEFT-ZICON3.
*   Archiving Status
*    PERFORM GET_ARCSTATUS
*                      USING PT_ADMI_RUN-OBJECT
*                            PT_ADMI_RUN-VARIANTWRI
*                            PT_ADMI_RUN-APPLIC
*                            PT_ADMI_RUN-SAVEMONTHS
*                            PT_ADMI_RUN-ZFORMAT_TYP
*                      CHANGING GT_LIST_LEFT-ARCSTATUS.
...
*&---------------------------------------------------------------------*
*&      Form  CALL_TRANSACTION_ZHACR01100
*&---------------------------------------------------------------------*
FORM CALL_TRANSACTION_ZHACR01100 .
  ...
  IF LT_ROWS[] IS INITIAL.
    LEAVE TO TRANSACTION 'ZHACR01100'.
  ELSE.
    LOOP AT LT_ROWS INTO LS_ROWS .
      READ TABLE GT_LIST_LEFT INDEX LS_ROWS-INDEX.

      SET PARAMETER ID 'API' FIELD GT_LIST_LEFT-APPLIC.
...
ENDFORM.                    " CALL_TRANSACTION_ZHACR01100
*&---------------------------------------------------------------------*
...
*&      Form  GET_ARCSTATUS
*&---------------------------------------------------------------------*
FORM GET_ARCSTATUS  USING    P_OBJECT
                             P_VARIANTWRI
                             P_APPLIC
                             P_SAVEMONTHS
                             P_ZFORMAT_TYP
                    CHANGING P_ARCSTATUS.

  DATA L_RETYP TYPE BAPI_MTYPE.
  DATA LT_VALUTAB LIKE RSPARAMS OCCURS 0 WITH HEADER LINE.
  DATA LT_OBJECT LIKE VANZ OCCURS 0 WITH HEADER LINE.
  DATA L_REPORT LIKE ARCH_OBJ-REORGA_PRG.
  DATA: BEGIN OF LT_LIST OCCURS 0,
          FIELDNAME LIKE ZHACT0070-FIELDNAME,
        END OF LT_LIST.
  DATA LR_DATE LIKE BAPI_EMMA_DATE_RANGE OCCURS 0 WITH HEADER LINE.
  DATA L_LOW_X TYPE TVARV_VAL.
  ...
  LOOP AT LT_LIST.
    ...
    L_TABIX = SY-TABIX.
    READ TABLE LT_VALUTAB WITH KEY SELNAME = LT_LIST-FIELDNAME.

    IF SY-SUBRC EQ 0.
      CASE P_ZFORMAT_TYP.
        WHEN 1 OR 7.
          PERFORM CONVERT_DATE_TO_INTERN_FORMAT
                                          CHANGING: LT_VALUTAB-LOW,
                                                    LT_VALUTAB-HIGH.
          IF LT_VALUTAB-LOW IS NOT INITIAL.
            SET_RANGE LR_DATE 'I' 'BT' LT_VALUTAB-LOW LT_VALUTAB-HIGH.
          ENDIF.
        WHEN 2.
          SPLIT LT_VALUTAB-LOW AT '.' INTO: L_STR1 L_STR2.
          CONCATENATE '01' L_STR1 L_STR2 INTO L_LOW_X.
          PERFORM CONVERT_DATE_TO_INTERN_FORMAT
                                          CHANGING L_LOW_X.
          L_LOW_X = L_LOW_X(6).
        WHEN 3.
          L_LOW_X = LT_VALUTAB-LOW.
        WHEN 4.
        SET_RANGE LR_DATE 'I' 'BT' LT_VALUTAB-LOW(8) LT_VALUTAB-HIGH(8).
        WHEN 5.
          CONCATENATE L_LOW_X LT_VALUTAB-LOW INTO L_LOW_X.
        WHEN 6.
          IF L_TABIX EQ 2.
            CONCATENATE L_LOW_X LT_VALUTAB-LOW+1(2) INTO L_LOW_X.
          ELSE.
            CONCATENATE L_LOW_X LT_VALUTAB-LOW INTO L_LOW_X.
          ENDIF.
        WHEN 7.
        WHEN 9.
          L_LOW_X = LT_VALUTAB-SELNAME.
        WHEN 10.
      ENDCASE.
    ENDIF.
  ENDLOOP.
  ...
  CASE P_ZFORMAT_TYP.
    WHEN 1 OR 7 OR 4.
      IF LR_DATE[] IS NOT INITIAL.
        CALL FUNCTION 'ZH_AC_ARCHIVE_READ_YN'
          EXPORTING
            I_BUDAT     = LR_DATE
            I_SAVEMONTH = P_SAVEMONTHS
          IMPORTING
            E_RETYP     = L_RETYP.

        IF L_RETYP EQ 'Y'.
          GT_LIST_LEFT-ARCSTATUS = 'Need Archiving'.
        ENDIF.
      ENDIF.

    WHEN 3.
      IF L_LOW_X IS NOT INITIAL.
        PERFORM OIUPR_DATE_SUBTRACT_MONTH_3
                                    USING P_SAVEMONTHS
                                          L_LOW_X
                                    CHANGING GT_LIST_LEFT-ARCSTATUS.
      ENDIF.
    WHEN 2 OR 5 OR 6.
      PERFORM OIUPR_DATE_SUBTRACT_MONTH
                                   USING P_SAVEMONTHS
                                         L_LOW_X
                                   CHANGING GT_LIST_LEFT-ARCSTATUS.
    WHEN 8.
    WHEN 9.
      GT_LIST_LEFT-ARCSTATUS = L_LOW_X.
    WHEN 10.
  ENDCASE.

  CLEAR L_LOW_X.

ENDFORM.                    " GET_ARCSTATUS
*&---------------------------------------------------------------------*
*&      Form  CONVERT_DATE_TO_INTERN_FORMAT
*&---------------------------------------------------------------------*
FORM CONVERT_DATE_TO_INTERN_FORMAT
                            CHANGING P_LOW.
...
ENDFORM.                    " CONVERT_DATE_TO_INTERN_FORMAT
*&---------------------------------------------------------------------*
*&      Form  OIUPR_DATE_SUBTRACT_MONTH
*&---------------------------------------------------------------------*
FORM OIUPR_DATE_SUBTRACT_MONTH  USING    P_SAVEMONTHS
                                         P_LOW_6
                                CHANGING P_ARCSTATUS.

  DATA L_DATUM_TO LIKE SY-DATUM.
  DATA L_DATUM_FROM LIKE SY-DATUM.
  ...
  IF P_LOW_6 < L_DATUM_FROM(6).
    P_ARCSTATUS = 'Need Archiving'.
  ENDIF.

ENDFORM.                    " OIUPR_DATE_SUBTRACT_MONTH
*&---------------------------------------------------------------------*
*&      Form  OIUPR_DATE_SUBTRACT_MONTH_3
*&---------------------------------------------------------------------*
FORM OIUPR_DATE_SUBTRACT_MONTH_3
                              USING    P_SAVEMONTHS
                                       P_LOW_6
  ...
  IF P_LOW_6 < L_DATUM_FROM(4).
    P_ARCSTATUS = 'Need Archiving'.
  ENDIF.

ENDFORM.                    " OIUPR_DATE_SUBTRACT_MONTH_3
*&---------------------------------------------------------------------*
...
*       Basic Variant Column
        ...
        PERFORM GET_ARCSTATUS1 USING GT_LIST_LEFT-APPLIC
                                    GT_LIST_LEFT-OBJECT
                                    GT_LIST_LEFT-ZSEQ
                                    GT_LIST_LEFT-SAVEMONTHS.
...
*&      Form  GET_ARCSTATUS1
*&---------------------------------------------------------------------*
FORM GET_ARCSTATUS1  USING    P_APPLIC
                              P_OBJECT
                              P_ZSEQ
                              P_SAVEMONTHS.
  DATA L_RETYP.
  DATA LR_DATE LIKE BAPI_EMMA_DATE_RANGE OCCURS 0 WITH HEADER LINE.
  DATA: BEGIN OF LT_ZHACT0090,
              FIELD_SEQ LIKE ZHACT0090-FIELD_SEQ,
              FIELDNAME LIKE ZHACT0090-FIELDNAME,
              FROM_VALUE LIKE ZHACT0090-FROM_VALUE,
              TO_VALUE LIKE ZHACT0090-TO_VALUE,
            END OF LT_ZHACT0090.
  DATA L_SAVEMONTHS TYPE ZSAVEMONTHS1.
  ...
  SELECT SINGLE
    ...
    TO_VALUE
    FROM ZHACT0090
    INTO CORRESPONDING FIELDS OF LT_ZHACT0090
   WHERE ZAPPLI EQ P_APPLIC
     AND OBJECT EQ P_OBJECT
     AND ZSEQ EQ P_ZSEQ
     AND FIELD_SEQ EQ '1'.
    ...
    IF LT_ZHACT0090-FROM_VALUE IS NOT INITIAL AND
       LT_ZHACT0090-FROM_VALUE NE '00000000'.

       SET_RANGE LR_DATE 'I' 'BT'
       LT_ZHACT0090-FROM_VALUE LT_ZHACT0090-TO_VALUE.

       L_SAVEMONTHS = P_SAVEMONTHS.

      CALL FUNCTION 'ZH_AC_ARCHIVE_READ_YN'
        EXPORTING
          I_BUDAT     = LR_DATE
          I_SAVEMONTH = L_SAVEMONTHS
        IMPORTING
          E_RETYP     = L_RETYP.
