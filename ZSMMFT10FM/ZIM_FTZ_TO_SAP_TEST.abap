...
DATA: W_SUCCESS  TYPE I,
  ...
  REFRESH : IT_ZSFTZHD_LOG, IT_ZSFTZHS_LOG, IT_ZSFTZMT_LOG,
            IT_ZSFTZHD,     IT_ZSFTZHS,     IT_ZSFTZMT.
  REFRESH : IT_ENTRY.

  SELECT * INTO CORRESPONDING FIELDS OF TABLE IT_ZSFTZHD
  FROM   ZTFTZHD_LOG
  WHERE  ZFDOCNO  EQ  ZFDOCNO.

  SELECT * INTO CORRESPONDING FIELDS OF TABLE IT_ZSFTZHS
  FROM   ZTFTZHS_LOG
  WHERE  ZFDOCNO   EQ  ZFDOCNO.

  SELECT * INTO CORRESPONDING FIELDS OF TABLE IT_ZSFTZMT
  FROM  ZTFTZMT_LOG
  WHERE ZFDOCNO   EQ  ZFDOCNO.
...
*>> BDC CALL.
    ...
    ELSEIF W_SUBRC NE 0 AND W_ERROR_FLAG IS INITIAL.
       ...
       ENDIF.
    ELSEIF W_SUBRC NE 0 AND W_ERROR_FLAG EQ 'X'.
       W_FAIL     =  W_FAIL     +  1.
       LV_ZRESULT =  'E'.
    ENDIF.
