...
*  10/21/2011  Valerian  UD1K953238  Fix logic to consider posting with
*                                    negative hours
*                                    Exclude wage type 1294 & 1295 to
*                                    be refreshed
*  10/28/2011  Valerian  UD1K953303  Add input field for payroll period
*                                    Selections
*                                    Add addition logic for hist.data
*  11/23/2011  Valerian  UD1K953325  Adjust logic to prevent records
*                                    to be cleared for certain TM/date/
*                                    wage type for historical data
*----------------------------------------------------------------------

REPORT zahru014 MESSAGE-ID zmfi NO STANDARD PAGE HEADING LINE-SIZE 180.
...
INITIALIZATION.                                             "UD1K953303
  PERFORM get_pay_period.                                   "UD1K953303
...
TOP-OF-PAGE.
  WRITE: / text-h01, text-h02, text-h03 RIGHT-JUSTIFIED, text-h04,
...
* BEGIN OF UD1K952959
    IF $gt_out-f3 LT 0.
      gt_out-msg = 'Negative retro amount'.                 "UD1K953238
      gt_out-flg = 'W'.                                     "UD1K953238
*     gt_out-msg = 'Posting negative hours not allowed'.    "UD1K953238
*     gt_out-flg = 'S'.                                     "UD1K953238
      MODIFY gt_out INDEX $gt_out-rec TRANSPORTING msg flg.
      PERFORM append_log.
      CHECK NOT gt_out-f2 BETWEEN l_begpp AND l_endpp.      "UD1K953303
*     CONTINUE.                                             "UD1K953238
    ENDIF.
* END OF UD1K952959
...
* Compare PA2010
  IF NOT ipa2010[] IS INITIAL.
    ...
    IF NOT t_asshr[] IS INITIAL.                            "UD1K953238
...
* Give Status '1'= delete '2'= update '3'= insert
    LOOP AT t_pa2010_new.
      ...
      IF sy-subrc = 0.
        ...
        MOVE-CORRESPONDING t_pa2010_new TO t_pa2010_chg.    "UD1K953303
        t_pa2010_chg-mode = '2'.                            "UD1K953303
        IF t_pa2010_new-begda BETWEEN l_begpp AND l_endpp.  "UD1K953303
...
*           t_pa2010_chg-mode = '2'.                        "UD1K953303
        ...
        ELSE.                                               "UD1K953303
* BEGIN OF UD1K953238
*         IF t_pa2010_chg-anzhl <= 0.                       "UD1K953303
          t_pa2010_chg-anzhl = t_pa2010_chg-anzhl +
                               t_pa2010_old-anzhl.
          IF t_pa2010_chg-anzhl <= 0.
            CLEAR ipa2010.
            ...
            CONTINUE.
          ENDIF.
          APPEND t_pa2010_chg.                              "UD1K953303
        ENDIF.
* END OF UD1K953238
...
*   LOOP AT t_pa2010_old.                                   "UD1K953238
    LOOP AT t_pa2010_old WHERE lgart <> '1294'              "UD1K953238
                           AND lgart <> '1295'              "UD1K953238
                           AND begda                        "UD1K953325
                       BETWEEN l_begpp AND l_endpp.         "UD1K953325
      CLEAR ipa2010.
      MOVE-CORRESPONDING t_pa2010_old TO ipa2010.
      ipa2010-subty = ipa2010-lgart.
      ipa2010-mode = '1'.
  ...
  LOOP AT ipa2010 WHERE mode = ' '.
    ...
    IF sy-subrc = 0.
      ...
      ipa2010-anzhl = t_pa2010_chg-anzhl.                   "UD1K953238
      MODIFY ipa2010 INDEX l_tabix TRANSPORTING mode anzhl. "UD1K953238
*     MODIFY ipa2010 INDEX l_tabix TRANSPORTING mode.       "UD1K953238
    ENDIF.
  ENDLOOP.
  FREE t_pa2010_chg.
...
* END OF UD1K952791
    ELSE.
      ...
      SHIFT gt_out-f1 LEFT DELETING LEADING '0'.            "UD1K953238
      SHIFT gt_out-f4 LEFT DELETING LEADING '0'.            "UD1K953238
  ...
  LOOP AT gt_out.
    CASE gt_out-flg.
      ...
      WHEN 'W'.                                             "UD1K953238
        gt_out-icon = icon_yellow_light.                    "UD1K953238
      WHEN OTHERS.
  ...
  MESSAGE s000 WITH 'Processing Completed'.                 "UD1K953238
ENDFORM.                    " SAVE_DATA
*---------------------------------------------------------------------*
*       FORM refresh_alv                                              *
*---------------------------------------------------------------------*
...
*----------------------------------------------------------------------*
FORM display_data.
  DATA: l_anzhl    TYPE pa2010-anzhl.                       "UD1K953238
* Write Changed Records Details
  LOOP AT gt_out.
    ...
    ADD 1 TO t_cnt.
    l_anzhl = gt_out-f3.                                    "UD1K953238
*   WRITE :/(10) gt_out-f1, (10) gt_out-f2, (10) gt_out-f3, "UD1K953238
    WRITE :/(10) gt_out-f1, (10) gt_out-f2, (10) l_anzhl,   "UD1K953238
...
* Write Deleted Records Details
  LOOP AT gt_outdel.
    ...
    ADD 1 TO t_cnt.
* BEGIN OF UD1K953238
    l_anzhl = gt_outdel-f3.
*   WRITE :/(10) gt_outdel-f1, (10) gt_outdel-f2, (10) gt_outdel-f3,
    WRITE :/(10) gt_outdel-f1, (10) gt_outdel-f2, (10) l_anzhl,
* END OF UD1K953238
