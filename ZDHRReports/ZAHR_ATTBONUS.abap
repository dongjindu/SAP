...
* 10/13/2005  Shiva       UD1K917936   fixed the program to work for
*                                      the above new code 2000.
...
*GLOBAL DATA
*****************************************************************
TABLES: pa0001, pa0002, t554s, t554t.
...
FORM check_input.
  ...
  IF p_bperd-high IS INITIAL.
    MESSAGE e001 WITH 'Please endter the period end date!'.
...
FORM fill_bdcdata USING p_person LIKE it_dis.
  ...
  PERFORM bdc_field       USING 'RP50G-SUBTY'
                                '0318'.
  PERFORM bdc_dynpro      USING 'MP026700' '2000'.
  PERFORM bdc_field       USING 'BDC_CURSOR'
                              'P0267-ZUORD'.
  PERFORM bdc_field       USING 'BDC_OKCODE'
                                '=UPD'.
  PERFORM bdc_field       USING 'P0267-LGART'
                                '0318'.
  PERFORM bdc_field       USING 'Q0267-BETRG'
                                l_amount.
  PERFORM bdc_field       USING 'P0267-WAERS'
                                'USD'.
  PERFORM bdc_field       USING 'P0267-BEGDA'
                                pdate.
  PERFORM bdc_field       USING 'P0267-ZUORD'
                                g_des.
  PERFORM bdc_field       USING 'P0267-OCRSN'
                                'H001'.
  PERFORM bdc_field       USING 'P0267-PAYID'
                                p_payid.
...
FORM check_duplicate_pay.
  ...
  SELECT * INTO CORRESPONDING FIELDS OF TABLE lt_p0267
    FROM pa0267
    FOR ALL ENTRIES IN it_dis
    WHERE pernr = it_dis-pernr  AND
          lgart = '0318'        AND
  ...
  LOOP AT it_dis WHERE eligi = 'YES'.
    ...
    CLEAR: lt_p0267.
    READ TABLE lt_p0267 WITH KEY pernr = it_dis-pernr
                                 begda = p_pdate.
    IF sy-subrc EQ 0.
      it_dis-updat = 'EXIST'.
