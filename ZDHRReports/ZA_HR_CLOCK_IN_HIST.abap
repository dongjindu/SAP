*----------------------------------------------------------------------
* Program ID        : ZA_HR_CLOCK_IN_HIST
* Title             : [HR] Team Member Availablity History
* Created on        : 4/30/2008
* Created by        : ig. Moon
*----------------------------------------------------------------------
REPORT za_hr_clock_in_hist MESSAGE-ID zmco.
...
SELECT-OPTIONS s_date FOR zthr_bhisthmma-rdate OBLIGATORY.
...
SELECTION-SCREEN BEGIN OF LINE.
SELECTION-SCREEN COMMENT 1(25)  text-x05.
SELECTION-SCREEN POSITION 30.
SELECT-OPTIONS s_categ  FOR zshrclockin-categ NO INTERVALS
                                 MODIF ID op4 DEFAULT 'A'..
SELECTION-SCREEN END   OF LINE.
...
PARAMETERS p_moon MODIF ID mgr.
...
DATA  : it_row_tab TYPE TABLE OF ty_row_tab WITH HEADER LINE,
        ...
        big_gt_out     TYPE TABLE OF ty_out     WITH HEADER LINE.
...
*----------------------------------------------------------------------*
INITIALIZATION.
  sy-title = '[HR] Team Member Availablity Report(History)'.
...
*    PERFORM show_progress USING 'Gather Data...' ' 5'.
    ...
    IF p_fill EQ true OR p_moon EQ true.
    ...
    IF p_moon EQ true.
      ...
      LOOP AT gt_emp.
        ...
        WHERE badge EQ gt_emp-badge
          AND NOT ( employeenumber LIKE '1%'
                    AND employeenumber LIKE '0%' ) .

        CHECK sy-subrc EQ 0.
        UPDATE zthr_bhisthmma
        SET employeenumber = gt_emp-employeenumber
        WHERE badge EQ gt_emp-badge
          AND NOT ( employeenumber LIKE '1%'
                    AND employeenumber LIKE '0%' ) .
      ENDLOOP.
    ...
    DO $times TIMES.
      ...
      CONCATENATE $current_cnt '/' $total_cnt INTO $text.
      CONDENSE $text.
...
FORM top_of_page.
  ...
  l_text = 'Team Member Availablity History'.
  PERFORM set_header_line USING :
          'P' 'H' '' l_text ''.

  PERFORM set_header_line USING :
          'D' 'S' 'Date' s_date-low      s_date-high.
...
FORM change_fieldcat USING    pt_fieldcat TYPE slis_t_fieldcat_alv.
  ...
  LOOP AT pt_fieldcat INTO gs_fieldcat.
    ...
    IF gs_fieldcat-fieldname EQ 'ZCLKAVA'.
      gs_fieldcat-no_out = true.
    ENDIF.
...
FORM refine_row_itab.
  ...
  LOOP AT it_row_tab.
    ...
    IF NOT gt_out-zdooridi IS INITIAL OR
       NOT gt_out-zdoorido IS INITIAL.
      APPEND gt_out.
    ENDIF.
  ENDLOOP.
...
*  endif.
 ...
 BINARY SEARCH.
  ...
  LOOP AT itab.
    ...
    CHECK $cnt LE 2.
*    read table it_row_tab with key pernr = itab-pernr binary search.
...
FORM modi_itab USING check_date.
  ...
  LOOP AT itab.
    ...
    IF sy-subrc EQ 0.
      ...
      MODIFY itab INDEX $ix TRANSPORTING pernr inout
            door_desc employeenumber.
    ELSE.
  ...
  SORT  itab BY employeenumber $str .
  SORT $itab BY employeenumber $str .
  ...
  LOOP AT itab.
    ...
    CHECK itab-rdate EQ check_date.

    READ TABLE it_pernr WITH KEY pernr = itab-employeenumber
                                    BINARY SEARCH.
    IF sy-subrc EQ 0.
      ...
      IF sy-subrc EQ 0.
        ...
        PERFORM check_time USING check_date
                                 it_ws-anzsh
                                 itab-rdate
                                 it_row_tab-zclkout " dummy
                        CHANGING $clr
                                 $end_time .

        $gap = itab-rtime - $end_time.
...
* count for Unavailable
    ...
    IF NOT it_row_tab-categ IN s_categ.
      DELETE it_row_tab INDEX $ix.
    ELSE.
...
FORM modify_screen.
  LOOP AT SCREEN.
    IF screen-group1 = 'MGR'.
      IF sy-uname NE '103569'.
        CLEAR p_moon.
        screen-input = 0.
        screen-invisible = 1.
        MODIFY SCREEN.
...
FORM really?.
  ...
  PERFORM pop_up USING
      'Your range exceeds 1 month! and this will be taken long time'
