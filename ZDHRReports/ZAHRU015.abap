...
*  08/17/2012  Valerian  UD1K954795  Change sequence of IT Creation
...
REPORT zahru015 MESSAGE-ID zmfi NO STANDARD PAGE HEADING.
...
* Compare PA2001
  IF NOT ipa2001[] IS INITIAL.
    SELECT * INTO CORRESPONDING FIELDS OF TABLE t_pa2001_old
      FROM pa2001
      FOR ALL ENTRIES IN ipa2001
     WHERE pernr = ipa2001-pernr
  ...
  LOOP AT ipa2001 WHERE mode = ' '.
    ...
    IF sy-subrc = 0.
      ...
      MODIFY ipa2001 INDEX l_tabix TRANSPORTING mode.
    ENDIF.
  ENDLOOP.
  FREE t_pa2001_chg.
...
* If ipa2001-rec initial, it is deleted record, append record
...
* Compare PA2002
  IF NOT ipa2002[] IS INITIAL.
    SELECT * INTO CORRESPONDING FIELDS OF TABLE t_pa2002_old
      FROM pa2002
...
* Give Status '1'= delete '2'= update '3'= insert
  ...
  LOOP AT ipa2002 WHERE mode = ' '.
    ...
    IF sy-subrc = 0.
      ...
      MODIFY ipa2002 INDEX l_tabix TRANSPORTING mode.
    ENDIF.
  ENDLOOP.
  FREE t_pa2002_chg.
...
* If ipa2002-rec initial, it is deleted record, append record
