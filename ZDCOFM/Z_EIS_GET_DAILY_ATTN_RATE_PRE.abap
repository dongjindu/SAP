FUNCTION z_eis_get_daily_attn_rate_pre.
*"----------------------------------------------------------------------
*"*"Local interface:
*"  IMPORTING
...
*"----------------------------------------------------------------------
  ...
  ELSE.
    GET PARAMETER ID 'ZTTP' FIELD prev_time.

    IF NOT prev_time IS INITIAL.
      diff_time = curr_time - prev_time.
      IF diff_time < 600.
        GET PARAMETER ID 'ZTCP' FIELD saved_date.
        IF saved_date EQ check_date.
          GET PARAMETER ID 'ZTHP' FIELD rate_hourly.
          GET PARAMETER ID 'ZTSP' FIELD rate_salary.
  ...
  SET PARAMETER ID 'ZTTP' FIELD curr_time.
  SET PARAMETER ID 'ZTCP' FIELD check_date.
...
* }
  ...
  SET PARAMETER ID 'ZTHP' FIELD rate_hourly.
  SET PARAMETER ID 'ZTSP' FIELD rate_salary.
