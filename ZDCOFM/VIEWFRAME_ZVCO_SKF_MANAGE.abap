*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVCO_SKF_MANAGE
*   generation date: 03/14/2004 at 23:46:22 by user EHJUNG
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVCO_SKF_MANAGE     .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVCO_SKF_MANAGE'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVCO_SKF_MANAGE_EXTRACT
TOTAL = ZVCO_SKF_MANAGE_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVCO_SKF_MANAGE'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVCO_SKF_MANAGE-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVCO_SKF_MANAGE_EXTRACT
TOTAL = ZVCO_SKF_MANAGE_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVCO_SKF_MANAGE-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVCO_SKF_MANAGE'
...
TOTAL = ZVCO_SKF_MANAGE_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVCO_SKF_MANAGE-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVCO_SKF_MANAGE-UPD_FLAG NE SPACE.
...
CALL FUNCTION 'VIEWPROC_ZVCO_SKF_MANAGE'
                    ...
                    IMPORTING
                              UPDATE_REQUIRED =
STATUS_ZVCO_SKF_MANAGE-UPD_FLAG
                    TABLES
                        EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVCO_SKF_MANAGE_EXTRACT
TOTAL = ZVCO_SKF_MANAGE_TOTAL
