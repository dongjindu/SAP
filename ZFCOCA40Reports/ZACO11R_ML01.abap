...
************************************************************************
report zaco11r_ml01 no standard page heading message-id zmco.
...
*
  ...
  data: l_date_Fr like sy-datum,
        l_date_to like sy-datum.

  data: begin of lt_remove OCCURS 0,
        matnr like mara-matnr,
        end of lt_remove.

  CONCATENATE P_BDATJ P_POPER+1(2) '01' into l_date_fr.
  CONCATENATE P_BDATJ P_POPER+1(2) '30' into l_date_to.
...
** Added on 01/14/14
  select distinct matnr into corresponding fields of table lt_remove
        from mseg
           where mjahr =  p_bdatj
           and zbudat >= l_date_fr
           and zbudat <= l_date_to
           and matnr in s_matnr
           and matnr <> ' '.

*  select distinct matnr into corresponding fields of table lt_remove
*        from ZMKPF_MSEG_V as z
*"        mkpf inner join mseg
*"           on mkpf~mjahr = mseg~mjahr
*"           and mkpf~mblnr = mseg~mblnr
*           where z~mjahr =  p_bdatj
*           and z~budat >= l_date_fr
*           and z~budat <= l_date_to
*           and z~matnr in s_matnr
*           and z~matnr <> ' '.
  sort lt_remove by matnr.
** End on 01/14/14
...
** Added on 01/14/14
    loop at t_mats.
      READ TABLE lt_remove with key matnr = t_mats-matnr
                  BINARY SEARCH.
      if sy-subrc <> 0.
        delete t_mats.
      endif.
