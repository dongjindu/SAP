*&----------------------------------------------------------------------
*& Development ID :
*& Program ID     : ZIIT_BPML_TCODE
*& Program Name   : APM T-code List I/F
...
REPORT  ziit_bpml_tcode MESSAGE-ID zmpp.
...
FORM modify_data .
  ...
  DELETE it_bpmla WHERE tcode IS INITIAL.

  LOOP AT it_bpmla.
    CLEAR : wa_bpml.

    it_data-yyyymm     = p_month.
    it_data-comp_name = 'HMMA'.
    ...
    it_data-module_nm = wa_bpml-l2.
    it_data-tcode     = it_bpmla-tcode.
    it_data-tcode_nm  = wa_bpml-ttext.
    it_data-menu_name1  = wa_bpml-l3.
    it_data-menu_name2  = wa_bpml-l4t.
    it_data-menu_name3  = wa_bpml-l5t.
...
FORM pro_send .
  ...
  CALL FUNCTION 'ZFIT_OB_BPML01' DESTINATION v_dest
    IMPORTING
      e_return              = e_return
    TABLES
      t_data                = it_data
...
FORM save_log  USING   p_type p_msg1 p_msg2.
  ...
  DATA : it_save LIKE ztit_apm_tcode OCCURS 0 WITH HEADER LINE.
  ...
  INSERT ztit_apm_tcode FROM TABLE it_save.
...
*         lv_time  TYPE sy-uzeit.
*
*  CLEAR : it_bpml[], it_bpml.
*
*  CONCATENATE p_month '01' INTO lv_sdate.
*  PERFORM cal_month_lastdate USING p_month  lv_edate.
*
*  SELECT * INTO CORRESPONDING FIELDS OF TABLE it_bpmlh
*  FROM ztitbpmlh
*  WHERE apm = 'X'.
*
...
*
*  LOOP AT it_bpmlh.
*    MOVE-CORRESPONDING it_bpmlh TO it_bpml.
*    APPEND it_bpml.
*  ENDLOOP.
*
*  SELECT * INTO TABLE it_package
