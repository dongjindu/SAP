*&----------------------------------------------------------------------
*& Development ID :
*& Program ID     : ZIIT_BPML_UNUSED
*& Program Name   : APM Unused List I/F
...
REPORT  ziit_bpml_unused MESSAGE-ID zmpp.
...
DATA : it_bpml   LIKE ztitbpma OCCURS 0 WITH HEADER LINE,
       ...
       it_data   LIKE zsit_apm_unused OCCURS 0 WITH HEADER LINE.
...
*-    when t-code was deleted during processing
    ...
    IF it_bpml-cstatus >= '20' AND it_bpml-cstatus <= '30'.
...
FORM pro_send .
  ...
  CALL FUNCTION 'ZFIT_OB_BPML03' DESTINATION v_dest
    IMPORTING
      e_return              = e_return
    TABLES
      t_data                = it_data
...
*     i_structure_name   = p_name
  ...
  LOOP AT pt_fieldcat.
    CASE pt_fieldcat-fieldname.
      ...
      WHEN 'UREASON'.
        pt_fieldcat-seltext_m    = 'Unused Reason'.
      WHEN 'MENU_NAME1'.
...
FORM save_log  USING   p_type p_msg1 p_msg2.
  ...
  DATA : it_save LIKE ztit_apm_unused OCCURS 0 WITH HEADER LINE.
  DATA : l_zseq(10) TYPE n.

  SELECT zseq INTO l_zseq
    FROM ztit_apm_unused
      UP TO 1 ROWS
  ...
  INSERT ztit_apm_unused FROM TABLE it_save.
