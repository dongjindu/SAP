FUNCTION z_bw_get_data_overstock.
*"----------------------------------------------------------------------
*"*"Local interface:
*"  IMPORTING
...
*"      E_T_DATA STRUCTURE  ZBWST_OVS OPTIONAL
*"  EXCEPTIONS
*"      NO_MORE_DATA
...
* Select ranges
  ...
  DATA: BEGIN OF lt_remark OCCURS 0,
          matnr   LIKE ztmm_ovs_eis-matnr,
          remark  LIKE ztmm_ovs_eis-remark,
          to_date LIKE ztmm_ovs_eis-to_date,
          to_time LIKE ztmm_ovs_eis-to_time,
        END OF lt_remark.
  ...
  DATA : l_zbwst_ovs LIKE zbwst_ovs OCCURS 0 WITH HEADER LINE. "Overstock
...
*      APPEND r_mtart.
      ...
      SELECT (s_s_if-t_fields) FROM ztmm_overstock
...
*   named E_T_'Name of extract structure'.
    ...
    LOOP AT e_t_data INTO l_zbwst_ovs.
      l_tabix = sy-tabix.

      READ TABLE lt_remark WITH KEY matnr = l_zbwst_ovs-matnr
                             BINARY SEARCH.
      IF sy-subrc EQ 0.
        l_zbwst_ovs-remark = lt_remark-remark.
      ENDIF.

      MODIFY e_t_data FROM l_zbwst_ovs INDEX l_tabix.
