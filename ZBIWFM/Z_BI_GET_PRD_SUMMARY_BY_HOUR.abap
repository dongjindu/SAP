FUNCTION z_bi_get_prd_summary_by_hour.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     VALUE(CHECK_DATE) TYPE  SY-DATUM DEFAULT SY-DATUM
*"  TABLES
*"      ZPRDQTYHOUR STRUCTURE  ZPRDQTYHOURBI
*"----------------------------------------------------------------------
  ...
  SELECT * INTO TABLE lt_ztppvr
    ...
    WHERE k04pdat EQ check_date
      AND p_status IN ('B01','P02','P19','P20','P29','P37',
       'P39','T01','T11','T24','T25','T26','T27','T28','V03',
       'V04','V05','V06','V07')
       AND zresult  = 'S'
  ...
  DO 24 TIMES.
    LOOP AT lt_ztppvr INTO ls_ztppvr.
      ...
      CASE ls_ztppvr-p_status.
        ...
        WHEN 'P19' OR 'P20'.
          f_name = 'ZPRDQTYHOUR-TC_QTY'.
        WHEN 'P29'.
          f_name = 'ZPRDQTYHOUR-PO_QTY'.
        WHEN 'P37'.
          f_name = 'ZPRDQTYHOUR-PBS_I_QTY'.
        WHEN 'P39'.
          f_name = 'ZPRDQTYHOUR-PBS_O_QTY'.
        WHEN 'T01'.
          f_name = 'ZPRDQTYHOUR-TRIM_IN_QTY'.
        WHEN 'T11'.
          f_name = 'ZPRDQTYHOUR-L_OFF_QTY'.
        WHEN 'T24' OR 'T25' OR 'T26'.
          f_name = 'ZPRDQTYHOUR-S_OFF_QTY'.
        WHEN 'T27' OR 'T28'.
          f_name = 'ZPRDQTYHOUR-CG_QTY'.
        WHEN 'V03'.
          f_name = 'ZPRDQTYHOUR-VPC_O_QTY'.
        WHEN 'V04' OR 'V06'.
          f_name = 'ZPRDQTYHOUR-SHIP_IN_QTY'.
        WHEN 'V05' OR 'V07'.
          f_name = 'ZPRDQTYHOUR-SHIP_OUT_QTY'.
      ENDCASE.
  ...
  ENDLOOP.
*
*  LOOP AT gt_zprdqtyhour_l.
*    COLLECT gt_zprdqtyhour_l INTO zprdqtyhour3.
*    CLEAR gt_zprdqtyhour_l.
*  ENDLOOP.

  DATA : $zprdqtyhour LIKE zprdqtyhour OCCURS 0 WITH HEADER LINE.
  ...
  LOOP AT $zprdqtyhour.
    CASE $zprdqtyhour-worktime.
      WHEN 'WORKTIME_25'.
        READ TABLE zprdqtyhour WITH KEY worktime = 'WORKTIME_09'.
        IF sy-subrc EQ 0.
          $ix = sy-tabix.
          INSERT $zprdqtyhour INTO zprdqtyhour INDEX $ix.
        ENDIF.
      WHEN 'WORKTIME_26'.
        READ TABLE zprdqtyhour WITH KEY worktime = 'WORKTIME_17'.
        IF sy-subrc EQ 0.
          $ix = sy-tabix.
          INSERT $zprdqtyhour INTO zprdqtyhour INDEX $ix.
        ENDIF.
      WHEN 'WORKTIME_27'.
        APPEND $zprdqtyhour TO zprdqtyhour.
    ENDCASE.
    AT LAST.
      SUM.
      $zprdqtyhour-worktime = 'WORKTIME_00'.
      INSERT $zprdqtyhour INTO zprdqtyhour INDEX 1.
    ENDAT.
  ENDLOOP.
...
*... Begin{ 3Shift, Commented original Source 3Shift 06/25/2012
*  __cls  gt_zprdqtyhour.
*
*  CLEAR :  p_status,
*           qty_01,         qty_02,         qty_03,
...
*
*  LOOP AT $zprdqtyhour.
*    IF $zprdqtyhour-worktime EQ 'WORKTIME_25'.
*      READ TABLE zprdqtyhour WITH KEY worktime = 'WORKTIME_13'.
*      IF sy-subrc EQ 0.
*        $ix = sy-tabix.
*        INSERT $zprdqtyhour INTO zprdqtyhour INDEX $ix.
*      ENDIF.
