...
*"  TABLES
*"      T_DATA STRUCTURE  ZSPP_ALC_BLOCK
*"----------------------------------------------------------------------
  ...
  IF sy-subrc = 0.
    ...
    ELSE.
      MOVE: 'E' TO e_result,
            'Update failed for ALC Block' TO e_msg.
      ROLLBACK WORK.
    ENDIF.
  ELSE.
    MOVE:'E' TO e_result,
         'Update failed for ALC Block history' TO e_msg.
    ROLLBACK WORK.
  ENDIF.
ENDFUNCTION.
