FUNCTION ZZPP_PPC1PR_STEP2_EXE.
*"----------------------------------------------------------------------
*"*"Local interface:
*"  IMPORTING
*"     VALUE(IF_DATE_RANGE) TYPE  PPCPR_POSTDATE_RANGE
*"     VALUE(IF_USER_RANGE) TYPE  PPCPR_USERNAME_RANGE OPTIONAL
*"     REFERENCE(IF_LIMIT) TYPE  PPC_COUNT DEFAULT 500
*"     REFERENCE(IF_PROTOCOL_SHOW) TYPE  PPC_PROTO DEFAULT CHARX
*"     REFERENCE(IF_COMPLMODE) TYPE  AS4FLAG DEFAULT 'D'
*"     VALUE(GROUP_NAME) TYPE  RZLLI_APCL
*"     REFERENCE(I_FLAG) OPTIONAL
*"  EXPORTING
*"     REFERENCE(EF_LOGNUMBER) TYPE  BALOGNR
*"  EXCEPTIONS
*"      PROTOCOL_ERROR
*"      ENQUEUE_ERROR
*"      NOTHING_SELECTED
...
*    MOVE <fs_step2_agg>-indexlist TO LT_INDEXLIST.
...
*      check if this one has been rollbacked.
       read table it_rollback_job with key jobname = ls_ppc_del-jobname.
          if sy-subrc ne 0.
* WHY DO WE DELETE THE TABLE PPC_STEP2 HERE??? THIS SHOULD happen in the
* LUW of the ZZPPC1PR_STEP2_SINGLE_EXE => I'm removing this step here !!
...
* save the second time start message
  ...
  if if_protocol_show = charx.
    perform protocol_show using lf_log_handle.
  endif.
