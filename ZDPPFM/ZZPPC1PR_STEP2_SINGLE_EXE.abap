FUNCTION ZZPPC1PR_STEP2_SINGLE_EXE.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"      IT_LOG_MSG TYPE  ZSPP_LOG_MSG_T OPTIONAL
*"  EXCEPTIONS
*"      PROTOCOL_ERROR
*"      PROFILE_ERROR
*"      UPDATE_ERROR
...
* --> check whether we can continue (mm doc limitation)
  DESCRIBE TABLE it_complist LINES lf_compcount.
*{ 09/06/11 Paul change : gc_mmdoclimit is change gf_mmdoxlimit
*           When changing ECC
...
* --> delete the items from PPC_STEP2 table
  perform delete_processed_items
      using it_indexlist[].
