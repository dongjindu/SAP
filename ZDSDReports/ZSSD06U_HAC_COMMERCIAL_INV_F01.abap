...
*  SELECT SINGLE BOLNR INTO L_BOLNR
*         FROM LIKP
*        WHERE VBELN EQ NAST-OBJKY.
*
*  IT_INV_H-Z081 = L_BOLNR.

* 1
*                 12345678901234567890123456789012345 "35
  it_inv_h-vbeln = vbrk-vbeln.
...
* 8
* YYYYMMDD + 01 (20040217 + Serial number) / YYMMDD
  PERFORM MAKE_SEQ USING W_CNT. ""WHEN FILE DOWNLOADED
  PERFORM MAKE_EDATE USING W_EDATE.
**  CONCATENATE 'B06' SY-DATUM+2(6) W_CNT
**              INTO IT_INV_H-Z081.
...
* 11
  IT_INV_H-Z111 = 'TO THE ORDER OF HYUNDAI AUTO CANADA'.

* 12
  IT_INV_H-Z121 = 'HAC  CANADA  MADE IN U.S.A.'. "FIXED IN FORM
    ...
    IF SY-SUBRC = 0.
      IF VBAK-BSTNK+14(2) = 'A1' OR
        ...
        CONCATENATE IT_LIST-MODEL '(MICA)'
                    INTO IT_LIST-ARKTX SEPARATED BY ' '.
      ELSE.
...
FORM CALL_SM USING RC.
  CALL FUNCTION 'SSF_FUNCTION_MODULE_NAME'
       EXPORTING
            FORMNAME           = 'ZSSD06_HAC_COMMERCIAL_INVOICE'
       IMPORTING
