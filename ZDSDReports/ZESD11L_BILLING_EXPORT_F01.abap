...
DATA : BEGIN OF it_vbfa OCCURS 0,
  ...
  r_kunag-sign = 'I'.
  r_kunag-option = 'BT'.
  CONCATENATE p_nation 'AA' INTO r_kunag-low.
  CONCATENATE p_nation 'ZZ' INTO r_kunag-high.
  APPEND r_kunag.

  SELECT k~lifex k~vbeln k~route
         p~matnr p~arktx p~lfimg
         p~vrkme p~vbelv k~wadat_ist
          INTO (it_list-lifex, it_list-vbeln, it_list-route,
                it_list-matnr, it_list-arktx, it_list-lfimg,
                it_list-vrkme, it_list-vbelv, it_list-wadat_ist)
          FROM lips AS p INNER JOIN likp AS k
          ON p~vbeln EQ k~vbeln
       WHERE k~lddat IN s_fkdat
       AND   k~kunnr IN r_kunag
*       AND   k~wadat_ist EQ '00000000' "NOT POST G/I "08.28.2014
       AND   k~lifex NE ''.
    APPEND it_list. CLEAR it_list.
  ENDSELECT.

*-< Victor 08.28.2014 delete already billed delivery document
  IF it_list[] IS NOT INITIAL.
    SELECT  vbelv INTO TABLE it_vbfa
    FROM vbfa
      FOR ALL ENTRIES IN it_list
    WHERE vbelv = it_list-vbeln
      AND vbtyp_n = 'M'.
    SORT it_vbfa BY vbelv.
  ENDIF.

  LOOP AT it_list WHERE wadat_ist <> '00000000'.
    READ TABLE it_vbfa WITH KEY vbelv = it_list-vbeln
                                BINARY SEARCH.
    IF sy-subrc = 0.
      DELETE it_list.
    ELSE.
...
FORM check_input_data .
  IF p_nation = 'B28'.
    MESSAGE s000 WITH 'You can Only input Export Nation code'.
    STOP.
  ENDIF.
