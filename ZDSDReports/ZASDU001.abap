...
* Tables
TABLES : csks, vbrk, *vbrp, vbuk, *a004, vbfa, likp, *vbfa, vbco6, equz,
 ausp,cabn.
...
DATA: BEGIN OF it_file_mat OCCURS 0,
         vbeln(10),
         matnr(18),
         kbetr(15),
      END OF it_file_mat.
...
DATA : BEGIN OF it_post_matnr  OCCURS 0.
        INCLUDE STRUCTURE  zssdu001.
DATA    kposn TYPE kposn.
DATA    flag.
DATA : END OF   it_post_matnr.
...
PARAMETERS p_rb1 RADIOBUTTON GROUP radb.
PARAMETERS p_rb2 RADIOBUTTON GROUP radb.
PARAMETERS p_fkart LIKE rv60a-fkart NO-DISPLAY.
SELECTION-SCREEN END   OF BLOCK b9s.
SELECTION-SCREEN END OF BLOCK bl1.

SELECTION-SCREEN BEGIN OF BLOCK b9 WITH FRAME TITLE text-0x1.
PARAMETERS p_rd1 RADIOBUTTON GROUP radx USER-COMMAND ucom.
PARAMETERS p_rd2 RADIOBUTTON GROUP radx.
PARAMETERS p_rd3 NO-DISPLAY.
...
*&----------------------------------------------------------------------
INITIALIZATION.
  message w000 with 'Please use new program for CR/DR memo.'.
...
FORM upload_file USING filename.
  ...
  __cls : it_file_vin, it_file_mat, it_file.
    ...
    ELSE.
      ...
      CASE true.
        ...
        WHEN p_rd2.
          LOOP AT it_itab.
            MOVE : it_itab-col TO v_index.
            ASSIGN COMPONENT v_index OF STRUCTURE it_file_mat TO <fs>.
            MOVE : it_itab-value TO <fs>.
            AT END OF row.
              APPEND it_file_mat.
            ENDAT.
  ...
  CASE true.
    ...
    WHEN p_rd2.
      __cls it_row_tab.
      LOOP AT it_file_mat.
        MOVE-CORRESPONDING it_file_mat TO it_row_tab.
        it_row_tab-fkdat = p_date.
        APPEND it_row_tab.
      ENDLOOP.
...
* Sorting
  ...
  CASE true.
    WHEN p_rd1.
      ...
      SORT it_post_matnr BY vbeln vin.
      DELETE ADJACENT DUPLICATES FROM it_post_matnr
        COMPARING vbeln vin.

      SORT it_post BY vbeln vin.
      DELETE ADJACENT DUPLICATES FROM it_post
        COMPARING vbeln vin.
    ...
    WHEN p_rd2.
      ...
      SORT it_post_matnr BY vbeln matnr.
      DELETE ADJACENT DUPLICATES FROM it_post_matnr
        COMPARING vbeln matnr.

      SORT it_post BY vbeln matnr.
      DELETE ADJACENT DUPLICATES FROM it_post
        COMPARING vbeln matnr.

    WHEN p_rd3.
      DELETE ADJACENT DUPLICATES FROM it_post
        COMPARING vbeln vin matnr.

      SORT it_post_matnr BY vbeln  matnr vin .
      DELETE ADJACENT DUPLICATES FROM it_post_matnr
        COMPARING vbeln matnr vin .

      SORT it_post BY vbeln matnr vin.
      DELETE ADJACENT DUPLICATES FROM it_post
        COMPARING vbeln matnr vin.
...
* {
  DATA: beleg_typ LIKE vbuk-vbtyp.       "Belegtyp
  DATA: BEGIN OF vbfa_tab OCCURS 0.      "Belegfluss
...
* }
  ...
  LOOP AT it_post.
    IF it_post-vin NE space.
      l_vin = true.
    ENDIF.
    IF it_post-matnr NE space.
      l_mat = true.
    ENDIF.
  ENDLOOP.

  IF l_vin EQ true AND l_mat NE true.
    l_rd3 = false.
    l_rd2 = false.
    l_rd1 = true.
  ENDIF.

  IF l_vin NE true AND l_mat EQ true.
    l_rd3 = false.
    l_rd2 = true.
    l_rd1 = false.
  ENDIF.

  IF l_vin EQ true AND l_mat EQ true.
    l_rd3 = true.
    l_rd2 = false.
    l_rd1 = false.
  ENDIF.

  IF l_vin NE true AND l_mat NE true.
    MESSAGE s000 WITH 'Please check your data.'.
    EXIT.
  ENDIF.
  ...
  LOOP AT it_post.
    ...
    LOOP AT it_kposn.
      ...
      IF sy-subrc EQ 0.
        ...
        IF sy-subrc EQ 0.
        ELSE.
          DELETE it_kposn INDEX $ix. CONTINUE.
        ENDIF.
      ELSE.
        DELETE it_kposn INDEX $ix. CONTINUE.
      ENDIF.

      CLEAR : it_post_matnr, $subrc.

      IF l_rd1 EQ true OR l_rd3 EQ true.

        CLEAR : vbco6, beleg_typ, vbfa_tab, vbfa_tab[].

        vbco6-mandt	= sy-mandt.
        vbco6-vbeln	= *vbrp-vbeln.
        vbco6-posnr	= *vbrp-posnr.
        ...
        IF sy-subrc EQ 0.
          SORT vbfa_tab BY erdat erzet DESCENDING.
          READ TABLE vbfa_tab WITH KEY " vbtyp_n = 'M'
                                       vbtyp_v = 'J'.
          IF sy-subrc EQ 0.
            __cls it_vin.
            SELECT SINGLE berot INTO it_vin-vin
                              FROM likp
                              WHERE vbeln EQ vbfa_tab-vbelv.
            IF sy-subrc EQ 0.
              IF it_vin-vin NE space.
                APPEND it_vin.
              ELSE.

                SELECT SINGLE atwrt INTO it_vin-vin
                                  FROM  ausp
                                  WHERE objek EQ vbfa_tab-vbelv
                                    AND atinn EQ $atinn.
                IF sy-subrc EQ 0 AND it_vin-vin NE space .
                  APPEND it_vin.
                ENDIF.
              ENDIF.
            ENDIF.
      ...
      CASE true.
        WHEN l_rd1.
          ...
          LOOP AT it_vin.
            READ TABLE it_post_matnr WITH KEY vbeln = it_post-vbeln
                                              vin = it_vin-vin.
            IF sy-subrc EQ 0.
              it_post_matnr-flag = true.
              it_post_matnr-kposn = it_kposn-kposn.
              MODIFY it_post_matnr INDEX sy-tabix TRANSPORTING
              flag kposn.
              $subrc = sy-subrc.
              EXIT.
            ENDIF.
        ...
        WHEN l_rd2.
          ...
          IF sy-subrc EQ 0.
            ...
            MODIFY it_post_matnr INDEX sy-tabix TRANSPORTING
            flag kposn.
            MODIFY it_post_matnr INDEX sy-tabix TRANSPORTING flag.
          ENDIF.
          $subrc = sy-subrc.
        WHEN l_rd3.
          READ TABLE it_post_matnr WITH KEY vbeln = it_post-vbeln
    ...
    SORT  it_post_matnr BY vbeln kposn.
    ...
    LOOP AT it_kposn.
      ...
      READ TABLE it_post_matnr WITH KEY kposn = it_post-vbeln
                                        kposn = it_kposn-kposn
                                        BINARY SEARCH.
      CHECK sy-subrc EQ 0.

      WRITE it_post_matnr-kbetr TO $rate.
    ...
    IF sy-subrc EQ 0.
      ...
      gt_out-msg = msg.
      MODIFY gt_out TRANSPORTING err msg
                       WHERE vbeln EQ it_post-vbeln
                                      AND chk EQ true .
    ENDIF.
  ...
  LOOP AT it_post_matnr WHERE flag EQ false.
    $ix = sy-tabix.

    CASE true.
      WHEN l_rd1.
        READ TABLE gt_out WITH KEY vbeln = it_post_matnr-vbeln
                                   vin   = it_post_matnr-vin.
        IF sy-subrc EQ 0.
          gt_out-err = true.
          gt_out-msg = 'No line item data to proceed!'.
          MODIFY gt_out TRANSPORTING err msg
                        WHERE vbeln = it_post_matnr-vbeln
                          AND vin   = it_post_matnr-vin
                          AND err EQ 'N'.
      ...
      WHEN l_rd2.
        READ TABLE gt_out WITH KEY vbeln = it_post_matnr-vbeln
                                   matnr = it_post_matnr-matnr.
        IF sy-subrc EQ 0.
          gt_out-err = true.
          gt_out-msg = 'No line item data to proceed!'.
          MODIFY gt_out TRANSPORTING err msg
                        WHERE vbeln = it_post_matnr-vbeln
                          AND matnr = it_post_matnr-matnr
                          AND err EQ 'N'.
        ENDIF.
    ENDCASE.
...
FORM really?.
  ...
  PERFORM pop_up USING
      'This will create billing docs for each selected line.'
...
FORM create_field_category USING mode_edit.
  ...
  IF p_rd1 EQ true OR p_rd3 EQ true.
    __catalog :
    ' '  'VIN'               'VIN#'             17 'CHAR' ''.
  ENDIF.
  IF p_rd2 EQ true OR p_rd3 EQ true.
    __catalog :
...
FORM build_cell_attr.
  ...
  LOOP AT gt_fcat INTO gs_fcat.
    ...
    IF ls_celltab-fieldname EQ 'KBETR' OR
          ls_celltab-fieldname EQ 'MATNR' OR
