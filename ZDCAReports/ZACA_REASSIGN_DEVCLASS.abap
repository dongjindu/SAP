...
FORM read_process.
  ...
  DATA: lt_tadir LIKE TABLE OF tadir WITH HEADER LINE,
        lt_tdevc LIKE TABLE OF tdevc WITH HEADER LINE.

  DATA: l_cn TYPE i.

  SELECT * INTO TABLE lt_tdevc
    FROM tdevc
    FOR ALL ENTRIES IN it_excl
    WHERE devclass = it_excl-devclass.

  SELECT * INTO TABLE lt_tadir
    FROM tadir
    FOR ALL ENTRIES IN it_excl
    WHERE obj_name = it_excl-obj_name.

  SORT lt_tadir BY obj_name pgmid object.

  CLEAR: it_data.
  REFRESH: it_data.
  LOOP AT it_excl.

    LOOP AT lt_tadir WHERE obj_name = it_excl-obj_name.
      MOVE-CORRESPONDING it_excl TO it_data.
      IF it_excl-devclass = lt_tadir-devclass.
        it_data-result = 'W'.
        it_data-message = 'Same development class'.

      ELSE.
        READ TABLE lt_tdevc WITH KEY devclass = it_excl-devclass.
        IF sy-subrc = 0.
          CASE lt_tadir-object.
            WHEN 'TRAN'.
              it_data-type_desc = 'T-code'.

              UPDATE tadir SET: devclass = it_excl-devclass
              WHERE obj_name = lt_tadir-obj_name
                AND object = lt_tadir-object.
*            AND DEVCLASS = LT_TADIR-DEVCLASS.
              IF sy-subrc <> 0.
                it_data-result = 'E'.
            ...
            WHEN 'PROG'.
              it_data-type_desc = 'Program'.
            WHEN 'TABL'.
              it_data-type_desc = 'Table'.
            WHEN 'TABU'.
              it_data-type_desc = 'Table contents'.
            WHEN 'VIEW'.
              it_data-type_desc = 'VIEW'.
          ENDCASE.

        ELSE.
          it_data-result = 'E'.
          it_data-message = 'Development class not exist'.
        ENDIF.
      ENDIF.
      APPEND it_data.
      CLEAR: it_data.
  ...
  LOOP AT it_excl.
    ...
    LOOP AT lt_tadir WHERE obj_name = it_excl-obj_name.
      CASE lt_tadir-object.
        WHEN 'TRAN'.
          it_data-type_desc = 'T-code'.
          it_data-DEVCLASS = lt_tadir-devclass.
          it_data-result = 'X'.
        WHEN 'PROG'.
          it_data-type_desc = 'Program'.
        WHEN 'TABL'.
