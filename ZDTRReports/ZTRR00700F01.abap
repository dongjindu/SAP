...
FORM SELECT_DATA.
  ...
  ICLEAR : PREV_ACT, PREV_PLAN, PLAN.
...
FORM MAKE_TREE_DATA .
  ...
  PERFORM HIERARCHY_COLLECT TABLES PREV_PLAN.
  PERFORM HIERARCHY_COLLECT TABLES PREV_ACT.
...
FORM DATA_PRINT .
  ...
  ICLEAR TITLE.
  APPEND 'Prev.Plan'       TO TITLE.
  APPEND 'Prev.Actual'     TO TITLE.
  APPEND 'Prev.Difference' TO TITLE.
  APPEND 'Curr.Plan'       TO TITLE.

  DATA : LT_DATE         LIKE  SY-DATUM OCCURS 0 WITH HEADER LINE,
         LT_BEGINNING    LIKE  FDSR OCCURS 0 WITH HEADER LINE,
         LT_PLAN         LIKE  FDSR OCCURS 0 WITH HEADER LINE,
         LT_ENDING       LIKE  FDSR OCCURS 0 WITH HEADER LINE.

  APPEND '00000001' TO LT_DATE.
  APPEND '00000002' TO LT_DATE.
  APPEND '00000003' TO LT_DATE.
  APPEND '00000004' TO LT_DATE.

  LOOP AT PREV_DATE.
    LOOP AT PREV_BEGIN WHERE DATUM = PREV_DATE.
      LT_BEGINNING = PREV_BEGIN.
      LT_BEGINNING-DATUM = '00000001'.
      APPEND LT_BEGINNING.
      LT_BEGINNING-DATUM = '00000002'.
      IAPPEND LT_BEGINNING.
      LT_BEGINNING-DATUM = '00000003'.
      IAPPEND LT_BEGINNING.
    ENDLOOP.

    LOOP AT PREV_END WHERE DATUM = PREV_DATE.
      LT_ENDING = PREV_END.
      LT_ENDING-DATUM = '00000001'.
      APPEND LT_ENDING.
      LT_ENDING-DATUM = '00000002'.
      IAPPEND LT_ENDING.
      LT_ENDING-DATUM = '00000003'.
      IAPPEND LT_ENDING.
    ENDLOOP.

    LOOP AT PREV_PLAN WHERE DATUM = PREV_DATE.
      LT_PLAN = PREV_PLAN.
    ...
    LOOP AT PREV_ACT WHERE DATUM = PREV_DATE.
      LT_PLAN = PREV_ACT.
  ...
  LOOP AT DATE.
    LOOP AT BEGINNING WHERE DATUM = DATE.
      LT_BEGINNING = BEGINNING.
      LT_BEGINNING-DATUM = '00000004'.
      IAPPEND LT_BEGINNING.
    ENDLOOP.

    LOOP AT ENDING WHERE DATUM = DATE.
      LT_ENDING = ENDING.
      LT_ENDING-DATUM = '00000004'.
      IAPPEND LT_ENDING.
    ENDLOOP.

    LOOP AT PLAN WHERE DATUM = DATE.
      LT_PLAN       = PLAN.
      LT_PLAN-DATUM = '00000004'.
      IAPPEND LT_PLAN.
    ENDLOOP.
  ENDLOOP.
...
MODULE SET_STATUS OUTPUT.
  ...
  SET TITLEBAR  'T9000' WITH P_DATUM P_SEQNO.
...
* ?? Column
  ...
  PERFORM ADD_COLUMN USING 'C03' '21' 'R' 'Previous Plan'.
  PERFORM ADD_COLUMN USING 'C04' '21' 'R' 'Previous Actual'.
  PERFORM ADD_COLUMN USING 'C05' '21' 'R' 'Previous Difference'.
  PERFORM ADD_COLUMN USING 'C06' '21' 'R' 'Current Plan'.
...
* Top Node ? Expand ??.
*  READ TABLE GT_NODE_TABLE WITH KEY NODE_KEY = P_HIER.
...
FORM MAKE_PLAN .
  ...
  LOOP AT HIER_DB.
    ...
    CLEAR L_DMSHB.
    LOOP AT PREV_PLAN WHERE GRUPP = HIER_DB-WERT1.
      GT_ITEM_TABLE-NODE_KEY  = HIER_DB-NODID.
      GT_ITEM_TABLE-ITEM_NAME = 'C03'.
      GT_ITEM_TABLE-CLASS     = CL_GUI_COLUMN_TREE=>ITEM_CLASS_TEXT.
      L_DMSHB = PREV_PLAN-DMSHB.
      PERFORM WRITE_AMOUNT USING PREV_PLAN-DMSHB G_WAERS
                                 GT_ITEM_TABLE-TEXT.
      IAPPEND GT_ITEM_TABLE.
    ENDLOOP.
    IF SY-SUBRC NE 0.
      ...
      GT_ITEM_TABLE-ITEM_NAME = 'C03'.
      GT_ITEM_TABLE-CLASS     = CL_GUI_COLUMN_TREE=>ITEM_CLASS_TEXT.
      CLEAR PREV_PLAN-DMSHB.
      PERFORM WRITE_AMOUNT USING PREV_PLAN-DMSHB G_WAERS
                                 GT_ITEM_TABLE-TEXT.
      IAPPEND GT_ITEM_TABLE.
    ENDIF.

    LOOP AT PREV_ACT WHERE GRUPP = HIER_DB-WERT1.
      GT_ITEM_TABLE-NODE_KEY  = HIER_DB-NODID.
      GT_ITEM_TABLE-ITEM_NAME = 'C04'.
      GT_ITEM_TABLE-CLASS     = CL_GUI_COLUMN_TREE=>ITEM_CLASS_TEXT.
      L_DMSHB = L_DMSHB - PREV_ACT-DMSHB.
      PERFORM WRITE_AMOUNT USING PREV_ACT-DMSHB G_WAERS
                                 GT_ITEM_TABLE-TEXT.
      IAPPEND GT_ITEM_TABLE.
    ENDLOOP.
    IF SY-SUBRC NE 0.
...
*CODE
  ...
  LOOP AT PREV_END.
    ...
    GT_ITEM_TABLE-ITEM_NAME = 'C03'.
    GT_ITEM_TABLE-CLASS     = CL_GUI_COLUMN_TREE=>ITEM_CLASS_TEXT.
    PERFORM WRITE_AMOUNT USING PREV_END-DMSHB G_WAERS
                               GT_ITEM_TABLE-TEXT.
    APPEND GT_ITEM_TABLE.
    GT_ITEM_TABLE-ITEM_NAME = 'C04'.
    IAPPEND GT_ITEM_TABLE.
...
* Difference
  ...
  GT_ITEM_TABLE-CLASS     = CL_GUI_COLUMN_TREE=>ITEM_CLASS_TEXT.
  CLEAR PREV_END-DMSHB.
  PERFORM WRITE_AMOUNT USING PREV_END-WRSHB G_WAERS
                             GT_ITEM_TABLE-TEXT.
  IAPPEND GT_ITEM_TABLE.
