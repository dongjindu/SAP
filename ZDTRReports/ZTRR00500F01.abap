...
FORM SELECT_DATA.
  ...
  IF ACT_DATE[] IS NOT INITIAL .   "and p_act = 'X'.
    PERFORM GET_ACTUAL_DATA.
  ENDIF.

  IF PLAN_DATE[] IS NOT INITIAL.
    PERFORM GET_PLAN_DATA.
  ENDIF.

  LOOP AT ACTUAL.
    PLAN = ACTUAL.     "CMF summary records for planning groups
    ICOLLECT PLAN.
  ENDLOOP.
...
FORM CREATE_COLUMN .
  DATA : COLNAME(12),
         HEADER(132).
* Tree Column
  WRITE SY-DATUM TO GS_HIERARCHY_HEADER-HEADING MM/DD/YY.
...
FORM GET_PLAN_DATA .
  ...
  DATA : L_PDAT1 LIKE ZTTR0008-PDAT1.

*//  "If display: (in CBO Table Select).
*//      SELECT FROM Table [ZTRR0008]
  IF P_R2 = 'X'.
    L_PDAT1 = P_PDAT1.
  ENDIF.
  ...
  CALL FUNCTION 'ZTR_GET_PLAN_MONTHLY'
    EXPORTING
      ...
      GJAHR    = P_GJAHR
      MONTH    = P_MONTH     "month
      DATUM    = L_PDAT1     "Planning period (only display)
      PLANDATA = P_R1        "planning(X)
      ZTYPE    = C_DAILY     "daily("1")
      SEQNO    = P_SEQNO     "Sequence Number
    TABLES
      DATE     = PLAN_DATE
      PLAN     = PLAN.

  LOOP AT PLAN WHERE DATUM < SY-DATUM.
    PLAN-DATUM = SY-DATUM.
    COLLECT PLAN.
    DELETE PLAN.
    CONTINUE.
...
*&---------------------------------------------------------------------*
FORM GET_DATE .
  ICLEAR : DATE, ACT_DATE, PLAN_DATE.

  DATA : L_LASTDATE LIKE SY-DATUM.

  CONCATENATE P_GJAHR P_MONTH '01' INTO DATE.
...
*//  last day of months.
  CALL FUNCTION 'RP_LAST_DAY_OF_MONTHS'
    EXPORTING
      DAY_IN            = DATE
    IMPORTING
      LAST_DAY_OF_MONTH = L_LASTDATE.

*// Actual / Plan date create.
  DO.
    APPEND DATE.
    IF DATE < p_kdate.      "fixme SY-DATUM.
      APPEND DATE TO ACT_DATE.
    ELSE.
      APPEND DATE TO PLAN_DATE.
    ENDIF.
    IF DATE >= L_LASTDATE.
      EXIT.
    ENDIF.
