FUNCTION ZIM_DISCHG_EDI_DATA_RECEIVE.
*"----------------------------------------------------------------------
*"*"Local interface:
*"  IMPORTING
...
*>>>>>> CURRENCY
           ...
           WHEN '26'.      " 할인수수료
              PERFORM READ_TABLE_1   TABLES   IT_SAITIN_A
                                     CHANGING Z_ZFDDSEQ.
              ZTPMTHD-ZFUSIT      = IT_SAITIN_A-ZFDDFDA.
*>>>>>> CURRENCY
              PERFORM READ_TABLE_1   TABLES   IT_SAITIN_A
                                     CHANGING Z_ZFDDSEQ.
              ZTPMTHD-ZFUSITC     = IT_SAITIN_A-ZFDDFDA.
           WHEN '304'.     " 기타수수료
              PERFORM READ_TABLE_1   TABLES   IT_SAITIN_A
                                     CHANGING Z_ZFDDSEQ.
...
*>>>>>> CURRENCY
           ...
           WHEN '17'.      " 합계금액 ( DB 관리하지 않음 ). --> SKIP
        ENDCASE.
* PCD Seg. ( 어음할인율 ).
      WHEN '{14'.
        Z_ZFDDSEQ = IT_SAITIN_S-ZFDDSEQ + 2.
        READ TABLE IT_SAITIN_A WITH KEY ZFDDSEQ = Z_ZFDDSEQ.
        ZTPMTHD-ZFUSITR     = IT_SAITIN_A-ZFDDFDA.
* DTM ( 통지일자, 최종결제일, 할인일자 )
      WHEN '{15'.
        Z_ZFDDSEQ = IT_SAITIN_S-ZFDDSEQ + 1.
        READ TABLE IT_SAITIN_A WITH KEY ZFDDSEQ = Z_ZFDDSEQ.
...
* DATE CONVERT
*-----------------------------------------------------------------------
  IF NOT ZTPMTHD-ZFDSDT IS INITIAL. "--> 할인일
