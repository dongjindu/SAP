FUNCTION ZIM_LDANTC_EDI_DATA_RECEIVE.
*"----------------------------------------------------------------------
*"*"Local interface:
*"  IMPORTING
...
*>>>>>> CURRENCY
           ...
           WHEN '2AE'.     " 어음금액(원화)
              PERFORM READ_TABLE_1   TABLES   IT_SAITIN_A
                                     CHANGING Z_ZFDDSEQ.
              ZTPMTHD-ZFTIVAMK    = IT_SAITIN_A-ZFDDFDA.
*>>>>>> CURRENCY
              PERFORM READ_TABLE_1   TABLES   IT_SAITIN_A
                                     CHANGING Z_ZFDDSEQ.
              ZTPMTHD-ZFKRW       = IT_SAITIN_A-ZFDDFDA.
        ENDCASE.
* CUX Seg. ( 매입시점의 대고객 전신환 매입율 )
      WHEN '{14'.
        Z_ZFDDSEQ = IT_SAITIN_S-ZFDDSEQ + 1.
        READ TABLE IT_SAITIN_A WITH KEY ZFDDSEQ = Z_ZFDDSEQ.
        ZTPMTHD-ZFEXRT      = IT_SAITIN_A-ZFDDFDA.
* DTM ( 통지일자, 최종결제일, 할인일자 )
      WHEN '{15'.
        Z_ZFDDSEQ = IT_SAITIN_S-ZFDDSEQ + 1.
        READ TABLE IT_SAITIN_A WITH KEY ZFDDSEQ = Z_ZFDDSEQ.
        CASE  IT_SAITIN_A-ZFDDFDA.
           ...
           WHEN '2AD'.   "NEGO일자
              PERFORM READ_TABLE_1   TABLES   IT_SAITIN_A
                                     CHANGING Z_ZFDDSEQ.
              ZTPMTHD-ZFDSDT      = IT_SAITIN_A-ZFDDFDA.
        ENDCASE.
* 기타사항   ===> SKIP
      WHEN '{16'.
* 통지은행명 ===> SKIP
...
*>>> 금액 내부 필드값으로 변경...
  ...
  ENDIF.
  IF NOT ZTPMTHD-ZFPNAM IS INITIAL.
     PERFORM    SET_CURR_CONV_TO_INTERNAL CHANGING ZTPMTHD-ZFBKCH
                                                   ZTPMTHD-ZFBKCHC.
  ENDIF.
*-----------------------------------------------------------------------
* DATE CONVERT
*-----------------------------------------------------------------------
  IF NOT ZTPMTHD-ZFDSDT IS INITIAL. "--> NEGO일
     CALL FUNCTION 'CONVERT_DATE_TO_INTERNAL'
          EXPORTING
                     DATE_EXTERNAL = ZTPMTHD-ZFDSDT
          IMPORTING
                     DATE_INTERNAL = ZTPMTHD-ZFDSDT.
