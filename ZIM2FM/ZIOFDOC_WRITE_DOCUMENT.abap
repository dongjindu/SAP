FUNCTION ZIOFDOC_WRITE_DOCUMENT        .
...
IF UPD_ZTOFF                          NE SPACE.
   CALL FUNCTION 'CHANGEDOCUMENT_SINGLE_CASE'
     EXPORTING
       TABLENAME              = 'ZTOFF                         '
       WORKAREA_OLD           = O_ZTOFF
       WORKAREA_NEW           = N_ZTOFF
        CHANGE_INDICATOR       = UPD_ZTOFF
        DOCU_DELETE            = ' '
     EXCEPTIONS
       NAMETAB_ERROR          = 1
       OPEN_MISSING           = 2
...
IF UPD_ZTOFFFTX                       NE SPACE.
   CALL FUNCTION 'CHANGEDOCUMENT_SINGLE_CASE'
     EXPORTING
       TABLENAME              = 'ZTOFFFTX                      '
       WORKAREA_OLD           = O_ZTOFFFTX
       WORKAREA_NEW           = N_ZTOFFFTX
        CHANGE_INDICATOR       = UPD_ZTOFFFTX
        DOCU_DELETE            = ' '
     EXCEPTIONS
       NAMETAB_ERROR          = 1
       OPEN_MISSING           = 2
  ...
  IF UPD_ICDTXT_ZIOFDOC         NE SPACE.
     CALL FUNCTION 'CHANGEDOCUMENT_TEXT_CASE'
       TABLES
         TEXTTABLE              = ICDTXT_ZIOFDOC
       EXCEPTIONS
         OPEN_MISSING           = 1
