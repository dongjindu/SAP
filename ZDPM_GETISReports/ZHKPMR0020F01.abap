...
*. Get row data
  SELECT rd~werks
    ...
    FROM  mard AS rd INNER JOIN mara AS ra
...
*----------------------------------------------------------------------*
FORM modify_data .
  DATA : lt_pmt0002 LIKE TABLE OF zhkpmt0002 WITH HEADER LINE .

  CHECK gt_data[] IS NOT INITIAL .

  SELECT * INTO TABLE lt_pmt0002
    FROM zhkpmt0002
    FOR ALL ENTRIES IN gt_data
   WHERE bukrs EQ gt_data-bukrs
     ...
     AND zcdate EQ sy-datum .

  SORT gt_data BY bukrs werks lgort matnr zcdate .

  LOOP AT lt_pmt0002 .
    READ TABLE gt_data WITH KEY bukrs = lt_pmt0002-bukrs
                                werks = lt_pmt0002-werks
                                lgort = lt_pmt0002-lgort
                                matnr = lt_pmt0002-matnr
                                zcdate = sy-datum
                                BINARY SEARCH .
    IF sy-subrc = 0 .
      gt_data-zifflag   = lt_pmt0002-zifflag .
      gt_data-zifresult = lt_pmt0002-zifresult .
...
FORM make_fieldcat .
  ...
  PERFORM make_fieldcat_att USING:
   ...
   ' '  l_tabnam   'ZCDATE'  'StDate'     '10' ' ' ' ' ' ' ' ' ' ' ,

   ' '  l_tabnam   'MEINS'  'Unit'        '04' ' ' ' ' ' ' ' ' ' ' ,
   ' '  l_tabnam   'LABST'  'UR Stock'    '10' '1' ' ' ' ' ' ' ' ' ,
   ...
   ' '  l_tabnam   'LMINB'  'ReorderQty'    '08' '1' ' ' ' ' ' ' ' ' ,
   ' '  l_tabnam   'LBSTF'  'ReplenishQty'  '08' '1' ' ' ' ' ' ' ' ' ,
...
FORM get_line_rfc .
  ...
  DATA : lt_data LIKE zhkpmt0002 OCCURS 0 WITH HEADER LINE ,
         lt_send LIKE zhkpmt0002 OCCURS 0 WITH HEADER LINE .
  ...
  LOOP AT gt_data  .
    ...
    MOVE-CORRESPONDING gt_data TO lt_pmt0002 .
    APPEND lt_pmt0002 .

  ENDLOOP .

*. Update CBO table
  MODIFY zhkpmt0002 FROM TABLE lt_pmt0002 .
  SORT lt_pmt0002 BY werks lgort matnr .
...
*. Call RFC
    CALL FUNCTION 'ZPM002_STOCK_GETIS'
      DESTINATION p_dest
      TABLES
        t_data                = lt_send
      EXCEPTIONS
