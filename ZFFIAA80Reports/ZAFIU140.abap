*----------------------------------------------------------------------
* Program ID        : ZAFIU140
* Title             : [FI] Printing Label for Furniture Asset
* Created on        : March,16 2010
* Created by        : Valerian Utama
* Specifications By : Calvin Kong
* Description       : Printing program for Furniture Asset
*----------------------------------------------------------------------
* Modification Logs
...
DATA: BEGIN OF it_anlz OCCURS 0,
        ...
        stort type stort,      " Asset location
      END   OF it_anlz.
...
*Begin HIS20094 - NOT required for this program
SELECTION-SCREEN SKIP 1.
SELECT-OPTIONS : s_equnr FOR zsfiu129-equnr NO-DISPLAY,
                 s_eqtyp FOR ankaz-eqtyp NO INTERVALS NO-DISPLAY.

*PARAMETER p_vali LIKE sy-datum DEFAULT sy-datum OBLIGATORY
PARAMETER p_vali LIKE sy-datum DEFAULT sy-datum NO-DISPLAY.
*End HIS20094.
...
INITIALIZATION.
  ...
  sy-title = '[FI] Printing Label for Furniture Asset'.
  PERFORM make_button.
...
FORM move_out.
  ...
  LOOP AT it_row_tab.
    ...
    read table it_anlz WITH KEY BUKRS = it_row_tab-bukrs
                                ANLN1 = it_row_tab-anln1
                                ANLN2 = it_row_tab-anln2.
    if sy-subrc eq 0.
      gt_out-stort = it_anlz-stort.
    endif.

    APPEND gt_out.
  ENDLOOP.

* Begin HIS20094 - Remove duplicated items
  sort gt_out BY ANLN1 ANLN2.
  delete adjacent duplicates from gt_out comparing ANLN1 ANLN2.
* End HIS20094
...
* cost center {
    ...
    SELECT bukrs anln1 anln2 kostl kostlv bdatu stort
...
*  PERFORM get_all_equi.     "HIS20094 - NOT required
...
*BEGIN HIS20094 - suppress unused fields
    IF gs_fcat-fieldname EQ 'EQUNR'  OR
       gs_fcat-fieldname EQ 'EQKTX'  OR
       gs_fcat-fieldname EQ 'HERST'  OR
       gs_fcat-fieldname EQ 'ANSWT'  OR
       gs_fcat-fieldname EQ 'ANSDT'  OR
       gs_fcat-fieldname EQ 'ARBPL'  OR
       gs_fcat-fieldname EQ 'WKTEXT' OR
       gs_fcat-fieldname EQ 'ESTORT' OR
       gs_fcat-fieldname EQ 'EKTEXT'.
  ...
  IF NOT s_ebeln[] IS INITIAL.
    IF p_all EQ true.
      SELECT  a~bukrs a~anln1 a~anln2
              ...
              b~stort
              b~iloan b~swerk a~anlkl a~zujhr
