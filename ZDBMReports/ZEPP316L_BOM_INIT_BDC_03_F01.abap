*----------------------------------------------------------------------*
*   INCLUDE ZEPP316L_BOM_INIT_BDC_03_F01                               *
*----------------------------------------------------------------------*
*&---------------------------------------------------------------------*
*&      Form  INITIALIZATION
*&---------------------------------------------------------------------*
...
*     AT NEW STLAL
      AT NEW ZSEQU.
        L_TANEW = L_TABIX.
        L_MODE = '1'.
        PERFORM READ_MAST_STPO_STLKN1 USING LA_EXCL-MATNR "Material
                                            LA_EXCL-WERKS "Plant
                                            LA_EXCL-STLAN "BOM usage
                                            LA_EXCL-STLAL "Alternative
                                            LA_EXCL-POSNR "Item number
                                            LA_EXCL-IDNRK "BOM component
                                            LA_EXCL-ZSUFF "SUFFIX
                                            LA_EXCL-ZSEQU
                                    CHANGING L_MODE
...
*     'UPDATE CONTROL MODE' different BDC
      IF L_MODE EQ '1'.
        ...
        L_MODE = '2'.
      ELSEIF L_MODE EQ '2'.
        PERFORM BADY_DEPENDENCY USING LA_EXCL-DPID.
      ENDIF.
*     AT END OF STLAL
      AT END OF ZSEQU.
        L_TAEND = L_TABIX.
...
FORM SAME_DATA_EXISTS_CHECK.
  ...
  LOOP AT LT_EXCL.
    ...
    LOOP AT IT_EXCL WHERE MATNR EQ LT_EXCL-MATNR
                    ...
                    AND   CLPT  EQ LT_EXCL-CLPT
                    AND   DPID  EQ LT_EXCL-DPID.
      M_TABIX = SY-TABIX.
      L_INDEX = L_INDEX + 1.
      IF L_INDEX GE 2.
        MT_EXCL = IT_EXCL.
...
*     BOM HEADER BDC PROCESS
      IF PA_CHECK NE 'X'.
        ...
        LOOP AT LT_MAST.
          READ TABLE IT_MAST WITH KEY MATNR = LT_MAST-MATNR
                                      WERKS = LT_MAST-WERKS
                                      STLAN = LT_MAST-STLAN
                                      STLAL = LT_MAST-STLAL
                             BINARY SEARCH.
          IF SY-SUBRC NE 0.
            PERFORM BOM_HEADER_CREATE USING LT_MAST-MATNR  "Material
                                            ...
                                            LT_MAST-BMENG  "Confirmed qu
                                            LT_MAST-STLST  "BOM Status
                                      CHANGING PA_CHECK.
          ENDIF.
          CLEAR: LT_MAST, IT_MAST.
        ENDLOOP.
        IF PA_CHECK EQ 'X'.
          CLEAR WA_ERRO_IDX.
