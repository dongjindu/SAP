...
*
************************************************************************
REPORT ZEPP313E_ROUTING_ASSIGNMENT
...
DATA: BEGIN OF IT_LFPD OCCURS 0,
        MATNR TYPE ZTBM_ABXLFPDT-MTNO,
        WERKS TYPE ZTBM_ABXLFPDT-PLNT,
        VERS  TYPE ZTBM_ABXLFPDT-VERS,
        TEXT  TYPE ZTBM_ABXLFPDT-TEXT,
        FVLD  TYPE ZTBM_ABXLFPDT-FVLD,
        TVLD  TYPE ZTBM_ABXLFPDT-TVLD,
        STLAL TYPE ZTBM_ABXLFPDT-ALTN,
        STLAN TYPE ZTBM_ABXLFPDT-USAG,
      END OF IT_LFPD.
...
*SELECT-OPTIONS: S_MTNO FOR ZTBM_ABXLFPDT-MTNO.
PARAMETERS:
  P_ZBDAT LIKE ZTBM_ABXLFPDT-ZBDAT DEFAULT SY-DATUM,
...
*&---------------------------------------------------------------------*
FORM READ_PROCESS.
  PERFORM READ_ZTBM_ABXLFPDT.
ENDFORM.                    " READ_PROCESS
*&---------------------------------------------------------------------*
*&      Form  READ_ZTBM_ABXLFPDT
*&---------------------------------------------------------------------*
FORM READ_ZTBM_ABXLFPDT.
  SELECT MTNO
         PLNT
...
*       WHERE ZBDAT EQ P_ZBDAT
       AND   ZRESULT NE 'E'.
*       WHERE MTNO  IN S_MTNO
*       AND   ZBDAT EQ P_ZBDAT
*       AND   ZRESULT NE 'E'.
  IF SY-SUBRC EQ 0.
...
*   READ PLKO
    SELECT SINGLE PLNNR
                ...
                INTO M_PLNNR
                WHERE PLNTY EQ 'M'
                AND   PLNNR EQ IT_LFPD-MATNR+6(7)
                AND   LOEKZ NE 'X'.
      ...
      LOOP AT LT_PLKO.
        ...
        ELSE.
          DO 6 TIMES.
            CASE SY-INDEX.
              WHEN '1'.
                IF LT_PLKO-PLNNR(2) EQ IT_LFPD-MATNR+6(2).
                  LT_PLKO-COUNT = LT_PLKO-COUNT + 200.
                ENDIF.
              WHEN '2'.
                IF LT_PLKO-PLNNR+2(1) EQ IT_LFPD-MATNR+8(1).
                  LT_PLKO-COUNT = LT_PLKO-COUNT + 100.
                ENDIF.
              WHEN '3'.
                IF LT_PLKO-PLNNR+3(1) EQ IT_LFPD-MATNR+9(1).
                  LT_PLKO-COUNT = LT_PLKO-COUNT + 50.
                ENDIF.
              WHEN '4'.
                IF LT_PLKO-PLNNR+4(1) EQ IT_LFPD-MATNR+10(1).
                  LT_PLKO-COUNT = LT_PLKO-COUNT + 20.
                ENDIF.
              WHEN '5'.
                IF LT_PLKO-PLNNR+5(1) EQ IT_LFPD-MATNR+11(1).
                  LT_PLKO-COUNT = LT_PLKO-COUNT + 10.
                ENDIF.
              WHEN '6'.
                IF LT_PLKO-PLNNR+6(1) EQ IT_LFPD-MATNR+12(1).
...
* FSC ROUTING MM02 ASSIGN
  ...
  ENDLOOP.
*  LOOP AT IT_LFPD.
*    CLEAR: L_PLNNR, LT_MAPL, LT_PLKO, M_PLNNR, N_PLNNR, L_LINES.
*    REFRESH: LT_MAPL, LT_PLKO.
**   CHECK ROUTING GROUP
*    PERFORM READ_MAPL_CHECK USING    IT_LFPD-MATNR
*                                     IT_LFPD-WERKS
*                            CHANGING L_PLNNR.
...
*        ENDIF.
*      ELSE.
**       RATE ROUTING CREATE & REFERENCE RATE ROUTION CREATE
**       READ PLKO
*        CONCATENATE IT_LFPD-MATNR+6(2) '%' INTO Q_PLNNR.
*        SELECT PLNNR
*             FROM PLKO
*             INTO TABLE LT_PLKO
*             WHERE PLNTY EQ 'M'
*             AND   LOEKZ NE 'X'.
**             AND   PLNNR LIKE Q_PLNNR.
*        IF SY-SUBRC EQ 0.
*          LOOP AT LT_PLKO.
*            L_TABIX = SY-TABIX.
*            IF LT_PLKO-PLNNR+6(1) IS INITIAL.
...
*            M_PLNNR = LT_PLKO-PLNNR.
*          ENDIF.
**          LT_PLKO-PLNNR = IT_LFPD-MATNR+6(7).
**          APPEND LT_PLKO.
**          CLEAR LT_PLKO.
**          SORT LT_PLKO BY PLNNR.
**          DESCRIBE TABLE LT_PLKO LINES L_LINES.
**          READ TABLE LT_PLKO WITH KEY PLNNR = IT_LFPD-MATNR+6(7)
**                             BINARY SEARCH.
**          IF SY-SUBRC EQ 0.
**            L_TABIX = SY-TABIX.
