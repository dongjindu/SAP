...
* SORTING
*  SORT IT_EXCL BY MATNR WERKS STLAN STLAL POSNR IDNRK ZSUFF ZSEQU.
  LOOP AT IT_EXCL.
    MOVE-CORRESPONDING IT_EXCL TO LT_EXCL.
    APPEND LT_EXCL.
    CLEAR: IT_EXCL, LT_EXCL.
  ENDLOOP.
  SORT LT_EXCL BY MATNR WERKS STLAN STLAL POSNR IDNRK ZSUFF ZSEQU UPCT.
  LOOP AT LT_EXCL.
    L_TABIX = SY-TABIX.
    LA_EXCL = LT_EXCL.
    CLEAR: L_STLKN.
      ...
      CASE L_MODE.
        WHEN '3'.
          IF NOT L_STLKN IS INITIAL.
            PERFORM CHANGE_HEADER_DE USING LA_EXCL-MATNR "Materialnumber
                                           LA_EXCL-WERKS "Plant
                                           LA_EXCL-STLAN "BOM usage
                                           LA_EXCL-STLAL "AlternativeBOM
                                           LA_EXCL-AENNR. "Change number
            PERFORM CHANGE_BADY_ITEM_DELETE USING LA_EXCL-POSNR
                                                  LA_EXCL-IDNRK
                                                  L_STLKN.
...
*        WHEN '2'.
*          CLEAR L_MODE.
*          PERFORM READ_MAST_STPO_STLKN USING
*                                       LA_EXCL-MATNR "Material number
*                                       LA_EXCL-WERKS "Plant
*                                       LA_EXCL-STLAN "BOM usage
*                                       LA_EXCL-STLAL "Alternative BOM
*                                       LA_EXCL-POSNR "BOM item number
...
*                                                L_STLKN.
        ENDCASE.
        CASE L_MODE.
          WHEN '1' OR '2'.  "CHANGE(ITEM CREATE)
          PERFORM CHANGE_HEADER_BOM USING LA_EXCL-MATNR "Material number
                                              LA_EXCL-WERKS "Plant
                                              LA_EXCL-STLAN "BOM usage
                                          LA_EXCL-STLAL "Alternative BOM
                                           LA_EXCL-AENNR. "Change number
*        WHEN '3'.  "CHANGE(ITEM DELETE)
*          PERFORM CHANGE_HEADER_DE  USING LA_EXCL-MATNR
*                                             "Material number
*                                          LA_EXCL-WERKS "Plant
*                                          LA_EXCL-STLAN "BOM usage
*                                          LA_EXCL-STLAL
...
*      WHEN '2'.
          PERFORM CHANGE_BADY_ITEM_CREATE1
                              USING LA_EXCL-POSNR  "BOM item number
                                   ...
                                   LA_EXCL-ITSOB  "procurement type item
...
*&---------------------------------------------------------------------*
FORM CHANGE_BADY_ITEM_DELETE USING P_POSNR
                                   P_IDNRK
                                   P_STLKN.
  PERFORM DYNPRO USING:
*     'X' 'SAPLCSDI'    '0150',
...
FORM COLOR_PART_DEPENDENCY USING P_MTNO
  ...
  PERFORM DYNPRO USING:
     ...
     'X' 'SAPLCUKD'    '0130',
     ' ' 'RCUKD-KNNAM(01)' P_OPID, "DEPENDENCY
     ' ' 'BDC_OKCODE'  '/00',
