...
*                                       e-BOM                          *
*& 10/06/2011  KDM          UP1K920247  4.6C --> ECC : CS02 BDM change *
*&                                      Index : KDM01                  *
************************************************************************
...
DATA: BEGIN OF it_bom_ecm OCCURS 0.
        INCLUDE STRUCTURE ztbm_ebom_ecm.
DATA:   create_seq(14),
      END   OF it_bom_ecm.
...
DATA: BEGIN OF it_module_status OCCURS 0,
        mtno   LIKE   it_bom_ecm-mtno,
        comp   LIKE   it_bom_ecm-comp,
        maktx   LIKE   it_9002-maktx,
      END   OF it_module_status.
...
DATA: BEGIN OF it_sub_val OCCURS 0.
        INCLUDE STRUCTURE ztbm_sub_bom.
...
DATA:   w_update_status(1),
        v_1st_srn TYPE bdc_dynr.
*DATA: g_first(1).
...
DATA:      BEGIN OF g_tab_9000,
             ...
             prog        LIKE sy-repid VALUE 'ZIPP302U_BOM_BDC_03_01',
...
SELECT-OPTIONS: s_plnt FOR ztbm_ebom_ecm-plnt NO INTERVALS NO-EXTENSION,
                s_mtno FOR ztbm_ebom_ecm-mtno,
...
** Changed by Furong on 09/29/10
  PERFORM send_email_for_module.
** End of change
...
** On 03/18/13  - exclude T/M 10 digits part number
  DATA: lt_fsc_cre_inf LIKE TABLE OF ztbm_fsc_cre_inf
         WITH HEADER LINE.
  SELECT * INTO TABLE lt_fsc_cre_inf
    FROM ztbm_fsc_cre_inf
    WHERE text = 'TM-ENG'.

  LOOP AT it_bom_ecm WHERE stgb = 'P' OR stgb = 'F'.

    CASE it_bom_ecm-stgb.
      WHEN 'P'.
        READ TABLE lt_fsc_cre_inf WITH KEY valu1 = it_bom_ecm-comp.
        IF sy-subrc = 0 AND lt_fsc_cre_inf-valu5 = 'PART'.
          UPDATE ztbm_ebom_ecm SET: zresult = 'P'
                                    zbnam = sy-uname
                                    zbdat = sy-datum
                                    zbtim = sy-uzeit
                 WHERE idoc = it_bom_ecm-idoc
                   AND item = it_bom_ecm-item
                   ...
                   AND seqc = it_bom_ecm-seqc.
          DELETE it_bom_ecm.
        ENDIF.
      WHEN 'F'.
        READ TABLE lt_fsc_cre_inf WITH KEY valu1 = it_bom_ecm-comp.
        IF sy-subrc = 0 AND lt_fsc_cre_inf-valu5 = 'UPG-VC'.
          UPDATE ztbm_ebom_ecm SET zresult = 'P'
                                   zbnam = sy-uname
                                   zbdat = sy-datum
                                   zbtim = sy-uzeit
...
** Added by Furong on 10/21/10
*  LOOP AT IT_BOM_ECM.
*    IF IT_BOM_ECM-USAG = '2'.
*      IT_BOM_ECM-USAG = '3'.
*      MODIFY IT_BOM_ECM.
*    ENDIF.
*  ENDLOOP.
** End of change on 10/21/10
...
FORM get_sub_bom_value.
  ...
  SELECT a~werks a~matnr a~sequ a~z_nation a~mtart a~z_car a~z_year
         a~z_dealer a~z_bt a~z_ec a~z_et a~z_ft a~z_tm a~z_ocn
         b~matnr AS check
    INTO CORRESPONDING FIELDS OF TABLE it_sub_val
    FROM ztbm_sub_bom AS a LEFT OUTER JOIN marc AS b
      ON a~mandt = b~mandt
     AND a~werks = b~werks
     AND a~matnr = b~matnr.
...
*    CASE it_fsc-mtno+4(2).
    CASE it_fsc-mtno+4(1).
*      WHEN 'XX'.                         "BIW
      WHEN 'W'.                         "BIW
*        IF NOT it_sub_val-z_nation CS 'XX'.
        IF NOT it_sub_val-z_dealer EQ 'W'.
          CONTINUE.
        ENDIF.
*      WHEN 'XY'.                         "BIP
      WHEN 'P'.                         "BIP
*        IF NOT it_sub_val-z_nation CS 'XY'.
        IF NOT it_sub_val-z_dealer EQ 'P'.
          CONTINUE.
        ENDIF.
      WHEN OTHERS.
        IF NOT ( it_fsc-mtno(1)    CP it_sub_val-z_year   AND
                 it_fsc-mtno+1(3)  CP it_sub_val-z_nation AND
                 it_fsc-mtno+4(1)  CP it_sub_val-z_dealer AND
                 it_fsc-mtno+5(2)  CP it_sub_val-z_car    AND
                 it_fsc-mtno+7(2)  CP it_sub_val-z_bt     AND
                 it_fsc-mtno+9(2)  CP it_sub_val-z_ec     AND
                 it_fsc-mtno+11(1) CP it_sub_val-z_et     AND
                 it_fsc-mtno+12(1) CP it_sub_val-z_ft     AND
                 it_fsc-mtno+13(1) CP it_sub_val-z_tm     AND
                 it_fsc-mtno+14(4) CP it_sub_val-z_ocn   ).
          CONTINUE.
        ENDIF.
    ENDCASE.
...
*                        pt_od     STRUCTURE wa_od
  ...
  DATA: l_inactive(1),
        l_stlst LIKE stko-stlst.

  CHECK it_9001-zresult EQ space.

** Furong on 10/21/13 et status to active
  IF it_9001-usag = '1'.

    SELECT SINGLE stlst INTO l_stlst
      FROM mast AS a
      ...
      WHERE matnr = pw_matnr
        AND stlan = pw_stlan
        AND a~stlal = pw_stlal
        AND werks = pw_werks
        AND stlty = 'M'.
    .
    IF l_stlst = '02'.
      l_inactive = 'X'.
      PERFORM set_bom_status USING pw_matnr pw_stlan
                                  pw_stlal pw_werks
                                  '01'.
    ENDIF.
  ENDIF.
...
** Furong on 10/21/13 reset status to inactive
  IF l_inactive = 'X'.
    PERFORM set_bom_status USING pw_matnr pw_stlan
                                 pw_stlal pw_werks
                                 '02'.
...
** Changed by Furong on 09/28/10
    IF it_9001-usag = '2' AND it_9001-zresult EQ space AND
                              w_bom_sts_err   EQ space.
      PERFORM bom_status_update.
    ENDIF.
** End of change on 09/28/10

    IF w_bom_sts_err   NE space AND
       it_9001-zresult EQ space.
      MOVE: c_sap_err TO it_9001-zresult,
            text-m03  TO it_9001-zmsg.
    ENDIF.
...
*     it_9001-mtno+6(2) NE it_9001-comp(2).
     it_9001-mtno+5(2) NE it_9001-comp(2).
...
FORM delete_bom_item TABLES pt_stb    STRUCTURE stpox.
  ...
  CHECK it_9001-datuv NE it_9001-datub.

  REFRESH: it_bdc.

*  WRITE: it_9001-datuv TO lw_datum.
  WRITE: c_init_date TO lw_datum.
...
**** : KDM01 - End
  ...
  READ TABLE pt_stb INDEX 1.
  IF sy-subrc EQ 0.
    CLEAR: l_found.
    LOOP AT pt_stb.
      IF pt_stb-datuv <= it_9001-datf AND
         pt_stb-datub > it_9001-datf.
        l_found = 'X'.
        EXIT.
      ENDIF.

      IF pt_stb-datuv NE pt_stb-datub AND
         pt_stb-datuv >  it_9001-datf AND
         NOT ( pt_stb-datuv <= it_9001-datf AND
               pt_stb-datub > it_9001-datf ).
        l_found = 'X'.
        EXIT.
      ENDIF.
    ENDLOOP.
    IF l_found EQ 'X'.
      MOVE: '0150'        TO v_1st_srn.
    ELSE.
      MOVE: '0140'        TO v_1st_srn.
    ENDIF.
  ELSE.
    MOVE: '0140'        TO v_1st_srn.
  ENDIF.
...
*** : KDM01 - Start
** Change don 09/19/13
*  IF g_first <> 'X'. "" AND IT_9001-UPT_MODE <> 'C'.
  PERFORM dynpro USING:
*     'X' 'SAPLCSDI'        '0150',
*     ' ' 'BDC_OKCODE'      '=FCNP'.
    'X' 'SAPLCSDI'        v_1st_srn,
   ' ' 'BDC_OKCODE'      '=FCNP'.
* ENDIF.
** End on 09/19/13
...
FORM table_update.
  LOOP AT it_9001.
    ...
    CLEAR: ztbm_ebom_ecm.
    MOVE: it_bom_ecm TO ztbm_ebom_ecm.
** Changed by Furong on 10/21/10
*    IF IT_BOM_ECM-USAG = '3'.
*      ZTBM_EBOM_ECM-USAG = '2'.
*    ENDIF.
** End of change on 10/21/10
    UPDATE ztbm_ebom_ecm.
  ENDLOOP.
...
*-<    Modified on 02.03.2014
      ( it_9001-plnt EQ 'P001' AND it_9001-usag EQ '1'
       AND it_9001-eitm EQ 'C' AND it_9001-stgb EQ 'P' ) OR

      ( it_9001-plnt EQ 'P001' AND it_9001-usag EQ '1'
       AND it_9001-eitm EQ 'C' AND it_9001-stgb EQ 'N' ).
*->
...
*  CHECK IT_BOM_ECM-PLNT EQ C_ENGINE_PLANT.
  CHECK it_bom_ecm-plnt NE c_plant.                         "P001
** END E002
...
*---// Second BDC is assignment to-change no.
  ...
  IF pw_stb-datuv NE c_end_date.  "01/17/2014
    MOVE: c_update TO wa_diff_chk-others_change.
  ENDIF.
...
*
  ...
  it_module_status-mtno = it_9001-mtno.
  it_module_status-comp = it_9001-comp.

  SELECT SINGLE maktx INTO it_module_status-maktx
    FROM makt
    WHERE matnr = it_9001-comp
      AND spras = 'EN'.

  APPEND it_module_status.

  CHECK  w_update_status = 'X'.
  REFRESH: it_bdc.

*  PERFORM DYNPRO USING: 'X' 'SAPLCSDI' '0100',
*                        ' ' 'BDC_CURSOR' 'RC29N-STLAL',
*                        ' ' 'BDC_OKCODE' '=KALL',
*                        ' ' 'RC29N-MATNR' IT_9001-MTNO,
*                        ' ' 'RC29N-WERKS' IT_9001-PLNT,
*                        ' ' 'RC29N-STLAN' IT_9001-USAG,
*                        ' ' 'RC29N-STLAL' IT_9001-ALTN,
**perform bdc_field       using 'RC29N-DATUV'
**                              record-DATUV_005.
*
*                        'X' 'SAPLCSDI' '2110',
*                        ' ' 'BDC_OKCODE' '=FCBU',
*                        ' ' 'BDC_CURSOR' 'RC29K-STLBE',
*                        ' ' 'RC29K-STLST' '02'.
**perform bdc_field       using 'RC29K-STLBE'
**                              record-STLBE_008.


  PERFORM dynpro USING:       'X' 'SAPLCSDI' '0100',
                              ' ' 'BDC_CURSOR' 'RC29N-STLAN',
                              ' ' 'BDC_OKCODE' '=KALL',
                              ' ' 'RC29N-MATNR'
                              it_9001-mtno,
                              ' ' 'RC29N-WERKS'
...
ENDFORM.                    " bom_status_update
*&---------------------------------------------------------------------*
*&      Form  send_email_for_module
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
...
*          GD_ERROR TYPE SY-SUBRC.
  ...
  CHECK NOT it_module_status[] IS INITIAL.
  CLEAR: it_mail,it_mail[].

** Make body part of email
  MOVE 'RE: Go to Transaction ZMME00001. To Activate the Module BOM(s)'
     TO it_mail.
  CONCATENATE  it_mail  'created or changed today' INTO it_mail
   SEPARATED BY space.
...
*  CLEAR: IT_MAIL.
  ...
  SORT it_module_status BY mtno comp.
  DELETE ADJACENT DUPLICATES FROM it_module_status.

  LOOP AT it_module_status.
    ON CHANGE OF it_module_status-mtno.
      MOVE: it_module_status-mtno TO it_mail+0(18).
    ENDON.
    MOVE: it_module_status-comp TO it_mail+20(18).
    MOVE: it_module_status-maktx TO it_mail+40(40).
    APPEND it_mail.
    CLEAR: it_mail.
  ENDLOOP.
...
*      -->P_2342   text
*----------------------------------------------------------------------*
FORM set_bom_status  USING p_matnr p_stlan
                           p_stlal p_werks p_status.
  REFRESH: it_bdc.
  PERFORM dynpro:      USING 'X' 'SAPLCSDI' '0100',
                       USING ' ' 'BDC_CURSOR' 'RC29N-DATUV',
                       USING ' ' 'BDC_OKCODE' '=KALL',
                       USING ' ' 'RC29N-MATNR' p_matnr,
                       USING ' ' 'RC29N-WERKS' p_werks,
                       USING ' ' 'RC29N-STLAN' p_stlan,
                       USING ' ' 'RC29N-STLAL' p_stlal.
*      using 'RC29N-DATUV' Rrecord-DATUV_005.
  PERFORM dynpro:      USING 'X' 'SAPLCSDI' '2110',
                       USING ' ' 'BDC_OKCODE' '=FCBU',
                       USING ' ' 'BDC_CURSOR' 'RC29K-STLST',
                       USING ' ' 'RC29K-STLST' p_status.
  ...
  IF sy-subrc NE 0 OR sy-msgno NE '031'.
    ...
    MOVE: c_sap_err TO it_9001-zresult,
          c_sap_err TO w_bom_sts_err.
    PERFORM get_err_msg USING 'CS02' it_9001-zmsg.
  ENDIF.
ENDFORM.                    " SET_BOM_STATUS
