...
*
************************************************************************
REPORT ZEPP309U_ABXHPCDT_UPLOAD
...
* INTERNAL-TABLE AREA
*----------------------------------------------------------------------*
DATA: BEGIN OF IT_HPC1 OCCURS 1,
      ...
      END   OF IT_HPC1.
DATA: BEGIN OF IT_HPC2 OCCURS 0,
        FSCC(23), "TYPEZTBM_ABXHPCDT-ZFSCC, "
        PQBG(1), "TYPEZTBM_ABXHPCDT-PQBG, "Type
        CEXT(3), "TYPEZTBM_ABXHPCDT-CEXT, "External Color
        CINT(3), "TYPEZTBM_ABXHPCDT-CINT, "Internal Color
        SEQU(4) TYPE N, "TYPEZTBM_ABXHPCDT-ZSEQU, "
        STAT(20), "TYPEZTBM_ABXHPCDT-STAT, "Material status
        ...
        ZBTIM(6), "TYPEZTBM_ABXHPCDT-ZBTIM, "SAP BDC EXECUTED TIME
        ZBNAM(12), "TYPEZTBM_ABXHPCDT-ZBNAM, "BDC User ID
        ZMODE(1), " "Data Characteristic Flag (Create/Update/Delete)
        ZRESULT(1), ""Result of the Processing
        ZMSG(220), "TYPEZTBM_ABXHPCDT-ZMSG, "Message text
      END   OF IT_HPC2.

DATA: BEGIN OF IT_HPC3 OCCURS 0,
        FSCC(23), "TYPEZTBM_ABXHPCDT-ZFSCC, "
        PQBG(1), "TYPEZTBM_ABXHPCDT-PQBG, "Type
        CEXT(3), "TYPEZTBM_ABXHPCDT-CEXT, "External Color
        CINT(3), "TYPEZTBM_ABXHPCDT-CINT, "Internal Color
...
START-OF-SELECTION.
*  DATA: IT_HPCD LIKE ZTBM_ABXHPCDT OCCURS 0 WITH HEADER LINE,
*        IT_HPCD01 LIKE ZTBM_ABXHPCDT01 OCCURS 0 WITH HEADER LINE.
*  SELECT *
*       FROM ZTBM_ABXHPCDT
*       INTO TABLE IT_HPCD.
*
...
*       INTO TABLE IT_HPCD01.
*  DELETE ZTBM_ABXHPCDT FROM TABLE IT_HPCD.
*  DELETE ZTBM_ABXHPCDT01 FROM TABLE IT_HPCD01.
*  COMMIT WORK.
  PERFORM UPLOAD_PROCESS.
  PERFORM UPDATE_ZTBM_ABXHPCDT.
...
FORM UPLOAD_PROCESS.
  CASE 'X'.
    WHEN P_RDO1.
      PERFORM UPLOAD_CODE_001_200.
    WHEN P_RDO2.
      PERFORM UPLOAD_CODE_201_400.
    WHEN P_RDO3.
      PERFORM UPLOAD_CODE_401_600.
    WHEN P_RDO4.
      PERFORM DELETE_ABXHPCDT.
  ENDCASE.

ENDFORM.                               " UPLOAD_PROCESS
*&---------------------------------------------------------------------*
*&      Form  UPDATE_ZTBM_ABXHPCDT
*&---------------------------------------------------------------------*
FORM UPDATE_ZTBM_ABXHPCDT.
  CASE 'X'.
    WHEN P_RDO1.
      PERFORM HPCS_CODE_001_200.
    WHEN P_RDO2.
      PERFORM HPCS_CODE_201_400.
    WHEN P_RDO3.
      PERFORM HPCS_CODE_401_600.
  ENDCASE.
ENDFORM.                    " UPDATE_ZTBM_ABXHPCDT
*&---------------------------------------------------------------------*
*&      Form  HPCS_CODE_001_200
*&---------------------------------------------------------------------*
FORM HPCS_CODE_001_200.
  DATA: LT_HPCD TYPE ZTBM_ABXHPCDT OCCURS 0 WITH HEADER LINE.
  LOOP AT IT_HPC1.
    MOVE-CORRESPONDING IT_HPC1 TO LT_HPCD.
    APPEND LT_HPCD.
    CLEAR: IT_HPC1, LT_HPCD.
  ENDLOOP.
  MODIFY ZTBM_ABXHPCDT FROM TABLE LT_HPCD.
  IF SY-SUBRC EQ 0.
    WRITE: / 'ZTBM_ABXHPCDT MODIFY SUCCESS'.
    COMMIT WORK.
  ELSE.
    WRITE: / 'ZTBM_ABXHPCDT MODIFY ERROR'.
    ROLLBACK WORK.
  ENDIF.
ENDFORM.                    " HPCS_CODE_001_200
*&---------------------------------------------------------------------*
*&      Form  HPCS_CODE_201_400
*&---------------------------------------------------------------------*
FORM HPCS_CODE_201_400.
  DATA: L_TABIX TYPE SY-TABIX.
  DATA: LT_HPC1 TYPE ZTBM_ABXHPCDT OCCURS 0 WITH HEADER LINE.
  DATA: LT_HPC2 TYPE ZTBM_ABXHPCDT01 OCCURS 0 WITH HEADER LINE.
  SELECT *
       FROM ZTBM_ABXHPCDT
       INTO TABLE LT_HPC1
       FOR ALL ENTRIES IN IT_HPC2
       WHERE FSCC   EQ IT_HPC2-FSCC
       AND   PQBG   EQ IT_HPC2-PQBG
       AND   CEXT   EQ IT_HPC2-CEXT
       AND   CINT   EQ IT_HPC2-CINT
       AND   SEQU   EQ IT_HPC2-SEQU.
  SORT LT_HPC1 BY FSCC PQBG CEXT CINT SEQU.

  LOOP AT IT_HPC2.
    MOVE-CORRESPONDING IT_HPC2 TO LT_HPC2.
    APPEND LT_HPC2.
    READ TABLE LT_HPC1 WITH KEY FSCC   = IT_HPC2-FSCC
                                PQBG   = IT_HPC2-PQBG
                                CEXT   = IT_HPC2-CEXT
                                CINT   = IT_HPC2-CINT
                                SEQU   = IT_HPC2-SEQU
    ...
    IF SY-SUBRC EQ 0.
      L_TABIX = SY-TABIX.
      MOVE-CORRESPONDING IT_HPC2 TO LT_HPC1.
      MODIFY LT_HPC1 INDEX L_TABIX.
    ENDIF.
    CLEAR: IT_HPC2, LT_HPC1, IT_HPC2.
  ENDLOOP.

  MODIFY ZTBM_ABXHPCDT FROM TABLE LT_HPC1.
  IF SY-SUBRC EQ 0.
    MODIFY ZTBM_ABXHPCDT01 FROM TABLE LT_HPC2.
    IF SY-SUBRC EQ 0.
      WRITE: / 'ZTBM_ABXHPCDT MODIFY ', ' & ',
               'ZTBM_ABXHPCDT01 MODIFY SUCCESS'.
      COMMIT WORK.
    ELSE.
      WRITE: / 'ZTBM_ABXHPCDT01 MODIFY ERROR'.
    ENDIF.
  ELSE.
    WRITE: / 'ZTBM_ABXHPCDT MODIFY ERROR'.
    ROLLBACK WORK.
  ENDIF.
ENDFORM.                    " HPCS_CODE_201_400
*&---------------------------------------------------------------------*
*&      Form  HPCS_CODE_401_600
*&---------------------------------------------------------------------*
FORM HPCS_CODE_401_600.
  DATA: L_TABIX TYPE SY-TABIX.
  DATA: LT_HPCD TYPE ZTBM_ABXHPCDT01 OCCURS 0 WITH HEADER LINE.
  SELECT *
       FROM ZTBM_ABXHPCDT
       INTO TABLE LT_HPCD
       FOR ALL ENTRIES IN IT_HPC3
       WHERE FSCC   EQ IT_HPC3-FSCC
       AND   PQBG   EQ IT_HPC3-PQBG
       AND   CEXT   EQ IT_HPC3-CEXT
       AND   CINT   EQ IT_HPC3-CINT
       AND   SEQU   EQ IT_HPC3-SEQU.
  SORT LT_HPCD BY FSCC PQBG CEXT CINT SEQU.

  LOOP AT IT_HPC3.
    READ TABLE LT_HPCD WITH KEY FSCC   = IT_HPC3-FSCC
                                PQBG   = IT_HPC3-PQBG
                                CEXT   = IT_HPC3-CEXT
                                CINT   = IT_HPC3-CINT
                                SEQU   = IT_HPC3-SEQU
    ...
    IF SY-SUBRC EQ 0.
      L_TABIX = SY-TABIX.
      MOVE-CORRESPONDING IT_HPC3 TO LT_HPCD.
      MODIFY LT_HPCD INDEX L_TABIX.
    ENDIF.
    CLEAR: IT_HPC3, LT_HPCD.
  ENDLOOP.

  MODIFY ZTBM_ABXHPCDT01 FROM TABLE LT_HPCD.
  IF SY-SUBRC EQ 0.
    WRITE: / 'ZTBM_ABXHPCDT01 MODIFY SUCCESS'.
    COMMIT WORK.
  ELSE.
    WRITE: / 'ZTBM_ABXHPCDT01 MODIFY ERROR'.
  ENDIF.
ENDFORM.                    " HPCS_CODE_401_600
*&---------------------------------------------------------------------*
*&      Form  UPLOAD_CODE_001_200
*&---------------------------------------------------------------------*
FORM UPLOAD_CODE_001_200.
...
*           FILELENGTH              =
  ...
  ENDCASE.
ENDFORM.                    " UPLOAD_CODE_001_200
*&---------------------------------------------------------------------*
*&      Form  UPLOAD_CODE_201_400
*&---------------------------------------------------------------------*
FORM UPLOAD_CODE_201_400.
  CALL FUNCTION 'WS_UPLOAD'
       EXPORTING
...
*           FILELENGTH              =
  ...
  CASE SY-SUBRC.
    ...
    WHEN OTHERS.
      MESSAGE E000 WITH 'FILE UPLOAD ERROR, CHECK YOUR FILE!.'.
  ENDCASE.
ENDFORM.                    " UPLOAD_CODE_201_400
*&---------------------------------------------------------------------*
*&      Form  UPLOAD_CODE_401_600
*&---------------------------------------------------------------------*
FORM UPLOAD_CODE_401_600.
  CALL FUNCTION 'WS_UPLOAD'
       EXPORTING
...
*           FILELENGTH              =
  ...
  CASE SY-SUBRC.
    ...
    WHEN OTHERS.
      MESSAGE E000 WITH 'FILE UPLOAD ERROR, CHECK YOUR FILE!.'.
  ENDCASE.
ENDFORM.                    " UPLOAD_CODE_401_600
*&---------------------------------------------------------------------*
*&      Form  DELETE
*&---------------------------------------------------------------------*
FORM DELETE.
  DATA: LT_HPCDT TYPE ZTBM_ABXHPCDT OCCURS 0 WITH HEADER LINE.
  DATA: LT_HPCDT01 TYPE ZTBM_ABXHPCDT01 OCCURS 0 WITH HEADER LINE.
  SELECT *
       FROM ZTBM_ABXHPCDT
       INTO TABLE LT_HPCDT.
  SELECT *
       FROM ZTBM_ABXHPCDT
       INTO TABLE LT_HPCDT01.
DELETE ZTBM_ABXHPCDT FROM TABLE LT_HPCDT.
DELETE ZTBM_ABXHPCDT01 FROM TABLE LT_HPCDT01.

ENDFORM.                    " DELETE
*&---------------------------------------------------------------------*
*&      Form  DELETE_ABXHPCDT
*&---------------------------------------------------------------------*
FORM DELETE_ABXHPCDT.
  DATA: LT_HPCD TYPE ZTBM_ABXHPCDT OCCURS 0 WITH HEADER LINE.
  ...
  SELECT *
       ...
       WHERE SEQU EQ '0002'.
  IF SY-SUBRC EQ 0.
    DELETE ZTBM_ABXHPCDT FROM TABLE LT_HPCD.
  IF SY-SUBRC EQ 0.
    COMMIT WORK.
    ENDIF.
  ENDIF.
ENDFORM.                    " DELETE_ABXHPCDT
