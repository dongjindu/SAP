...
** : new entries logic change(10/24/2011 KDM01)
** : --> START
    DATA l_mandt TYPE mandt.

    SELECT single mandt INTO l_mandt
      FROM mkal
     WHERE matnr = IT_ALFP-MTNO
       AND werks = IT_ALFP-PLNT.
    ...
    ELSE.
      PERFORM DYNPRO USING:
         ...
         ' ' 'MKAL-VERID(01)' IT_ALFP-VERS,   "
         ' ' 'MKAL-TEXT1(01)' IT_ALFP-TEXT,   "
         ' ' 'MKAL-ADATU(01)' L_FVLD,   "
         ' ' 'MKAL-BDATU(01)' L_TVLD,   "
         ' ' 'BDC_OKCODE'  '=DETA'.
    ENDIF.
** : new entries logic change(10/24/2011 KDM01)
** : --> END
...
* Material number does not exist WRITE
  IF NOT LT_ALFP[] IS INITIAL.
    ...
    UPDATE ZTBM_ABYLFPDT FROM TABLE LT_ALFP.
    IF SY-SUBRC EQ 0.
      COMMIT WORK.
    ELSE.
      ROLLBACK WORK.
...
*&---------------------------------------------------------------------*
FORM UPDATE_PROCESS.
  UPDATE ZTBM_ABYLFPDT FROM TABLE IT_ALFP.
  IF SY-SUBRC EQ 0.
    COMMIT WORK.
  ELSE.
    ROLLBACK WORK.
