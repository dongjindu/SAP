...
*   UPDATE
    PERFORM UPDATE_ZTBM_ABXCFIDT TABLES LT_ACFI.
  ENDIF.
...
*&---------------------------------------------------------------------*
FORM UPDATE_PROCESS.
  UPDATE ZTBM_ABXCFIDT FROM TABLE IT_ACFI.
  IF SY-SUBRC EQ 0.
    COMMIT WORK.
  ELSE.
    ROLLBACK WORK.
...
*&---------------------------------------------------------------------*
FORM UPDATE_ZTBM_ABXCFIDT TABLES   PT_ACFI STRUCTURE IT_ACFI .
  UPDATE ZTBM_ABXCFIDT FROM TABLE PT_ACFI.
  IF SY-SUBRC EQ 0.
    COMMIT WORK.
  ELSE.
    ROLLBACK WORK.
...
*       ' ' 'BDC_OKCODE'  '/EBACK',
       ...
       'X' 'SAPLCUCO'    '0200',
       ' ' 'RCUCO-PRIO(02)' IT_ACFI-PRIT,   "
       ' ' 'RCUCO-PRFID(02)' IT_ACFI-PROF,   "
       ' ' 'RCUCO-KLART(02)' IT_ACFI-CLTY,   "
       ' ' 'BDC_OKCODE'  '/00',
       ...
       ' ' 'RCTMS-MWERT(03)' IT_ACFI-CLMI,  "
       ' ' 'RCTMS-MWERT(04)' IT_ACFI-CLO1,  "
       ' ' 'RCTMS-MWERT(05)' IT_ACFI-CLO2,  "
       ' ' 'RCTMS-MWERT(06)' IT_ACFI-CLO3,  "
       ' ' 'RCTMS-MWERT(07)' IT_ACFI-CLO4,  "
       ' ' 'BDC_OKCODE'  '=BACK',
...
FORM MM02_BDC_COLOR_EXT_INT.
  ...
  LOOP AT IT_ACFI WHERE ZRESULT NE 'E'.
    L_TABIX = SY-TABIX.
    PERFORM READ_MARA_COLOR_EXT_INT USING    IT_ACFI-MTNO+6(2)
                                    CHANGING L_COLOREXT
                                             L_COLORINT.
    PERFORM DYNPRO USING:
       'X' 'SAPLMGMM'    '0060',
