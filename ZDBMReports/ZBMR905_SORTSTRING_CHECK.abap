...
*     READ THE COMPONENTS FROM COMPONENT TABLES
      SELECT a~idnrk a~stlkn a~stlnr a~posnr a~suff
             ...
             c~stlan c~werks d~mtart a~aennr
...
*FOR TEST
  ...
  LOOP AT it_material.
*    i_count = i_count + 1.                      "for test only
    IF it_material-mtart = c_roh OR
       it_material-mtart = c_roh1 .
      it_material-sortf = '18'.
    ELSEIF it_material-mtart = c_halb.
      it_material-sortf = space.
    ENDIF.
    PERFORM fill_data USING it_material.
    PERFORM call_transaction USING 'CS02'.

*    IF i_count = 3.                           "for test only
*      EXIT.                                   "for test only
*    ENDIF.                                    "for test only
