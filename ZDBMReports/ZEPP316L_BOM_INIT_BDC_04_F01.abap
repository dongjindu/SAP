*----------------------------------------------------------------------*
*   INCLUDE ZEPP316L_BOM_INIT_BDC_04_F01                               *
*----------------------------------------------------------------------*
*&---------------------------------------------------------------------*
*&      Form  INITIALIZATION
*&---------------------------------------------------------------------*
...
*&---------------------------------------------------------------------*
FORM NON_COL_CHANGE_HEADER_BOM USING P_MTNO
                             P_PLNT
                             P_USAG
                             P_ALTN
                             P_EONO.
...
*&---------------------------------------------------------------------*
FORM   NON_COL_BADY_ITEM_CREATE1 USING P_PREF     "BOM item number
...
*&---------------------------------------------------------------------*
FORM NON_COL_READ_MAST_STPO_STLKN USING    P_MATNR
                                           P_WERKS
                                           P_STLAN
                                           P_STLAL
                                           P_POSNR
...
* MATERIAL & COMPENENT COLLECT
  LOOP AT IT_COLO WHERE CLPT EQ 'C'
                  AND   UPCT NE '2'.
...
*&---------------------------------------------------------------------*
FORM NON_COL_BOM_ITEM_DELETE USING    P_MTNO
                              P_PLNT
                              P_USAG
                              P_ALTN
                              P_EONO
...
* SORTING
  SORT IT_NCOL BY MATNR WERKS STLAN STLAL POSNR
                  IDNRK ZSUFF ZSEQU ZSEQC UPCT  AENNR.
* BDC PROCESS
  LOOP AT IT_NCOL.
    L_TABIX = SY-TABIX.
    CLEAR: L_STLKN, LA_NCOL.
    LA_NCOL = IT_NCOL.
*   UPDATE CONTROL TYPE
    IF LA_NCOL-UPCT EQ '2'.
*     BOM ITEM DELETE
      PERFORM NON_COL_READ_MAST_STPO_STLKN
                                   USING LA_NCOL-MATNR "Material
                                         LA_NCOL-WERKS "Plant
                                         LA_NCOL-STLAN "BOM usage
                                         LA_NCOL-STLAL "Alternative BOM
                                         LA_NCOL-POSNR "BOM item number
                                         LA_NCOL-IDNRK "BOM component
                                         LA_NCOL-ZSUFF "SUFFIX
                                         LA_NCOL-ZSEQU
                                         LA_NCOL-ZSEQC
                                CHANGING L_MODE
                                         L_STLKN.
      IF L_MODE EQ '3'. "DELETE(ITEM DELETE)
        IF NOT L_STLKN IS INITIAL.
          PERFORM NON_COL_BOM_ITEM_DELETE
                                  USING LA_NCOL-MATNR "Material number
                                        LA_NCOL-WERKS "Plant
                                        LA_NCOL-STLAN "BOM usage
                                        LA_NCOL-STLAL "Alternative BOM
                                        LA_NCOL-AENNR "Change number
                                        L_STLKN.      "Item node number

          PERFORM REFRESH_IT_BDC_IT_MESS.
        ENDIF.
...
*   BOM ITEM CREATE
    ELSEIF LA_NCOL-UPCT EQ '1'.
*     AT NEW STLAL
      AT NEW STLAL.
        L_TANEW = L_TABIX.
        L_MODE = '1'.
        IF L_MODE EQ '1'. "CHANGE(ITEM CREATE)
          PERFORM NON_COL_CHANGE_HEADER_BOM
                                    USING LA_NCOL-MATNR "Material number
                                          LA_NCOL-WERKS "Plant
                                          LA_NCOL-STLAN "BOM usage
                                          LA_NCOL-STLAL "Alternative BOM
...
*     'UPDATE CONTROL MODE' different BDC
      IF L_MODE EQ '1'.
        ...
        PERFORM NON_COL_BADY_ITEM_CREATE1
                            USING LA_NCOL-POSNR  "BOM item number
                                  LA_NCOL-IDNRK  "BOM compenent
                                  LA_NCOL-MENGE  "Compenent quantity
                                  LA_NCOL-POSTP  "Item category
                                  LA_NCOL-ITSOB  "procurementtype item
                                  LA_NCOL-ZEITM  "END ITEM TYPE
                                  LA_NCOL-ZSUFF  "SUFFIX NO
                                  LA_NCOL-ZSTGB  "STRUCTURE TYPE
                                  LA_NCOL-ZSEQU  "SEQUENCE NO
                                  LA_NCOL-ZSEQC  "COL.SEQUENCE
                                  LA_NCOL-ZUPGN  "UPG
                                  LA_NCOL-ZINFO  "INFO
                                  LA_NCOL-CLPT   "COLOR PART
                                  LA_NCOL-DPID   "DEPENDENCY ID
                                  LA_NCOL-MATNR "Material
                                  LA_NCOL-WERKS "Plant
                                  LA_NCOL-STLAN "BOM usage
                                  LA_NCOL-STLAL. "Alternative
...
*       AT NEW ZSEQC
        AT NEW ZSEQC.
          L_TANEW = L_TABIX.
          PERFORM COLOR_PART_CONTROL_1 USING    WA_COLO
                                       CHANGING L_MODE
                                                L_STLKN.
...
*&---------------------------------------------------------------------*
FORM APPEND_OBJECT_DEPENDENCY USING P_MTNO P_PLNT P_USAG P_ALTN P_EONO
                                    P_STLKN.
...
FORM COLOR_PART_CONTROL_1 USING    PA_COLO STRUCTURE WA_COLO
  ...
  IF P_MODE EQ '1'. "CHANGE(ITEM CREATE)
    PERFORM CHANGE_HEADER_BOM USING PA_COLO-MATNR "Material
                                    PA_COLO-WERKS "Plant
                                    PA_COLO-STLAN "BOM usage
                                    PA_COLO-STLAL "Alternative
                                    PA_COLO-AENNR. "Change NO

  ELSEIF P_MODE EQ '2'.
    PERFORM APPEND_OBJECT_DEPENDENCY USING
                                    PA_COLO-MATNR "Material
                                    PA_COLO-WERKS "Plant
                                    PA_COLO-STLAN "BOM usage
                                    PA_COLO-STLAL "Alternative
                                    PA_COLO-AENNR  "Change NO
...
FORM COLOR_PART_BADY_1 USING    PA_COLO STRUCTURE WA_COLO
  ...
  IF P_MODE EQ '1'.
    PERFORM CHANGE_BADY_ITEM_CREATE1
                        USING PA_COLO-POSNR  "BOM item number
                              PA_COLO-IDNRK  "BOM compenent
                              PA_COLO-MENGE  "Compenent quantity
                              PA_COLO-POSTP  "Item category
                              PA_COLO-ITSOB  "procurementtype item
                              PA_COLO-ZEITM  "END ITEM TYPE
                              PA_COLO-ZSUFF  "SUFFIX NO
                              PA_COLO-ZSTGB  "STRUCTURE TYPE
                              PA_COLO-ZSEQU  "SEQUENCE NO
                              PA_COLO-ZSEQC  "COL.SEQUENCE NO
                              PA_COLO-ZUPGN  "UPG
                              PA_COLO-ZINFO  "INFO
                              PA_COLO-CLPT   "COLOR PART
                              PA_COLO-DPID   "DEPENDENCY ID
                              PA_COLO-MATNR "Material
                              PA_COLO-WERKS "Plant
                              PA_COLO-STLAN "BOM usage
                              PA_COLO-STLAL. "Alternative
...
*&---------------------------------------------------------------------*
*&      Form  REFRESH_IT_BDC_IT_MESS
*&---------------------------------------------------------------------*
FORM REFRESH_IT_BDC_IT_MESS.
  REFRESH: IT_BDC, IT_MESS.
  CLEAR:   IT_BDC, IT_MESS.
ENDFORM.                    " REFRESH_IT_BDC_IT_MESS
