...
*
************************************************************************
REPORT  zipp302u_update_matl_mast LINE-SIZE 255.
INCLUDE <icon>.
...
FORM read_bom_structure.
  ...
  LOOP AT it_fsc.
    ...
    LOOP AT lt_stb.
      ...
      PERFORM read_sub_bom USING it_fsc-matnr w_stlan
                                 lt_stb it_9002.
    ENDLOOP.
  ENDLOOP.
...
FORM read_sub_bom USING pw_matnr pw_stlan
  ...
  SELECT SINGLE * FROM ztbm_ipp302u_tmp WHERE ernam = w_ernam
                                          AND erdat = w_erdat
                                          AND erzet = w_erzet
                                          AND matnr = pw_stb-idnrk
                                          AND werks = pw_bom-werks
                                          AND stlan = pw_stlan
  ...
  LOOP AT lt_stb.
    PERFORM check_n_append_it_9002 USING pw_stb-idnrk pw_stlan
                                         lt_stb.
    PERFORM read_sub_bom USING pw_stb-idnrk pw_stlan
                               lt_stb it_9002.
  ENDLOOP.
  IF sy-subrc NE 0.
    MOVE: c_end TO it_9002-node_type.

    UPDATE ztbm_ipp302u_tmp SET stlkz     = space
                                node_type = it_9002-node_type
     WHERE ernam = w_ernam
       AND erdat = w_erdat
       AND erzet = w_erzet
       AND matnr = pw_matnr
...
FORM check_n_append_it_9002 USING pw_matnr pw_stlan
                                  pw_stb   LIKE stpox.
  CLEAR: it_9002,ztbm_ipp302u_tmp.
...
*---// Sub BOM check
  ...
  IF sy-subrc EQ 0.
    ...
    IF it_9002-node_type IS INITIAL.
      MOVE: c_halb TO it_9002-node_type.
    ENDIF.
  ENDIF.
...
*  APPEND it_9002.
  SELECT SINGLE * FROM ztbm_ipp302u_tmp WHERE ernam = w_ernam
                                          ...
                                          AND werks = it_9002-werks
                                          AND stlan = it_9002-stlan
                                          AND stlal = it_9002-stlal
                                          AND idnrk = it_9002-idnrk.
  IF sy-subrc EQ 0.
    IF ztbm_ipp302u_tmp-kzbez EQ space AND
       it_9002-kzbez NE space.
      MOVE: it_9002-kzbez TO ztbm_ipp302u_tmp-kzbez.
      UPDATE ztbm_ipp302u_tmp.
    ENDIF.
  ELSE.
    CLEAR: ztbm_ipp302u_tmp.

    MOVE-CORRESPONDING it_9002 TO ztbm_ipp302u_tmp.

    MOVE: w_ernam TO ztbm_ipp302u_tmp-ernam,
          w_erdat TO ztbm_ipp302u_tmp-erdat,
          w_erzet TO ztbm_ipp302u_tmp-erzet.
    INSERT ztbm_ipp302u_tmp.
    IF sy-subrc NE 0.
      ROLLBACK WORK.
...
*  APPEND it_9002.
  ...
  ELSE.
    ...
    MOVE: w_ernam TO ztbm_ipp302u_tmp-ernam,
          w_erdat TO ztbm_ipp302u_tmp-erdat,
          w_erzet TO ztbm_ipp302u_tmp-erzet.
    INSERT INTO ztbm_ipp302u_tmp VALUES ztbm_ipp302u_tmp.
    IF sy-subrc NE 0.
      ROLLBACK WORK.
      MESSAGE e000(zz) WITH text-m05.
    ENDIF.
...
FORM append_it_9001 USING pw_matnr pw_werks     pw_normt
  ...
  CASE pw_node_type.
    ...
    WHEN c_halb.
      ...
      IF w_master-verkz EQ space.
        MOVE: 'X'      TO it_9001-beskz_n,
              '50'     TO it_9001-sobsl_n,
              'X'      TO it_9001-ncost_n.
      ENDIF.

      "Appended on 2005.03.17
      IF w_master-verkz EQ c_check OR
         w_master-beskz EQ 'E'.
        MOVE: it_9001-profl_m TO it_9001-profl_n,
              it_9001-normt_m TO it_9001-normt_n,
              it_9001-kzkfg_m TO it_9001-kzkfg_n,
              it_9001-dispo_m TO it_9001-dispo_n,
              it_9001-beskz_m TO it_9001-beskz_n,
              it_9001-sobsl_m TO it_9001-sobsl_n,
              it_9001-ncost_m TO it_9001-ncost_n.
...
FORM append_halb_to_it_9001 USING pw_color
  ...
  LOOP AT lt_9002.
    ...
    CHECK NOT ( ( it_9001-dispo_n   EQ 'M03' AND
                  it_9001-node_type EQ 'M'       ) OR
                it_9001-kzkfg_n     EQ 'X'         OR
                it_9001-node_type   EQ 'F'         ).

    PERFORM check_color_od USING lt_9002-idnrk lt_9002-werks pw_color
                                 lt_9002-kzbez lt_9002-node_type.

    PERFORM append_halb_to_it_9001 USING pw_color lt_9002.
  ENDLOOP.
...
FORM set_end_item.
  SELECT DISTINCT idnrk
    INTO TABLE it_end
    FROM ztbm_ipp302u_tmp
   WHERE   ernam     EQ w_ernam
     AND   erdat     EQ w_erdat
     AND   erzet     EQ w_erzet
     AND   node_type EQ c_end
      OR ( node_type IN (c_engine,c_tm) AND
           stlkz     EQ space ).

  SELECT * INTO CORRESPONDING FIELDS OF TABLE it_9002
    FROM ztbm_ipp302u_tmp
   WHERE ernam     EQ w_ernam
     AND erdat     EQ w_erdat
     AND erzet     EQ w_erzet.
