...
TABLES : TAPL.   "// Allocation of task lists to functional locations
...
FORM GENERATE_BDC_DATA.
  ...
  IF IT_TREC-TPLNR NE WA_PREV_TPLNR OR
    ...
    SELECT  SINGLE *
            FROM TAPL
            WHERE TPLNR = IT_TREC-TPLNR.
    IF SY-SUBRC EQ 0.
      WA_PREV_TPLNR = IT_TREC-TPLNR.
    ENDIF.

    IF IT_TREC-TPLNR EQ WA_PREV_TPLNR.
      PERFORM DYNPRO USING:
             'X' 'SAPLCPDI'   '3200',
             ' ' 'BDC_OKCODE'  '=ANLG'.
    ENDIF.
...
*         ' ' 'PLPOD-DAUNO(01)' IT_TREC-DAUNO,	
         ' ' 'PLPOD-ANZZL(01)'  IT_TREC-ANZZL,
         ' ' 'PLPOD-DAUNE(01)' IT_TREC-DAUNE,
...
*           ' ' 'PLPOD-LIFNR(01)' IT_TREC-LIFNR,
    ...
    ELSE.
      ...
      PERFORM DYNPRO USING:
             ...
             'X' 'SAPLSTXX'          '1100',
             ' ' 'BDC_OKCODE'	       '=TXBA',
             ' ' 'RSTXT-TXLINE(02)'	 WA_TEXT01,
             ' ' 'RSTXT-TXLINE(03)'	 WA_TEXT02,
             ' ' 'RSTXT-TXLINE(04)'  WA_TEXT03.
    ENDIF.
