*&----------------------------------------------------------------------
*& Development ID :
*& Program ID     : ZMMI01160T
*& Program Name   : Daily GR Quantity to G-pos
*& Created by     : T00304
...
*&======================================================================
*& RFC func. : Z_MM_IF_OB_10_DAILY_GR_QTY
*& Stru.     : ZMMS_GS003
*&----------------------------------------------------------------------

REPORT zmmi01160t MESSAGE-ID zmpp.

TABLES : mseg, mara, s001, mast, eord,
...
SELECT-OPTIONS :" s_werks FOR mseg-werks MEMORY ID wrk OBLIGATORY
                "                        DEFAULT 'P001' NO INTERVALS,
                 ...
                 s_budat FOR mseg-zbudat  NO-EXTENSION
  ...
  SELECT g~matnr
         ...
         FROM mseg AS g INNER JOIN mara AS a
                   ON a~mandt = g~mandt AND
                      a~matnr = g~matnr
         WHERE " g~werks    IN s_werks
               g~matnr    IN s_matnr
           AND g~zbudat   IN s_budat
...
*    MOVE  p_bukrs         TO it_data-bukrs.
    ...
    MOVE lt_mseg-zbudat   TO it_data-zbudat.
...
*  ENDIF.
  ...
  CALL FUNCTION 'Z_MM_IF_OB_10_DAILY_GR_QTY' DESTINATION v_dest
    IMPORTING
      e_return              = e_return
    TABLES
      t_data                = it_send
...
FORM save_log  USING    p_type p_msg1 p_msg2.
  ...
  DATA : lt_zmmt_gs003 LIKE zmmt_gs003 OCCURS 10 WITH HEADER LINE.
...
*// Current date max Seqence number.
  ...
  LOOP AT it_send.
    MOVE-CORRESPONDING it_send TO lt_zmmt_gs003.

    lt_zmmt_gs003-zdate = sy-datum.
    lt_zmmt_gs003-zseq  = l_zseq + sy-tabix.
    lt_zmmt_gs003-ztime = sy-uzeit.
    lt_zmmt_gs003-ernam = sy-uname.
    lt_zmmt_gs003-zrslt = p_type.
    IF p_type = 'E'.
      IF p_msg1 IS NOT INITIAL.
        lt_zmmt_gs003-zmsg  = p_msg1.
      ELSE.
        lt_zmmt_gs003-zmsg  = p_msg2.
      ENDIF.
    ENDIF.

    APPEND lt_zmmt_gs003.
    CLEAR : lt_zmmt_gs003.

  ENDLOOP.

  INSERT zmmt_gs003 FROM TABLE lt_zmmt_gs003
