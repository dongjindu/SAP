************************************************************************
* Program Name      : ZEMMPM10E_STL_HOURLY_NEW
* Author            : Sung-Tae, Lim
* Creation Date     : 2004.04.23.
* Specifications By : Sung-Tae, Lim
* Development Request No : UD1K909855
* Addl Documentation:
* Description       : Copy of ZEMMPM10E_STL_HOURLY06
*
* Modification Logs
* Date            Developer        RequestNo      Description
* 08/23/06        Manju            UD1K921877    Reprocessing Options
* 08/01/07        IG.MOON          UD1K941196    Add hint for query
************************************************************************

REPORT ZEMMPM10E_STL_HOURLY_NEW NO STANDARD PAGE HEADING
                        LINE-SIZE 400
                        MESSAGE-ID zmmm.
...
parameters: P_nor radiobutton group r1 default 'X' USER-COMMAND ucom,
            p_P1  radiobutton group r1 ,
            p_p2  radiobutton group r1 .
SELECTION-SCREEN END OF BLOCK blk2.


SELECTION-SCREEN BEGIN OF BLOCK blk3 wITH FRAME TITLE text-005.
parameters : p_cdate1 LIKE ztmm_stl_log-sdate MODIF ID RP ,
             p_ctime1 LIKE sy-uzeit MODIF ID RP .
...
* Supress  Earlier Date & time on Selection screen
* Initially.
  if p_p2 eq ''.
    loop at screen.
      IF SCREEN-GROUP1 = 'RP'.
...
* Display/don't display Earlier Date & Time Depending on the
* Option choosen.
  if p_p2 eq 'X'.
    loop at screen.
      IF SCREEN-GROUP1 = 'RP'.
...
* When Earlier Date is entered for Option 2 Query
* log table to display possible earlier Run time sorting
* In descending Order

AT SELECTION-SCREEN ON VALUE-REQUEST FOR p_ctime1.
  DATA : BEGIN OF value_tab OCCURS 0,
           d1  like ZTMM_STL_LOG-sdate,
           t1  like ztmm_stl_time-stime,
          END OF value_tab.
DATA: wa_datum LIKE sy-datum.

  dynpfields-fieldname = 'P_CDATE1'.
  APPEND dynpfields.
  l_dyname = sy-repid.
  l_dynumb = '1000'.
  ...
  if sy-subrc eq 0.
    CALL FUNCTION 'CONVERT_DATE_TO_INTERNAL'
         EXPORTING
              DATE_EXTERNAL = dynpfields-FIELDVALUE
         IMPORTING
              DATE_INTERNAL = wa_datum.
  endif.

  if not wa_datum is initial.
* Query Log table for possible Run time for given date
    select distinct sdate feeding_time from ZTMM_STL_LOG
               into table value_tab
                where SDATE eq wa_datum.
    sort value_tab  by t1 descending.

* Set F4 values for Earlier Run time based on Run Date
    CALL FUNCTION 'F4IF_INT_TABLE_VALUE_REQUEST'
         EXPORTING
              retfield        = 'T1'
              dynpprog        = l_dyname
              dynpnr          = '1000'
              dynprofield     = 'P_CTIME1'
              window_title    = 'Time from LOG table for given Table '
              value_org       = 'S'
         TABLES
              value_tab       = value_tab
              RETURN_TAB      = T_RETURN
...
AT SELECTION-SCREEN.
  if   p_p2  eq 'X' .
    if P_CDATE1 is initial and SSCRFIELDS-UCOMM eq 'ONLI'.
      MESSAGE e000(zz) WITH text-A12.
      stop.
    endif.
    if P_Ctime1 is initial and SSCRFIELDS-UCOMM eq 'ONLI'.
      MESSAGE e000(zz) WITH text-A13.
      stop.
    endif.
  endif.
...
* Normal Run or Option 1 is choosen
  if p_p2 eq '' .
...
*  Option2 is choosen
* If Option2 is choosen that means  No ALC data and therefore no BF data
* Read from Log Table for Given Earlier Date and Time
    perform read_log_data.
   IF not it_log[] IS INITIAL.
    perform create_to_by_option2.
    PERFORM update_table.
    PERFORM comment_build.     " USING w_top_of_page[].
    PERFORM make_alv_grid.
  endif.
...
* UD1K941196 - by IG.MOON 08/01/2007 {
...
* If Option2 is choosen that means No ALC data and therefore no BF data
* Read from Log table for earlier date & time instead of Vehicle Master
  if p_p2 eq '' .
...
*if Option 1 is choosen ( ALC Good but no Backflush Qty - So Skip )
  if  P_P1 eq '' .
...
*** added by furong on 28/07/2005, checking rounding qty by storage type
...
*---
  if p_P1  eq 'X' .      "Option 1 is choosen
* ALC Good but no BF data -
* LT01 & LT1A is called since in LT01 start date & time can't be update.
* So in change mode ( LT1A)  the same is changed
    perform create_to_by_option1.
  else.                  " Normal Run
* Normal / Regular Run Option is choosen
...
* For Option Normal Run & Option 1 consider Date & time entered
  if p_p2 eq '' .
    CONCATENATE p_cdate p_ctime INTO lw_current.
  else.
* If Option 2 is choosen Pass System Date & Time
    CONCATENATE sy-datum sy-uzeit INTO lw_current.
  endif.
  ...
  ENDLOOP.
  check  p_p2 eq '' .  " For Option 2 Ignore Time Validation
  IF sy-subrc NE 0.
...
* TO Qty
    ...
    ELSE.
      lv_tqty = it_toline-bdmng.
    ENDIF.
* Applying Rounding Qty to TO Qty
    Perform applying_rounding_qty.
...
*---
  ...
  endloop.
ENDFORM.                    " create_to_by_option1
*&---------------------------------------------------------------------*
*&      Form  applying_rounding_qty
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
...
*----------------------------------------------------------------------*
FORM read_log_data.
* Select Data from Log table
  SELECT * INTO CORRESPONDING FIELDS OF TABLE it_log
             FROM ztmm_stl_log
            WHERE sdate            eq p_cdate1
              AND feeding_time     eq p_ctime1
...
ENDFORM.                    " read_log_data
*&---------------------------------------------------------------------*
*&      Form  create_to_by_option2
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
...
FORM create_to_by_option2.
  ...
  CLEAR : it_itab, it_itab[], it_dispo, it_dispo[].
* For Option 2
  w_screen_date  =  sy-datum.
  w_ending_date  = sy-datum.
  w_starting_time = sy-uzeit.
  w_ending_time = w_starting_time + 1. "Add 1 hr to run time
  loop at it_log.
    move-corresponding it_log to it_toline.
    CLEAR : l_stats.
    MOVE : sy-tabix TO l_tabix.
*--- TO GET Start Date(Feeding Date) and Start Time
    MOVE : w_screen_date   TO it_toline-sdate,
...
*  TO Qty
    it_toline-tqty = it_log-TQTY.   "TO Qty

* Material   Number
    it_toline-matnr = it_log-matnr.

* Feeder
    it_toline-feedr  = it_log-FEEDR.
...
*--- Get Destination Storage type/bin
    ...
    IF it_log-DES_LGTYP is initial and it_log-DES_LGPLA is initial.
      MOVE : 'XXX'         TO it_toline-des_lgtyp,
             'XXXXXXXXXX'  TO it_toline-des_lgpla.
    else.
      MOVE : it_log-des_lgtyp TO it_toline-des_lgtyp,
             it_log-des_lgpla TO it_toline-des_lgpla.
    ENDIF.
...
* Default Zero's to all Qty fields.
    it_itab-bdmng     = 0.
    it_itab-verme  = 0.
    it_itab-lpmin  = 0.
    it_itab-open_to  = 0.
    it_itab-bferrorqty = 0.
    it_itab-bf_to_qty = 0.
    it_itab-rdmng = 0.
