************************************************************************
* Program Name      : ZEMMPM10E_STL_HOURLY06_TEST
* Author            : Sung-Tae, Lim
...
*                     ( Copy of ZEMMPM10E_STL_HOURLY06 )
* Modification Logs
* Date            Developer        RequestNo      Description
* 08/15/2006      Manju            UD1K921765     Initial coding
* 08/17/2006      manju            UD1K921784     Program changes
************************************************************************
...
*   PERFORM update_table.  - Commented -  UD1K921765
    PERFORM comment_build.     " USING w_top_of_page[].
    MESSAGE s999(zmmm) WITH 'Start: ALV Display'.
    PERFORM make_alv_grid.
    MESSAGE s999(zmmm) WITH 'END: ALV Display'.
...
data :lt_ppc_head_temp type standard table of ppc_head with header line.
...
* Based on ZTPP_PPC_HEAD & ZTPP_PPC1_ALL_2
  elseif P2 eq 'X'.
    delete from ztpp_ppc_head where HEADID ne ''.
    select  *  into table lt_ppc_head_temp
              from ppc_head
              where flg_asynch ne 'X'.

    insert ztpp_ppc_head from table lt_ppc_head_temp.
    commit work.

* Select  from ZTMM_PPC1_ALL_2   VIEW
    SELECT matnr
          werks
          lgort
          komp_quant     " quantity
...
*---
  ...
  MESSAGE s999(zmmm) WITH 'Start: Fetch Vehicle Master'.
  PERFORM get_vehicle_master.
  MESSAGE s999(zmmm) WITH 'End:  fetch Vehicle Master'.
...
*--- get scheduled backflush quantity
  MESSAGE s999(zmmm) WITH 'Start: Get Sch Backflush Qty'.
  PERFORM get_scheduled_bf_qty.
  MESSAGE s999(zmmm) WITH 'END: Get Sch Backflush Qty'.


*--- get current stock
  MESSAGE s999(zmmm) WITH 'Start: Get Current Stock'.
  PERFORM get_verme_lpmin.
  MESSAGE s999(zmmm) WITH 'END: Get Current Stock'.


*--- get open TO quantity
  MESSAGE s999(zmmm) WITH 'Start: Get OPEN TO Qty'.
  PERFORM get_open_to.
  MESSAGE s999(zmmm) WITH 'END: Get OPEN TO Qty'.



*--- get BackFlush error quantity(/nCOGI)
  MESSAGE s999(zmmm) WITH 'Start: Get Backflush Error Qty'.
  PERFORM get_bf_error_qty.
  MESSAGE s999(zmmm) WITH 'END: Get Backflush Error Qty'.


*** added by furong on 28/07/2005, checking rounding qty by storage type
  MESSAGE s999(zmmm) WITH 'Start: Get Source Type'.
  PERFORM get_source_type.
  MESSAGE s999(zmmm) WITH 'END: Get Source Type'.

*** end of addition
*--- get rounding value
  MESSAGE s999(zmmm) WITH 'Start: Get Rounding Value'.
  PERFORM get_rdmng.
  MESSAGE s999(zmmm) WITH 'END: Get Rounding Value'.
...
*--- get destination storage type/bin
  MESSAGE s999(zmmm) WITH 'Start: Get Storage type'.
  PERFORM get_des_storage_type_bin.
  MESSAGE s999(zmmm) WITH 'END: Get Storage type'.


*---
*   PERFORM create_transfer_order.   Commented -  UD1K921765
