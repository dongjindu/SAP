REPORT ZMMIFTZC_MULTI_SHIP_COUNTRY.
INCLUDE ZMMFZTOP.
INCLUDE ZMMCONTP.
*&--------------------------------------------------------------------&*
*&    Program: ZMMIFTZC.                                              &*
...
*& 05/16/2008  Furong   UD1K943660      CA Ship to NJ
*&--------------------------------------------------------------------&*

PARAMETERS: P_DATE   LIKE SY-DATUM OBLIGATORY.
** Change by Furong on 04/05/10
** Added by Furong on 03/01/10
*SELECT-OPTIONS: S_TIME FOR SY-UZEIT NO-EXTENSION.
** End of change
** End of change on 04/05/10
...
PARAMETERS: P_US RADIOBUTTON GROUP GRP1,
            P_OTH RADIOBUTTON GROUP GRP1.

** changed by Furong on 04/07/09
*R_DT-SIGN = 'I'.
*R_DT-OPTION = 'BT'.
...
** Changed by Furong on 03/01/10
R_DT_SHOP-SIGN = 'I'.
R_DT_SHOP-OPTION = 'BT'.
W_DATE_TIME_SHOP = P_DATE.
R_DT_SHOP-LOW = W_DATE_TIME_SHOP.
R_DT_SHOP-HIGH = W_DATE_TIME_SHOP.
APPEND R_DT_SHOP.
CLEAR: R_DT_SHOP, W_DATE_TIME.
**End of change on 03/01/10
...
*                         OR    ATNAM = 'P_RP25_ACTUAL_DATE'
*                         OR    ATNAM = 'P_RP27_ACTUAL_DATE'
                         OR    ATNAM = 'P_RP25_SHOP_DATE'
                         OR    ATNAM = 'P_RP27_SHOP_DATE'
...
** Changed by Furong on 04/05/10
*              AND ATWRT IN R_DT
              AND ATFLV IN R_DT_SHOP
** End of change on 04/05/10
*              AND ATFLV IN R_DT_SHOP

** End of chnage on 03/10/10
          OR  ATINN = W_ATINN7
              AND ADZHL = W_ADZHL7
** Changed by Furong on 04/07/09
*              AND ATWRT IN R_DT ).
...
** end of change on 03/12/08
    W_DEST = W_DESTN+0(3).
    IF P_US = 'X' AND W_DEST = 'B28' OR P_OTH = 'X' AND W_DEST <> 'B28'.
    ...
    CLEAR: WA_EQUI_INFO, W_MODYR, W_DESTN, W_MODIDX, W_OCN, W_FSC,
           W_LEN, W_ODEALER, W_NDEALER, W_DEST.
  ENDAT.
ENDLOOP.
...
IF NOT P_PLNUM IS INITIAL.
  ...
  ENDIF.
ENDIF.
** Changed by Furong on 05/21/08 ; UD1K943685
IT_EQUI_PLAN_INFO[] = IT_EQUI_INFO[].
** End of change  ; UD1K943685
...
** Changed by Furong on 05/16/08 ; UD1K943660
*  CONCATENATE 'SHIPCAR' P_DATE INTO WA_ZTMM_6026_01-ORDERNUMSHIP.
** End of change ;UD1K943660
...
** Changed by Furong on 05/16/08 ; UD1K943660
  IF WA_VBPA-LAND1 = 'CA'.
    READ TABLE IT_EQUI_PLAN_INFO WITH KEY PLNUM = <FS_PLANORD>-PLNUM.

    CLEAR: LW_LIKP.
    SELECT SINGLE * INTO LW_LIKP
      FROM LIKP
      WHERE VBELN = IT_EQUI_PLAN_INFO-EQUNR.
    IF LW_LIKP-ROUTE = 'FAPNJ'.
      CONCATENATE 'SHIPCAR' P_DATE 'NJ'
                INTO WA_ZTMM_6026_01-ORDERNUMSHIP.
    ELSE.
      CONCATENATE 'SHIPCAR' P_DATE INTO WA_ZTMM_6026_01-ORDERNUMSHIP.
    ENDIF.
    CLEAR: IT_EQUI_PLAN_INFO.
  ELSE.
    CONCATENATE 'SHIPCAR' P_DATE INTO WA_ZTMM_6026_01-ORDERNUMSHIP.
  ENDIF.
** End of change ;UD1K943660
