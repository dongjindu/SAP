...
*                   from vbfa g, likp f, lips e, eket c,
*                        ekpo b, ekko a, marc d
...
*                    and b.mandt = e.mandt
*                    and b.ebeln = e.vgbel
*                    and substr(b.ebelp,1,5) = substr(e.vgpos,2,5)
*                    and f.vbeln between :w_vbeln_s and :w_vbeln_e
*                    and e.mandt = f.mandt
*                    and e.vbeln = f.vbeln
*                    and g.vbeln(+) between :w_mblnr_s and :w_mblnr_e
*                    and g.vbtyp_n(+) = 'R'
*                    and g.mandt(+) = e.mandt
*                    and g.vbelv(+) = e.vbeln
*                    and g.posnn(+) = e.posnr
...
**                        ekpo b, ekko a, marc d
    SELECT A.LIFNR, A.EBELN,     A.BSART, A.BEDAT, J.NAME1,
           ...
           H.USNAM, C.WEMNG,     I.POSNN
                  ...
                  WHERE A.MANDT = :SY-MANDT
*                    AND A.AEDAT BETWEEN :W_BUDAT_S AND :W_BUDAT_E
                    AND A.BSTYP IN ( 'F', 'L' )
                    AND A.BSART BETWEEN :W_BSART_S AND :W_BSART_E
                    AND A.LIFNR BETWEEN :W_LIFNR_S AND :W_LIFNR_E
                    AND A.EBELN BETWEEN :W_EBELN_S AND :W_EBELN_E
...
*                    AND I.MANDT(+)    = g.MANDT
*                    AND I.VBELN(+)    = g.MBLNR
*                    AND I.POSNN(+)    = CONCAT('00',g.ZEILE)
*                    AND I.VBTYP_N(+) = 'R'
  ENDEXEC.

*  SORT it_temp BY lifnr.
*
*  LOOP AT it_temp.
**--
*    MOVE-CORRESPONDING it_temp TO it_itab.
...
*
**---
*  SORT it_itab BY ebeln ebelp lifnr.
