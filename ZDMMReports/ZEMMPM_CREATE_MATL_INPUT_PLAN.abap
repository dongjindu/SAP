...
FORM get_wbs_prj_vehicle.
  ...
  LOOP AT lt_input_plan.
    IF lt_input_plan-mitu EQ 'Y'.
      DELETE lt_input_plan. CONTINUE.
    ENDIF.

    IF lt_input_plan-work_order+12(2) = 'XX'.
      DELETE lt_input_plan. CONTINUE.
    ENDIF.
  ENDLOOP.
  ...
  LOOP AT lt_input_plan.
    ...
    IF lt_input_plan-work_order+12(2) = 'XX' OR
       lt_input_plan-work_order+12(2) = 'XY'.
      DELETE lt_input_plan. CONTINUE.
    ENDIF.
  ENDLOOP.
...
** SHOULD READ THE DATA OF TWO SHIFT TO
** CALCULATE THE UPH.
** FOR ONE SHIFT, MAY HAVE MORE THAN ONE RECORD FOR
** DIFFERENT RATE DURING DIFFERENT PERIOD OF TIME
*
...
FORM get_worktime1 USING pa_wdate pa_cnt pa_wktime pa_day
  ...
  LOOP AT lt_capa WHERE datub = l_date AND tagnr = pa_day .
    ...
    CONCATENATE it_shift-date it_shift-ftime INTO it_shift-daytime.
    APPEND it_shift.
  ENDLOOP.
...
*  <--  p2        text
*----------------------------------------------------------------------*
FORM get_input_plan USING pw_rp pw_shop.
  ...
  LOOP AT it_data.
    ...
    SELECT SINGLE * FROM plaf WHERE plnum = it_data-plnum.
    IF sy-subrc NE 0.
      PERFORM read_plan_order_from_vehicle USING it_data.
    ENDIF.
...
FORM set_dvrt_866_per_shop USING pw_rp pw_shop.
  ...
  PERFORM get_input_plan USING pw_rp pw_shop.
  ...
  LOOP AT it_mitu.
    ...
    IF sy-subrc NE 0.
      MOVE: w_datum_f          TO it_dvrt-erdat,
            ...
            '9'                TO it_dvrt-tprog,
            it_mitu-work_order TO it_dvrt-wkord,
            it_mitu-status     TO it_dvrt-rpsts,
            sy-uzeit           TO it_dvrt-uzeit,
            sy-uname           TO it_dvrt-ernam.

      SELECT SINGLE * FROM plaf WHERE plnum = it_mitu-plnum.
      IF sy-subrc NE 0.
        PERFORM read_plan_order_from_vehicle USING it_mitu.
      ENDIF.
...
FORM get_material_requirement.
  EXEC SQL PERFORMING APPEND_MATERIAL_REQUIREMENT.
    SELECT /*+ ORDERED+*/
           ...
           A.WKORD, A.P_MODEL,  A.P_BODY_SERIAL
      INTO :W_REQUIREMENT
...
FORM set_time_zone.
  ...
  MOVE: lw_date_time+8(6) TO lw_time.

  LOOP AT it_shift WHERE arbpl =  lw_shop
                     AND daytime <= lw_date_time.
  ENDLOOP.
...
*&---------------------------------------------------------------------*
*&      Form  read_plan_order_from_vehicle
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
*  <--  p2        text
*----------------------------------------------------------------------*
FORM read_plan_order_from_vehicle USING pw_data LIKE it_data.
  DATA: lw_atinn LIKE cabn-atinn.

  CONCATENATE pw_data-modl pw_data-body_ser INTO pw_data-objek.
  ...
  SELECT SINGLE atwrt
    INTO pw_data-plnum
    FROM ausp
   WHERE objek = pw_data-objek
     AND atinn = lw_atinn
     AND klart = '002'.
ENDFORM.                    " read_plan_order_from_vehicle
