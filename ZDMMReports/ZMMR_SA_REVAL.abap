...
*    SELECT DMBTR GJAHR MENGE SHKZG REEWR
*           XBLNR
*    INTO CORRESPONDING FIELDS OF TABLE LT_EKBE
*    FROM EKBE
*    WHERE EBELN = LT_ITAB-EBELN
*      AND EBELP = LT_ITAB-EBELP
*      AND BUDAT BETWEEN LT_ITAB-EXCFDATE AND LT_ITAB-EXCTDATE
*      AND BEWTP ='E'
*      AND BWART = '101'.
*    IF SY-SUBRC = 0.
*      LOOP AT LT_EKBE.
*        LT_TEMP-DMBTR = LT_EKBE-DMBTR.
*        LT_TEMP-MENGE = LT_EKBE-MENGE.
*        LT_TEMP-SHKZG = LT_EKBE-SHKZG.
*        LT_TEMP-REEWR = LT_EKBE-REEWR.
*        LT_TEMP-PRICE = LT_ITAB-PEINH * LT_EKBE-DMBTR / LT_EKBE-MENGE.
*        APPEND LT_TEMP.
*      ENDLOOP.
*      SORT LT_TEMP BY PRICE.
*      READ TABLE LT_TEMP INDEX 1.
*      L_PRICE = LT_TEMP-PRICE.
*      LOOP AT LT_TEMP.
*        IF L_PRICE = LT_TEMP-PRICE.
*          IF LT_TEMP-SHKZG = 'H'.
*            L_GRQTY = L_GRQTY - LT_TEMP-MENGE.
*            L_GRAMT  = L_GRAMT  - LT_TEMP-DMBTR.
*          ELSE.
*            L_GRQTY = L_GRQTY + LT_TEMP-MENGE.
*            L_GRAMT  = L_GRAMT  + LT_TEMP-DMBTR.
*          ENDIF.
...
*          APPEND IT_ITAB.
*          L_PRICE = LT_TEMP-PRICE.
*          CLEAR: L_GRQTY, L_GRAMT, L_NEWAMT.
*        ENDIF.
*      ENDLOOP.
*      IF L_GRQTY > 0.
*        IT_ITAB-GRPRICE = L_PRICE.
