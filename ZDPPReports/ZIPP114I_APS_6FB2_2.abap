************************************************************************
* Program Name           : ZIPP114I_APS_6FB2
...
* Description       : Transportation of Plan Quantity per ALC Code
*----------------------------------------------------------------------
* Modification Logs
* Date       Developer    RequestNo    Description
*09/22/2008 Haseeb UD1K944561  Copied from old version for APS2.
************************************************************************
REPORT zipp114i_aps_6fb2 MESSAGE-ID zMpp
...
DATA: it_pmt06fb            LIKE TABLE OF ztpp_pmt06fb WITH HEADER LINE,
      it_pms06fb            LIKE TABLE OF zspp_pmt06fb WITH HEADER LINE.
...
SELECTION-SCREEN COMMENT  (75) text-100 FOR FIELD p_run.
...
FORM excute_process.
  DATA: l_msgtxt(100),
        ...
        l_count(3)  TYPE n   .
*
  REFRESH: it_pmt06fb. CLEAR it_pmt06fb.

  SELECT * FROM  ztpp_pmt06fb
     INTO CORRESPONDING FIELDS OF TABLE  it_pmt06fb
         WHERE zresult EQ 'E' OR zresult EQ space.

  IF sy-subrc EQ 0.
*   The Total Number of Data to be transported
    DESCRIBE TABLE it_pmt06fb LINES l_size.
...
*      l_count = l_count + 1.
      ...
      MOVE-CORRESPONDING it_pmt06fb TO it_pms06fb.
      APPEND it_pms06fb.
      IF l_count >= l_size.
        l_curr = l_curr + 1.
        CALL FUNCTION 'Z_FPP_SET_PMT06FB_2'
             DESTINATION  'WMPP01'
             EXPORTING
               total_c               = l_idx
               curr_c                = l_curr
        ...
        IF sy-subrc <> 0.
          ...
          wa_error = 'X' .          EXIT.
        ELSE.
          LOOP AT it_pms06fb WHERE zzret = 'E'.
            wa_error = 'X' .
            PERFORM create_log USING 'E' 1  text-001  l_msgtxt .
            PERFORM create_log USING 'R' 1  text-001  l_msgtxt .
          IF sy-batch = ' '.
