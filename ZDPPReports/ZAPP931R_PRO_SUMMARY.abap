...
** changed by Furong, requested by Ignacio on 09/06/2005
    ...
    SORT it_vehicle BY atnam.
*    LOOP AT it_vehicle WHERE atnam = 'P_RP27_SHOP_DATE'.
*      IF it_vehicle-atwrt <> space.
...
*  ENDLOOP.
  l_count = 1.
  SORT it_vehi BY objek.
  SORT it_vehicle BY objek.
  LOOP AT it_vehi.
    LOOP AT it_vehicle FROM l_count.
      IF it_vehicle-objek EQ it_vehi-objek.
        PERFORM extract_value_model USING it_vehicle-atnam
                                       it_vehicle-atwrt c_date.
      ELSE.
        IF it_vehicle-objek > it_vehi-objek.
          l_count = sy-tabix.
          EXIT.
        ENDIF.
      ENDIF.
...
* changed by furong from 23 to 24   " 24 fo truck
  PERFORM vpcwip_vpc USING '24' '24' 'T'.
* changed by furong from 25-27 to 26 " 26 for railing
  PERFORM vpcwip_vpc USING '26' '26' 'R'.
** end of change
