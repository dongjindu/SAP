************************************************************************
* Program Name      : ZIPP105I_PMT03FB_RFC
* Author            : Bobby
...
************************************************************************
REPORT zipp105i_pmt03fb_rfc NO STANDARD PAGE HEADING
*                          LINE-SIZE 120
                          MESSAGE-ID zmpp.
*----------------------------------------------------------------------*
...
*----------------------------------------------------------------------*
DATA : it_pms03fb   LIKE TABLE OF zspp_pmt03fb WITH HEADER LINE,
       it_pmt03fb   LIKE TABLE OF ztpp_pmt03fb WITH HEADER LINE.
...
FORM excute_process.
  ...
  CLEAR : it_pmt03fb, it_pmt03fb[].
  SELECT *
         INTO CORRESPONDING FIELDS OF TABLE it_pmt03fb
         FROM ztpp_pmt03fb
         WHERE zresult EQ 'E' OR zresult EQ space.

  IF sy-subrc EQ 0.
    DESCRIBE TABLE it_pmt03fb LINES l_size.
*    l_idx = l_size DIV c_count .
    l_idx = l_idx + 1          .          " Total RFC Call Count
    LOOP AT it_pmt03fb.
*      l_count = l_count + 1.

      MOVE-CORRESPONDING it_pmt03fb TO it_pms03fb.
      APPEND it_pms03fb.
      IF l_count >= l_size.
        l_curr = l_curr + 1.
        CALL FUNCTION 'Z_FPP_SET_PMT03FB_2'
          DESTINATION  c_dest
              EXPORTING
            total_c              = l_idx
            curr_c               = l_curr
          TABLES
            i_ztpp_pmt03fb       = it_pms03fb
       EXCEPTIONS
        ...
        IF sy-subrc <> 0.
          ...
          wa_error = 'X' .          EXIT.
        ELSE.
          LOOP AT it_pms03fb WHERE zzret = 'E'.
            wa_error = 'X' .
            PERFORM create_log USING 'E' 1  text-001  l_msgtxt .
            PERFORM create_log USING 'R' 1  text-001  l_msgtxt .
          IF sy-batch = ' '.
