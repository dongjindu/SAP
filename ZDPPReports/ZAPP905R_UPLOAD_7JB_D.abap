************************************************************************
* Program Name      : ZAPP904R_UPLOAD_7JB_D
* Author            : Bobby
...
* Addl Documentation:
* Description       : 'Upload the data for the table ZTPP_PMT07JB_D
*
* Modification Logs
* Date       Developer    RequestNo    Description
************************************************************************
REPORT ZAPP904R_UPLOAD_7JB_D      NO STANDARD PAGE HEADING
...
*----------------------------------------------------------------------*
TABLES: ZTPP_PMT07JB_D.

*----------------------------------------------------------------------*
* INTERNAL-TABLE AREA
*----------------------------------------------------------------------*
DATA: BEGIN OF IT_AEHD OCCURS 0,
        WERKS     TYPE ZTPP_PMT07JB_D-WERKS,
        PBDNR     TYPE ZTPP_PMT07JB_D-PBDNR,
        MATNR     TYPE ZTPP_PMT07JB_D-MATNR,
        PDATU(10) TYPE C                   ,
        COGUB     TYPE ZTPP_PMT07JB_D-COGUB,
        INEXC     TYPE ZTPP_PMT07JB_D-INEXC,
        PLNMG(10) TYPE C                   ,
        PVER      TYPE ZTPP_PMT07JB_D-PVER ,
      END OF IT_AEHD.
...
*&---------------------------------------------------------------------*
*&      Form  UPDATE_ZTPP_PMT07JB_D
*&---------------------------------------------------------------------*
FORM UPDATE_ZTPP_PMT07JB_D.
  DATA LT_AEHD LIKE ZTPP_PMT07JB_D OCCURS 0 WITH HEADER LINE.
  LOOP AT IT_AEHD.
...
*-> Upload after all deletion
    WHEN R1.
      DELETE FROM ZTPP_PMT07JB_D
             CLIENT SPECIFIED
             WHERE MANDT EQ SY-MANDT.
      INSERT ZTPP_PMT07JB_D FROM TABLE LT_AEHD ACCEPTING DUPLICATE KEYS.
      IF SY-SUBRC EQ 0.
        COMMIT WORK.
      ELSE.
        ROLLBACK WORK.
...
*-> Upload except deletion
    WHEN R2.
      MODIFY ZTPP_PMT07JB_D FROM TABLE LT_AEHD.
      IF SY-SUBRC EQ 0.
        COMMIT WORK.
      ELSE.
        ROLLBACK WORK.
