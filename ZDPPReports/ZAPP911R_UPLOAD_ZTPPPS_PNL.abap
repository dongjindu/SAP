************************************************************************
* Program Name      : ZAPP911R_UPLOAD_ZTPPPS_PNL
* Author            : Bobby
...
* Addl Documentation:
* Description       : 'Upload the data for the table ZTPPPS_PNL
*
* Modification Logs
* Date       Developer    RequestNo    Description
************************************************************************
REPORT ZAPP911R_UPLOAD_ZTPPPS_PNL NO STANDARD PAGE HEADING
...
DATA: BEGIN OF IT_AEHD OCCURS 0,
        ...
        MATNR(18)        TYPE C             ,
        PRS_PNL_NW(8)    TYPE C             ,
        PRS_PNL_VMDL(4)  TYPE C             ,
        MAKTX(40)        TYPE C             ,
        PRS_PNL_SPEC(50) TYPE C             ,
        PRS_PNL_BLKN(18) TYPE C             ,
        PRS_PNL_DIEN(18) TYPE C             ,
        PRS_PNL_PLTN(18) TYPE C             ,
        PRS_PNL_OHQ(13)  TYPE C             ,
        PRS_PNL_TQP(13)  TYPE C             ,
        MEINS(03)        TYPE C             ,
        EFLAG(02)        TYPE C             ,
        FLAG(001)        TYPE C             ,
      END OF IT_AEHD.
...
*&---------------------------------------------------------------------*
*&      Form  UPDATE_ZTPPPS_PNL
*&---------------------------------------------------------------------*
FORM UPDATE_ZTPPPS_PNL    .
  DATA LT_AEHD LIKE ZTPPPS_PNL     OCCURS 0 WITH HEADER LINE.
  LOOP AT IT_AEHD.
...
*-> Upload after all deletion
    WHEN R1.
      DELETE FROM ZTPPPS_PNL
             CLIENT SPECIFIED
             WHERE MANDT EQ SY-MANDT.
      INSERT ZTPPPS_PNL     FROM TABLE LT_AEHD ACCEPTING DUPLICATE KEYS.
      IF SY-SUBRC EQ 0.
        COMMIT WORK.
      ELSE.
        ROLLBACK WORK.
...
*-> Upload except deletion
    WHEN R2.
      MODIFY ZTPPPS_PNL     FROM TABLE LT_AEHD.
      IF SY-SUBRC EQ 0.
        COMMIT WORK.
      ELSE.
        ROLLBACK WORK.
