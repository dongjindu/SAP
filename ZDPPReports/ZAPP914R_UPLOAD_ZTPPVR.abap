************************************************************************
* Program Name      : ZAPP914R_UPLOAD_ZTPPVR
* Author            : Bobby
...
* Addl Documentation:
* Description       : 'Upload the data for the table ZTPPVR
*
* Modification Logs
* Date       Developer    RequestNo    Description
************************************************************************
REPORT ZAPP914R_UPLOAD_ZTPPVR     NO STANDARD PAGE HEADING
...
DATA: BEGIN OF IT_AEHD OCCURS 0,
        FLAG(002)            TYPE C             ,
        P_STATUS(03)         TYPE C             ,
        P_RP_SERIAL(05)      TYPE C             ,
        P_MODEL(04)          TYPE C             ,
        P_BODY_SERIAL(6)     TYPE C             ,
        P_PLANT_NO(1)        TYPE C             ,
        K04PDAT(10)          TYPE C             ,
        K04SER(5)            TYPE C             ,
        P_LINE_NO(1)         TYPE C             ,
        P_WORK_ORDER(14)     TYPE C             ,
        P_DEST_CODE(5)       TYPE C             ,
        P_INT_COLOR(3)       TYPE C             ,
        P_EXT_COLOR(3)       TYPE C             ,
        P_VIN(18)            TYPE C             ,
        P_RP_ACTUAL_DATE(10) TYPE C             ,
        P_RP_ACTUAL_TIME(06) TYPE C             ,
        DVRS(01)             TYPE C             ,
        MODEL2(4)            TYPE C             ,
        BODY_NO2(6)          TYPE C             ,
        ZUSER(12)            TYPE C             ,
...
*&---------------------------------------------------------------------*
FORM UPDATE_ZTPPVR        .
  DATA LT_AEHD LIKE ZTPPVR         OCCURS 0 WITH HEADER LINE.
  LOOP AT IT_AEHD.
...
*-> Upload after all deletion
    WHEN R1.
      ...
      INSERT ZTPPVR         FROM TABLE LT_AEHD ACCEPTING DUPLICATE KEYS.
      IF SY-SUBRC EQ 0.
        COMMIT WORK.
      ELSE.
        ROLLBACK WORK.
...
*-> Upload except deletion
    WHEN R2.
      MODIFY ZTPPVR         FROM TABLE LT_AEHD.
      IF SY-SUBRC EQ 0.
        COMMIT WORK.
      ELSE.
        ROLLBACK WORK.
