************************************************************************
* Program Name      : ZIPP106I_PMT03NB_RFC
* Author            : Bobby
...
************************************************************************
REPORT zipp106i_pmt03nb_rfc NO STANDARD PAGE HEADING
*                          LINE-SIZE 120
                          MESSAGE-ID zmpp.
*----------------------------------------------------------------------*
...
*----------------------------------------------------------------------*
DATA : it_pms03nb   LIKE TABLE OF zspp_pmt03nb WITH HEADER LINE,
       it_pmt03nb   LIKE TABLE OF ztpp_pmt03nb WITH HEADER LINE.
...
FORM excute_process.
  ...
  CLEAR : it_pmt03nb, it_pmt03nb[].
  SELECT *
         INTO TABLE it_pmt03nb
         FROM ztpp_pmt03nb
         WHERE zresult EQ 'E' OR zresult EQ space.

  IF sy-subrc EQ 0.
    DESCRIBE TABLE it_pmt03nb LINES l_size.
...
*      l_count = l_count + 1.
* End of change.
      MOVE-CORRESPONDING it_pmt03nb TO it_pms03nb.
      APPEND it_pms03nb.
      IF l_count >= l_size.
        l_curr = l_curr + 1.
        CALL FUNCTION 'Z_FPP_SET_PMT03NB_2'
          DESTINATION  c_dest
              EXPORTING
            total_c              = l_idx
            curr_c               = l_curr
          TABLES
            i_ztpp_pmt03nb       = it_pms03nb
          EXCEPTIONS
        ...
        IF sy-subrc <> 0.
          ...
          wa_error = 'X' .          EXIT.
        ELSE.
          LOOP AT it_pms03nb WHERE zzret = 'E'.
            wa_error = 'X' .
            PERFORM create_log USING 'E' 1  text-001  l_msgtxt .
            PERFORM create_log USING 'R' 1  text-001  l_msgtxt .
          IF sy-batch = ' '.
