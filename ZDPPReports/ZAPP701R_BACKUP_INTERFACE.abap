...
************************************************************************
REPORT zapp701r_backup_interface LINE-SIZE 132  MESSAGE-ID zmpp.
...
FIELD-SYMBOLS: <desc>, <check>, <current>, <backup>, <date_field>.
...
SELECTION-SCREEN COMMENT 49(7) text-004.
SELECTION-SCREEN COMMENT 56(7) text-005.
SELECTION-SCREEN COMMENT 64(15) text-006.
...
FORM backup_processing.
  ...
  WRITE: 5 'Active Table'.
  WRITE: 22 'Total Recoreds'.
  WRITE: 50 'Backup Table'.
  WRITE: 68 'Total Recoreds'.
  ULINE /5(80).
  DO 18 TIMES.
    REFRESH: i_alv_cat.
...
** Furong on 05/31/12
    clear: l_date_c.
      CONCATENATE  l_sign l_date_cd l_sign
          into l_date_c.

    CONCATENATE <date_field> '<=' l_date_c
...
*      LOOP AT <f_fs> INTO <wa_archive>.
*        ASSIGN COMPONENT <date_field> OF STRUCTURE <wa_archive>
*                                           TO <wa_field>.
*        IF sy-subrc = 0 AND <wa_field> > l_date_cd.
*          DELETE TABLE <f_fs> FROM <wa_archive>.
*        ENDIF.
*      ENDLOOP.
...
** Furong on 05/31/12
    clear: l_date_c.
      CONCATENATE  l_sign l_date_bd l_sign
          into l_date_c.
...
*----------------------------------------------------------------------*
FORM check_data.
  DATA: w_tabname LIKE ztpp_if_status-tabname,
        wa_text(33).
  CLEAR wa_count.
  DO 18 TIMES.
    wa_count  = wa_count + 1.
...
*        FROM ztpp_if_status WHERE tabname = <table>.
*    IF sy-subrc NE 0.
*      MESSAGE e001 WITH text-w02 <table>.
*    ENDIF.

*    READ TABLE it_dd02t WITH KEY tabname = <table>.
    ...
    CONCATENATE wa_tablename_bk '_BK' INTO wa_tablename_bk.
    ASSIGN wa_tablename_bk TO <table_bk>.
*    select single ddtext into wa_text from dd02t
*           where tabname = <table_bk>
*           and ddlanguage = 'EN'.
*    IF sy-subrc nE 0.
*       MESSAGE e001 WITH text-w04 <table_bk>.
*    ENDIF.
...
*  IF it_ztpp_if_status[] IS INITIAL.
*    MESSAGE e001 WITH text-002.
*  ENDIF.
*  wa_count = 0.
*  LOOP AT it_ztpp_if_status.
*    wa_count = wa_count + 1.
*    CONCATENATE 'p_tb_' wa_count INTO wa_tablename.
*    ASSIGN (wa_tablename) TO <table>.
*    MOVE it_ztpp_if_status-tabname TO <table>.
*    IF NOT it_ztpp_if_status-tabname IS INITIAL.
*      READ TABLE it_dd02t WITH KEY tabname = it_ztpp_if_status-tabname.
*      IF sy-subrc EQ 0.
*        CONCATENATE 'w_tn_' wa_count INTO wa_tn.
*        ASSIGN (wa_tn) TO <desc>.
*        MOVE it_dd02t-ddtext+0(33) TO <desc>.
*      ENDIF.
*      CONCATENATE 'p_ch_' wa_count INTO wa_ch.
*      ASSIGN (wa_ch) TO <check>.
*      MOVE 'X' TO <check>.
