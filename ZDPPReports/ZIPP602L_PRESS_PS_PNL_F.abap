...
FORM READ_PROCESS.
  ...
  CASE C_MARK.
    ...
    WHEN P_RETR.    " Re-transfer
      PERFORM SELECT_ZTPPPS_PNL.
  ENDCASE.
...
*----> SELECT MARA & MARC
  SELECT A~MATNR
         ...
         WHERE A~WERKS EQ P_WERKS
           ...
           AND A~FEVOR EQ C_SPP       " Prod Scheduler  'SPP'
...
*----> Read Classification
  ...
  SORT IT_ZSPPPS BY MATNR.

ENDFORM.                    "SELECT_MARA_MARC
*&---------------------------------------------------------------------*
*&      Form  SELECT_ZTPPPS_PNL
*&---------------------------------------------------------------------*
FORM SELECT_ZTPPPS_PNL.
  ...
  LOOP AT *IT_ZTPPPS.
    ...
    APPEND IT_ZSPPPS .  CLEAR IT_ZSPPPS .
  ENDLOOP.
ENDFORM.                    " SELECT_ZTPPPS_PNL
*&---------------------------------------------------------------------*
*&      Form  DISPLAY_ZSPPPS_PNL
*&---------------------------------------------------------------------*
FORM DISPLAY_ZSPPPS.
  LOOP AT IT_ZSPPPS.
...
*&---------------------------------------------------------------------*
FORM CREATE_INTERFACE_LOG.
*  DESCRIBE TABLE IT_ZTPPPS_PNL LINES Z_TOTAL.
*  CHECK Z_TOTAL <> 0.
...
*    'Plant'                                '5'      ''   ,
    W_COL_POS 'PRS_PNL_MWC'    'PRS_PNL_MWC'   'X'    ''      ''
    'Main W/C'                             '8'      ''   ,
...
*    W_COL_POS 'PRS_PNL_MWC'    'PRS_PNL_MWC'   'X'    ''      ''
*    'Main W/C'                             '8'      ''   ,
    W_COL_POS 'PRS_PNL_SWC'    'PRS_PNL_SWC'   'X'    ''      ''
    'Alt W/C'                              '8'      ''   ,
    W_COL_POS 'PRS_PNL_VMDL'   'PRS_PNL_VMDL'   ''    ''      ''
    'Body Model'                           '4'      ''   ,
    W_COL_POS 'PRS_PNL_SPNLN'  'PRS_PNL_SPNLN'  ''    ''      ''
    'Alter Panel #'                        '12'     ''   ,
    W_COL_POS 'PRS_PNL_PLTN'   'PRS_PNL_PLTN'   ''    ''      ''
    'Pallet No.'                           '12'     ''   ,
    W_COL_POS 'PRS_PNL_OHQ'    'PRS_PNL_OHQ'    ''    ''      ''
    'On Hand Pallet Qty'                   '7'      'EA' ,
    W_COL_POS 'PRS_PNL_BLKN'   'PRS_PNL_BLKN'   ''    ''      ''
    'Blank # '                             '12'     ''   ,
    W_COL_POS 'PRS_PNL_DTYP'   'PRS_PNL_DTYP'   ''    ''      ''
    'Die Type'                             '1'      ''   ,
    W_COL_POS 'PRS_PNL_COLN'   'PRS_PNL_COLN'   ''    ''      ''
    'Coil #'                               '14'     ''   ,
    W_COL_POS 'PRS_PNL_DIEN'   'PRS_PNL_DIEN'   ''    ''      ''
    'Die #'                                '12'     ''   ,
...
FORM BUILD_SORT.
*  W_SORTCAT-SPOS           = 1.
*  W_SORTCAT-FIELDNAME      = 'WERKS'.
*  W_SORTCAT-TABNAME        = 'IT_LIST'.
*  W_SORTCAT-UP             = 'X'.
*  APPEND W_SORTCAT.

  W_SORTCAT-SPOS           = 1.
  W_SORTCAT-FIELDNAME      = 'PRS_PNL_MWC'.
  W_SORTCAT-TABNAME        = 'IT_LIST'.
  W_SORTCAT-UP             = 'X'.
  APPEND W_SORTCAT.
...
ENDFORM.                    " EVENT_BUILD
*&---------------------------------------------------------------------*
*&      Form  MODIFY_ZTPPPS_PNL
*&---------------------------------------------------------------------*
FORM MODIFY_ZTPPPS_PNL.

  MODIFY ZTPPPS_PNL FROM TABLE IT_ZTPPPS.
  IF SY-SUBRC EQ 0.
    COMMIT WORK.
  ELSE.
    ROLLBACK WORK.
...
*----> Tranfer to MES
  CALL FUNCTION 'Z_FPP_PRESS_PS_PNL'
    DESTINATION  C_DEST
    TABLES
      T_ENGINE  = IT_ZSPPPS
  EXCEPTIONS
...
*     DISPLAY            = 'D'
  ...
  IF SY-SUBRC EQ 0 .
    LOOP AT IT_VM .
      CASE IT_VM-ATNAM .
        WHEN 'PRS_PNL_MWC'   .    "Main W/C
          IT_ZSPPPS-PRS_PNL_MWC   = IT_VM-ATWRT .
        WHEN 'PRS_PNL_SWC'   .    "Sub  W/C
          IT_ZSPPPS-PRS_PNL_SWC   = IT_VM-ATWRT .
        WHEN 'PRS_PNL_VMDL'  .    "Body Model
          IT_ZSPPPS-PRS_PNL_VMDL  = IT_VM-ATWRT .
        WHEN 'PRS_PNL_SPNLN' .    "Alter Panel #
          IT_ZSPPPS-PRS_PNL_SPNLN = IT_VM-ATWRT .
        WHEN 'PRS_PNL_PLTN'  .    "Pallet No
          IT_ZSPPPS-PRS_PNL_PLTN  = IT_VM-ATWRT .
        WHEN 'PRS_PNL_OHQ'   .    "On Hand Pallet Qty
          IT_ZSPPPS-PRS_PNL_OHQ   = IT_VM-ATWRT .
        WHEN 'PRS_PNL_TQP'   .    "Total Pallet Qty
          IT_ZSPPPS-PRS_PNL_TQP   = IT_VM-ATWRT .
        WHEN 'PRS_PNL_BLKN'  .    "Press Blank Parts No
          IT_ZSPPPS-PRS_PNL_BLKN  = IT_VM-ATWRT .
        WHEN 'PRS_PNL_DTYP'  .    "Type of Blank Die
          IT_ZSPPPS-PRS_PNL_DTYP  = IT_VM-ATWRT .
        WHEN 'PRS_PNL_COLN'  .    "Coil #
          IT_ZSPPPS-PRS_PNL_COLN  = IT_VM-ATWRT .
        WHEN 'PRS_PNL_DIEN'  .    "Die #
          IT_ZSPPPS-PRS_PNL_DIEN  = IT_VM-ATWRT .
      ENDCASE.
    ENDLOOP.
  ENDIF.
