...
FORM read_step2_table
        USING lt_ppc_step2_agg TYPE ppcpr_type_tab_step2_agg
              ...
              lt_seldt         TYPE ppcpr_postdate_range
              lt_seluser       TYPE ppcpr_username_range
  ...
  FIELD-SYMBOLS:
    <comps> TYPE ppc_apocomplist_ext.
...
* are there any entries in ppc_step2 according to input date?
  SELECT DISTINCT accassobj post_date
      ...
      WHERE post_date IN lt_seldt
        AND crname IN lt_seluser.
  IF sy-dbcnt = 0.
...
*   ...determine the rmprofile, plant and matnr for the cost collector
    SELECT SINGLE rmprofile plant version materialnr        "#EC *
        FROM ppc_ord_inf
        INTO (ls_ppc_step2_agg-rmprofile,
              ls_ppc_step2_agg-plant,
              ls_ppc_step2_agg-version,
              ls_ppc_step2_agg-baugr)
       WHERE accassobj = ls_ppc_step2_agg-accassobj.    "#EC CI_NOFIELD
    IF sy-subrc <> 0.
      CLEAR  ls_ppc_step2_agg-rmprofile.
      APPEND ls_ppc_step2_agg TO lt_failed_ccoll.
...
*   ... determine backflushing logical system
    SELECT SINGLE conflogsys flg_info_dest                  "#EC *
          FROM  ppc_head
          INTO  (ls_ppc_step2_agg-confsys,
                 ls_ppc_step2_agg-flg_dest)
          WHERE accassobj = ls_ppc_step2_agg-accassobj
            AND postdate  = ls_ppc_step2_agg-post_date. "#EC CI_NOFIELD
    IF sy-subrc <> 0.
...
*   ...for every cost collector, fill the matpostid table
    SELECT matpostid
       INTO  TABLE ls_ppc_step2_agg-indexlist
       FROM  ppc_step2
       WHERE accassobj = ls_ppc_step2_agg-accassobj AND
             post_date = ls_ppc_step2_agg-post_date AND
             crname IN lt_seluser.

*   ...for every cost collector, fill the components table
    SELECT matnr
           werks
           charg
           SUM( confquant ) AS confquant
           SUM( delta_confquant ) AS delta_confquant
           confunit
           sobkz
           pspnr
           kzvbr
       ...
       INTO  CORRESPONDING FIELDS OF
             TABLE ls_ppc_step2_agg-complist
       FROM  ppc_step2
       WHERE accassobj = ls_ppc_step2_agg-accassobj AND
             post_date = ls_ppc_step2_agg-post_date AND
             crname IN lt_seluser
...
*   ...for every component, fill the order number
    LOOP AT ls_ppc_step2_agg-complist ASSIGNING <comps>.
      <comps>-aufnr = ls_ppc_step2_agg-aufnr.
    ENDLOOP.

*   ...count the components of the specified cost collector
    DESCRIBE TABLE ls_ppc_step2_agg-complist
      LINES ls_ppc_step2_agg-compcount.

*   ...add the cost collector row to the big table
    APPEND ls_ppc_step2_agg TO lt_ppc_step2_agg.
    CLEAR ls_ppc_step2_agg.
