************************************************************************
* Program Name      : ZRPP207R_BODY_INPUT_PLAN
* Author            : KIM GIL-HYUN (Tonkey)
* Creation Date     : 02/11/2004
* Specifications By : B. Choi
* Pattern           : 2.1
* Development Request No : UD1K907246
* Addl Documentation: YRPP207R_BODY_INPUT_PLAN_OLD
* Description       : Body Input Plan.
*
* Modification Logs
* Date         Developer    RequestNo    Description
* 03/22/2005 wskim        UD1K914858   add weekly data
*
************************************************************************
REPORT  zrpp207r_body_input_plan MESSAGE-ID zmpp .

TABLES: ztpp_common_vals, "[PP] COMMON Information Table
        ztpp_alc_binput,  "[PP] Body Input Plan Table
        ausp.
...
DATA: BEGIN OF it_data OCCURS 0.
        INCLUDE STRUCTURE ztpp_alc_binput.
DATA: END OF it_data.
...
FORM build_comment USING    p_gt_header TYPE slis_t_listheader.
 ...
 DATA: l_serial LIKE ztpp_input_plan-serial,
        ...
        l_rp01 LIKE ztpp_input_plan-rp01,
        l_rp01_time LIKE sy-uzeit,
        l_rp01_date LIKE sy-datum,
        l_rp01_time_char(8),
        l_bodyno(9).
...
SELECT MAX( serial ) INTO l_serial
   FROM ztpp_input_plan
    WHERE status = '01'.
   select single modl body_ser rp01
     INTO (l_model, l_body, l_rp01)
    FROM ztpp_input_plan
    WHERE serial = l_serial.

  CONCATENATE l_model l_body INTO l_bodyno.

  select single atwrt into l_rp01_serial
    from ausp as a
    inner join cabn as b
    on a~atinn = b~atinn
    where objek = l_bodyno
      and klart = '002'
      and atnam = 'P_RP01_SERIAL'.

   write: l_rp01+8(6) to l_rp01_time.
   write: l_rp01+0(8) to l_rp01_date.

   WRITE l_rp01_date TO l_date .

  ls_line-typ = 'S'.
  CONCATENATE 'AS OF :'  l_date(10)
              INTO ls_line-info SEPARATED BY ' '.
  WRITE: l_rp01_time TO l_rp01_time_char.
  CONCATENATE ls_line-info l_rp01_time_char(8) INTO ls_line-info
              SEPARATED BY ' '.

  CONCATENATE ls_line-info l_bodyno 'SEQ'  l_rp01_serial
              INTO ls_line-info SEPARATED BY ' '.
