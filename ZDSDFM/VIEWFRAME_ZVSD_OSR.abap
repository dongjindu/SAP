*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVSD_OSR
*   generation date: 12/07/2012 at 11:20:59 by user HIS20158
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVSD_OSR            .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVSD_OSR'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVSD_OSR_EXTRACT
TOTAL = ZVSD_OSR_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVSD_OSR'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVSD_OSR-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVSD_OSR_EXTRACT
TOTAL = ZVSD_OSR_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVSD_OSR-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVSD_OSR'
...
TOTAL = ZVSD_OSR_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVSD_OSR-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVSD_OSR-UPD_FLAG NE SPACE.
